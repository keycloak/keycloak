...
There is also new key provider `ecdh-generated` for generating realm keys and the support for ECDH algorithms is added into Java KeyStore provider.
...
There are improvements in the DPoP (OAuth 2.0 Demonstrating Proof-of-Possession) preview feature. The DPoP is now supported for all grant types.
Until the previous release, it was supported only for the `authorization_code` grant type. There is also support for the DPoP token type on the UserInfo endpoint.
...
The condition based on the client-attribute was added into Client Policies. This condition can be used to specify that it is applied for the clients
with the specified client attribute having specified value. It is possible to use either AND or OR condition when evaluating this condition as mentioned in the documentation
for client policies.
...
The OpenID for Verifiable Credential Issuance (OID4VCI) is still experimental feature in {project_name}, but it was greatly improved in this release. Lots of development and discussions
happen in the https://github.com/keycloak/kc-sig-fapi[Keycloak OAuth SIG]. Anyone from the Keycloak community is welcome to join.
