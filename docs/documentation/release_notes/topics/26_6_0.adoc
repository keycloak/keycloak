...
When managing a user's group membership through user administration APIs, administrators now need this scope on the target group in addition to the user-side `manage-group-membership` scope.
The existing `manage-membership` scope keeps its current behavior and is no longer used as a bridge for user-side membership operations.
