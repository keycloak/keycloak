...
configuration in the respective client authentication flows. Additionally, server administrators can use the SPI configuration options `--spi-client-authenticator-client-jwt-lenient-audience-validation=true`and `--spi-client-authenticator-client-secret-jwt-lenient-audience-validation=true` to configure the old audience validation behavior.
