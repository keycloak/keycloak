...
Metrics and health checks endpoints are not accessible through the standard {project_name} server port anymore.
As these endpoints should be hidden to the outside world, they can be accessed on a separate default management port `9000`.
It allows to not expose it to the users as common {project_name} endpoints in Kubernetes environments.
