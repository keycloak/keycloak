...
The cookie `KC_AUTH_STATE` was removed and it is not set anymore by {project_name} server as server does not need this cookie anymore.
...
In Keycloak 23 release, there was improvement added for the case when user authenticated in parallel in multiple browser tabs. However it did not handled well the case when authentication session
expired. Now for the case when user is already logged-in in one browser tab and authentication session expired in other browser tabs, {project_name} is able to redirect back to the client
application with some OIDC/SAML error, so client application can immediately retry authentication, which should usually automatically login the application because of the SSO session. More
details in the link:{adminguide_link}#_authentication-sessions[{adminguide_name} authentication sessions chapter].
