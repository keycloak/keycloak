...
There is ability for required actions to be configurable. Realm administrator can configure some required actions in the admin console in the *Required actions* tab of particular realm. At
this moment, the *Update password* is the only built-in configurable required action. It supports setting *Maximum Age of Authentication*, which is the maximum time user is able to update his password
...
actions during authentication, all actions are sorted together regardless of whether those are actions set during authentication (for instance by `kc_action` parameter) or actions added to the user account manually by administrator.
...
The support for Passkeys conditional UI was added. When Passkeys preview feature is enabled, there is dedicated authenticator available, which allows to select from list of available passkeys accounts
and authenticate user based on that. Thanks to https://github.com/tnorimat[Takashi Norimatsu] for the contribution.

= Default client profile for SAML

The default client profile to have secured SAML clients was added. When browsing through client policies of some realm in admin console, there is now new client profile `saml-security-profile` added. When it is used, there are
security best practices applied for SAML clients like for instance signatures are enforced, SAML Redirect binding is disabled and wildcard redirect uris are not allowed.
...
There is work in progress on the support of OpenID for Verifiable Credential Issuance (OID4VCI). Right now, this is still work in progress, but things are being gradually added. The {project_name}
