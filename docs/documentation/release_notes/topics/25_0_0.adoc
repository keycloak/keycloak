...
There is possibility to use HTTP Header `Accept: application/jwt` when invoking token introspection endpoint. When enabled for particular client, it will allow to return claim `jwt` from the
