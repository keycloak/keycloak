...
The SCIM API received extensive improvements in this release.
Users can now manage multivalued user attributes through SCIM, mapped through User Profile.
Read and write operations enforce User Profile view, edit, and mutability permissions, and Fine-Grained Admin Permissions are applied to search filters.
Search and listing performance has been significantly improved, especially for large user bases.
Several RFC 7643 compliance issues have been addressed, including case-insensitive matching, `caseExact` defaults, and a configurable `maxOperations` limit.
User creation behavior with federation providers is now documented and tested.
...
The SCIM (System for Cross-domain Identity Management) API provides a standards-based interface for managing users and groups across realms. It enables seamless integration with identity management systems and applications that support the SCIM protocol.

In this release, the SCIM API is promoted from preview to supported.
