...
Hostname patterns match against DNS hostnames and IPv4 addresses. IPv6 addresses must use bracket notation (for example, `[2001:db8::1]`); bare IPv6 without brackets is not accepted. Port suffixes are always rejected. Patterns are matched against the bare hostname or IP address without brackets.
