...
== Token Exchange Delegation for Clients (experimental)

The experimental token exchange delegation feature now supports client-level delegation (`delegation:client:<client_id>` scope), allowing users to authorize a client application, such as an AI agent, to act on their behalf through OAuth consent. For more details, see the link:{securing_apps_token_exchange_link}#_client-delegation[Client delegation] section.
