...
Organization invitations can now carry realm roles and client roles that are granted when the invitee joins the organization. This removes the need for event listeners or manual role assignment after an invitation is accepted. For details, see the link:{adminguide_link}#_organization_invitation_roles[{adminguide_name}].
