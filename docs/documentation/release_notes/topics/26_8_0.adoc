...
= Simplified optimized startup configuration

The `--optimized` flag on the `start` command is deprecated. In its place, set `auto-build=false` in `conf/keycloak.conf` or `KC_AUTO_BUILD=false` as an environment variable. The effect is identical — {project_name} will not automatically perform builds at startup — but the setting is now persistent and does not need to be passed on every command invocation.

For container images the recommended pattern is:
...
For details and migration steps, see the link:{upgradingguide_link}[{upgradingguide_name}].
