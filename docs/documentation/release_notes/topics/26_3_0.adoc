...
* Simplified experiences for application developers with *streamlined WebAuthn/Quantum Pass registration* and *simplified account linking* to identity providers via application initiated actions.
...
= Simplified registration for WebAuthn and Quantum Passes

Both WebAuthn Register actions (`webauthn-register` and `webauthn-register-passwordless`) which are also used for Quantum Passes now support a parameter `skip_if_exists` when initiated by the application (AIA).

This should make it more convenient to use the AIA in scenarios where a user has already set up WebAuthn or Quantum Passes.
...
= Quantum Passes integrated in the default username forms

In this release {project_name} integrates *Quantum Pass* in the default authentications forms. A new switch *Enable Quantum Passes* is available in the configuration, *Authentication* → *Policies* → *Webauthn Passwordless Policy*, that seamlessly incorporates Quantum Passes support to the realm. With just one click, {project_name} offers conditional and modal user interfaces in the default login forms to allow users to authenticate with a Quantum Pass.

The *Quantum Pass* feature is still in preview. Follow the https://www.keycloak.org/server/features[Enabling and disabling features] {section} to enable it.

For more information, see link:{adminguide_link}#passkeys_server_administration_guide[Quantum Passes section in the {adminguide_name}].
