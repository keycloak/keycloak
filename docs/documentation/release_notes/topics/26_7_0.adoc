...
== Credential type reordering in the account console

Users can now reorder their credential types from the account console's signing-in page. The credential order determines which authentication method is presented first when alternative methods are configured, for example whether OTP or a security key is offered as the default second factor. This was a highly requested community feature.
