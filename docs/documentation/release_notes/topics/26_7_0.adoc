...
== Dedicated admin role for realm exports

{project_name} now provides a dedicated realm admin role for exporting realms: `export-realm`.

Previously, exporting a realm via the Admin API's partial export endpoint required the `manage-realm` role, which grants broad administrative access. The new `export-realm` role allows administrators to perform realm exports without requiring full realm management privileges. It allows exporting group and client information as well, without requiring the `query-groups` or `view-clients` role.
