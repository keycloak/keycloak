...
Step-up authentication allows applications to require a stronger level of authentication for sensitive operations — for example, prompting for a second factor before — even when the user already has an active session. Previously, this capability was only available for OpenID Connect clients.
