...
the parameter `prompt=create` to notify {project_name} that a new user should be registered rather than the existing authenticated user. Initiating user registration was already supported in {project_name} with the use of dedicated endpoint `/realms/<realm>/protocol/openid-connect/registrations`,
...
The OIDC authentication request supports a limited amount of additional custom parameters of maximum length. The additional parameters can be
used for the custom purposes (for example, adding the claims into the token with the use of the protocol mappers). In the previous versions, the maximum count of
...
if additional parameters will cause a request to fail or if parameters will be ignored.
