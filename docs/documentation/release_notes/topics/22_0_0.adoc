...
= Account Console Feature Flags Removed

Two of the feature flags available to Account Console V2 and V3 were unused.  They were therefore removed.  It is possible that if a developer extended the account console, he or she could make use of thse flags.  However, it would not be recommended.

The flags are `isEventsEnabled` and `isTotpConfigured`.