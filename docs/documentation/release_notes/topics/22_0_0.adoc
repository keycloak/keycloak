...
The initial plans was to remove it much earlier, but was kept for two reasons. Firstly, to provide
sufficient enough time for everyone to migrate to the v2 of the Account Console. Secondly, it was heavily used through
our tests, which required a fair bit of effort to update the tests to rely on the Account APIs instead of the console itself.
