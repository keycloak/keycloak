...
For more details, see the link:{operatorguide_link}[{operatorguide_name}].

= Additional datasources configuration is supported

Users have the need to connect to the existing external database and be able to integrate it with {project_name}.
The configuration of the external database connection can be specified via the additional datasource configuration that is supported now.
