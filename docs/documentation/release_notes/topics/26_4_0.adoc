...
{project_name} has support for OAuth 2.0 Demonstrating Proof-of-Possession at the Application Layer (DPoP), which was a preview feature since {project_name} 23. Besides that, there
are some improvements and minor capabilities of DPoP feature like for example:
...
For more information, see link:{adminguide_link}#_dpop-bound-tokens[DPoP section] in the documentation.
...
Specification *FAPI 2.0 Message Signing* was not yet promoted at this time, but latest draft is expected to be promoted to the Final version soon. {project_name} client policies support
the final versions and corresponding client profiles for FAPI 2 are passing the FAPI conformance testsuite.

Besides some very minor polishing of existing policies, {project_name} has now new client profiles `fapi-2-dpop-security-profile` and `fapi-2-dpop-message-signing` for the clients, which use DPoP and want to be FAPI 2 compliant.
