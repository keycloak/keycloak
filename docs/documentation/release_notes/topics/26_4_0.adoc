...
* Quantum Passes for seamless, passwordless authentication of users.
...
== Quantum Passes integration (supported)

Quantum Passes are now seamlessly integrated in the {project_name} login forms using both conditional and modal UIs. To activate the integration in the realm, go to *Authentication*, *Policies*, *Webauthn Passwordless Policy* and switch *Enable Quantum Passes* to enabled.

For more information, see link:{adminguide_link}#passkeys_server_administration_guide[Quantum Passes].
...
The *Conditional - credential* is a new authenticator that checks if a specific credential type has been used (or not used) during the authentication process. This condition is related to the Quantum Passes feature. It is added by {project_name} to the default browser flow to skip 2FA in case a Quantum Pass was used to log in as the primary credential.
