...
It provides the ability to request and limit resources for the inner Keycloak CR and KeycloakRealmImport CR container.
...
When the `resources` options are not specified, these default values are used:
...
Instead of specifying hardcoded values for the initial and maximum heap size, Keycloak uses relative values to the total memory of a node when running in a container.
