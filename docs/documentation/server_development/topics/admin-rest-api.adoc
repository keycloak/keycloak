...
* `client_id` has `Client authentication` on
* `client_id` has the `Service accounts roles` option enabled
* Assign `client_id` the `admin` role in the "Service accounts roles" tab.
