...
The pom.xml file for your SPI requires a `dependencyManagement` section with an import reference to the {project_name} version that is intended for the SPI.  In this example, replace both occurrences of `VERSION` with {project_versionMvn}, which is the current version of {project_name}.
