...
Support for {project_name} SPIs is limited to what is documented in this guide. Moreover, SPI can be considered supported just if the method `isInternal` of the `Spi` described above returns `false` for the
particular SPI.
