...
Moreover it is recommended that your SPI implementation has dependency just on the {project_name} supported modules like `keycloak-server-spi` or `keycloak-model-storage`. Other modules
(for instance `keycloak-server-spi-private` or `keycloak-services`) are considered internal and method signatures here can change without further notice.
