...
registered by annotating the class with `org.keycloak.provider.KeycloakProvider`. The annotation ships in
`keycloak-server-spi` alongside `ProviderFactory`, so no additional dependency is required:
...
No service configuration file is needed in this case. The SPI is derived from the `ProviderFactory` interface the class
implements, and configuration, ordering and the `enabled` option work exactly as for factories registered through a
...
* does not implement `ProviderFactory`,
