...
User Storage SPI provider implementations are packaged and deployed similarly to Jakarta EE components. They are not enabled by default, but instead must be enabled and configured per realm under the `User Federation` tab in the administration console.
