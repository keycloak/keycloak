...
NOTE: To align with Keycloak's built-in transaction per request lifecycle, you should use only blocking, synchronous endpoints. Examples of asynchronous endpoints include
the usage of `AsyncResponse`, or returning `CompletableFuture` or `CompletionStage`.  
...
. Create the file `META-INF/services/com.acme.ExampleServiceProviderFactory` and add the factory implementation class to it. For example:
+
[source]
----
com.acme.ExampleServiceProviderFactoryImpl
----
