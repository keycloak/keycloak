...
An organization member is basically a realm user but with a link to one or more organizations. They are logically separated
...
.Managing organization members
image:images/organizations-users-section.png[alt="Managing organization members from the Users section"]
...
an account created through an identity provider whose link to the organization is configured with the Managed membership type. That account does not belong to a realm as it was federated from the organization.
...
The membership type can be configured proactively when linking an identity provider to an organization. Each identity provider
link carries a membership type setting that determines whether users auto-added through that provider become managed or unmanaged
members. For details, see <<_managing_identity_provider_,Managing identity providers>>.

== Changing membership type

You can change the membership type of an existing organization member using the Admin REST API.

`PUT /admin/realms/{realm}/organizations/{orgId}/members/{memberId}/membership-type`

The request body should contain the membership type as a JSON string: `"MANAGED"` or `"UNMANAGED"`.

WARNING: Changing the membership type affects the lifecycle of the member's account. When changing from *Unmanaged* to *Managed*,
the member's account becomes bound to the organization -- removing the member from the organization or deleting the organization will also remove the account from the realm.
Conversely, changing from *Managed* to *Unmanaged* means the account will persist in the realm even if the member is removed from the organization, but the user may lack local credentials if originally federated via an identity provider.
...
[[_organization_invitation_roles]]
=== Granting roles with an invitation

When sending an invitation through the Admin REST API, you can include the ids of realm roles or client roles using the
`roles` form parameter of the `invite-user` and `invite-existing-user` endpoints. The roles are granted to the user in the
same transaction that adds the user as a member of the organization, whether the invitation is accepted through the
invitation link, the registration flow, or an identity provider.

The administrator sending the invitation must be allowed to map each of the roles to the user, using the same permissions
required to grant the roles directly. When the invitation is for a user who does not have an account yet, the administrator
must be allowed to map roles to any user in the realm. Roles deleted before the invitation is accepted are removed from the
invitation, and resending an invitation keeps its roles.
...
users federated from the identity provider can be automatically added as members of the organization, provided the identity provider link has auto membership enabled and the user's email domain passes the domain gate.

When users join an organization through an identity provider, the membership type assigned to them
depends on the per-association configuration of the identity provider link. By default, new links use the unmanaged membership type,
but this can be changed to managed when linking the identity provider to the organization. Users go through the broker login flows
configured in the realm and join the organization once they successfully authenticate, subject to the per-association and domain gate checks described in <<authenticating-members_{context},Authenticating members>>.
...
the email domain matches the organization, and a domain is configured to route to an identity provider with *Auto redirect*
enabled, the user is automatically redirected to the identity provider. Once the user authenticates at the identity provider
and completes the first broker login flow, the user is automatically added as an organization member.

On the other hand, if *Auto redirect* is not enabled on the domain, but the identity provider is configured not to
...
. To view group memberships from the *Members* tab:
.. Navigate to your organization.
.. Click the *Members* tab.
.. Locate the member.
.. Click the kebab menu (⋮) next to the member.
.. Select *Show organization group memberships*.
. To view group memberships from a specific group:
.. Navigate to your organization.
...
.. Locate the member.
.. Click the kebab menu (⋮) next to the member.
.. Select *Show memberships*.
. To view group memberships from the user's detail page:
.. Click *Users* in the menu.
.. Select the user.
.. Click the *Organizations* tab.
.. Locate the organization.
.. Click the kebab menu (⋮) next to the organization.
.. Select *Show organization group memberships*.
