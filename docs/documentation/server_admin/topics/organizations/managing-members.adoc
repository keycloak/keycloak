...
The administrator sending the invitation must be allowed to map each of the roles, using the same permissions required to
grant the roles directly to a user. Roles deleted before the invitation is accepted are not granted, and resending an
invitation keeps the roles that still exist.
