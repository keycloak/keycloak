...
When a new user follows an invitation link and then authenticates using an identity provider during registration,
the invitation remains bound to that authentication session across the broker redirect and first broker login flow.
After successful broker authentication, the user joins the organization targeted by the invitation, and the
invitation is consumed.

In this invitation-driven broker onboarding path, membership is created as *Unmanaged*, consistent with existing-user
invitation acceptance.
The organization is determined by the invitation link itself, and invitation consumption is authorized only when the
resulting user email matches the invitation email.
This behavior applies whether the selected identity provider is linked to the organization or is available as a
realm-level provider during registration.
...
If the previous invitation was still pending, that previous invitation is replaced and can no longer be consumed.
