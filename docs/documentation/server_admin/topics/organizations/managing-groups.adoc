...
When enabled, realm and client roles assigned to the user's organization groups are included in the token for each organization. See <<oidc-token-with-roles,Token structure with roles>> for the output format.
...
NOTE: These roles are specific to the organization context and appear inside the `organization` claim. Standard `realm_access` and `resource_access` claims already include roles from all groups (including organization groups) regardless of this mapper setting.
...
Add group role mappings::
When enabled, realm and client roles assigned to the user's organization groups are included as additional SAML attributes. See <<saml-assertion-with-roles,SAML assertion structure with roles>> for the output format.
...
==== SAML assertion structure with roles
...
Organization groups support realm, client, and organization role mappings. This lets you define access that applies to all members of a group. Organization role mappings are limited to regular roles from the same organization. Every member inherits the default organization role through the hidden internal root group, so the default role cannot be assigned to a visible group.

=== Assigning realm and client roles
...
. Click *Assign role*.
. Select one or more realm or client roles.
. Click *Assign*.
...
To also include these roles within the `organization` claim (organized per organization), enable the *Add group role mappings* option on the *Organization Group Membership* mapper. See <<oidc-token-with-roles,Token structure with roles>> and <<saml-assertion-with-roles,SAML assertion structure with roles>> for details.
...
. Select an organization group.
. Click the *Role mapping* tab.
. Click *Assign role* and select *Organization roles*.
. Select one or more roles from the organization.
. Click *Assign*.

Members inherit these roles from the selected group and its parent groups. The roles appear in the `organization` claim when the *Organization Role Membership* mapper is enabled. The role picker only lists regular roles from the current organization that the administrator is allowed to map; it does not list the default role.

NOTE: Assigning organization roles to an organization group requires permission to manage the organization and to map each selected role.
...
|Role assignment
|Supported—realm, client, and regular roles from the same organization can be assigned via the Admin API and Admin Console
