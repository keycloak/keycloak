...
Organization groups support realm, client, and organization role mappings. This lets you define role-based access that applies to all members of a group.
...
. Click the *Role mapping* tab.
. Click *Assign role*.
. In the role selection dialog, make sure you're viewing *Organization roles*:
  - Look for a filter or tab at the top of the dialog
  - Select *Organization roles* to view only roles scoped to this organization (instead of realm or client roles)
. Select one or more organization roles from the list.
. Click *Assign*.

Organization roles assigned to groups are automatically included in token claims for members of those groups. They appear in the `organization` claim structure, just like directly assigned organization roles.

NOTE: Assigning organization roles to organization groups requires the `manage-organizations` realm admin role (or equivalent Fine-Grained Admin Permission on the organization) and the `map-role` permission on the specific role being assigned.
