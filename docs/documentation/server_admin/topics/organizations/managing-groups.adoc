...
When a user authenticates, the assertion includes attributes for each organization they belong to:
...
Each organization gets its own attribute named `organization.{alias}.groups` with group paths as values.
...
Use organization groups when:
- You need team structures specific to each organization
- Different organizations need the same group names independently
- You want to map organizational structure to tokens for tenant-aware applications

Use realm groups when:
- You need groups shared across the entire realm
- You're configuring authorization policies
- You need to assign roles to groups
...
Organization groups can be managed programmatically.

=== Create a top-level group

[source,bash]
----
POST /admin/realms/{realm}/orgs/{orgId}/groups
{
  "name": "Engineering"
}
----
...
[source,bash]
----
PUT /admin/realms/{realm}/orgs/{orgId}/groups/{groupId}/members/{userId}
----
...
[source,bash]
----
DELETE /admin/realms/{realm}/orgs/{orgId}/groups/{groupId}/members/{userId}
----
...
[source,bash]
----
GET /admin/realms/{realm}/orgs/{orgId}/groups
----
...
[source,bash]
----
GET /admin/realms/{realm}/orgs/{orgId}/groups/{groupId}/members
----

For complete API documentation, refer to the {project_name} Admin REST API reference.
