...
This chapter documents the proof types supported by OpenID for Verifiable Credential Issuance (OID4VCI), how to configure trusted attestation keys, and how key binding works in the resulting credentials.
...
| `aud`
| Yes
| Must be the Credential Issuer Identifier (e.g., `https://localhost:8443/realms/{realm}`)
