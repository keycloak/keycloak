...
[NOTE]
====
This feature is in preview and is not fully supported. It is disabled by default and can be enabled with `--features=preview` or `--features=oid4vc-vci`. Backward compatibility is not guaranteed, and future updates may introduce breaking changes.
====
...
https://github.com/{project_name}/{project_name}-oauth-sig/tree/main/oid4vci-deployment[OID4VCI deployment].
