...
With this Access Token, the Verifiable Credential can be issued at the credentials endpoint.
...
You can create the ClientScopes using the {project_name}'s Webadministration Console, but the Webadministration Console
does not yet support adding metadata configuration. For metadata configuration you will need to use the Admin REST-API.

Client scopes define **which user attributes** are included in Verifiable Credentials (VCs). Therefore, they are considered the Verifiable Credential configuration itself. These scopes use **protocol mappers** to map specific claims into VCs. The protocol mappers also contain the corresponding metadata for the claim they represent, which is exposed via the credential issuer's metadata endpoint.

You can create ClientScopes using the `{project_name}` Web Administration Console. However, the Web Console does not currently support adding metadata configuration. For that, you will need to use the Admin REST API.
...
- It is important to set `include.in.token.scope=true`
