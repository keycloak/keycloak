...
* The self-signed root certificate, or trust anchor, is not included in the `x5c` JOSE header. If the configured chain ends with a self-signed root certificate, {project_name} removes it when creating the SD-JWT VC.
...
. Store the private key and its certificate chain in a supported Java keystore. The private-key entry must contain the leaf certificate first, followed by any intermediate certificates and, optionally, the root certificate.
...
[NOTE]
====
The keystore may contain the self-signed root certificate so that {project_name} can load and validate the complete chain. The root certificate is not emitted in the SD-JWT VC `x5c` header.
