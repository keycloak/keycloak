...
There is the switch *Include in token scope* on the client scope. If on, the name of this client scope will be added to the access token property `scope` as well as to the Token Response and Token Introspection Endpoint
...
Linking between a client scope and a client is configured in the *Client Scopes* tab of the client. Here is how it looks like for the client application `myclient`:
...
=== Client dedicated client scope

There is special client scope, which is linked to every client. It is dedicated client scope, which is always shown as a first client scope when you click to the tab *Client scopes* of the particular client.
...
It is not possible to unlink the dedicated client scope from client. Also it is not possible to link this dedicated client scope to some different client. In other words, the dedicated client scope is useful
just for protocol mappers and role scope mappings, which are specific to a single client. In case you want to share same protocol mapper configuration among multiple clients, it is usually useful to create
a client scope in the realm tab *Client scopes* and then link this shared client scope to every client, which should apply this shared configuration.
...
NOTE: In the admin REST API and in the internal {project_name} storage, the dedicated client scope does not exists as it's protocol mappers and role scope mappings are internally linked to the client itself. The
...
The term _scope_ is used in various contexts in {project_name} and in the various OAuth/OIDC specifications. Here is some clarification of various _scopes_ used in {project_name}:
