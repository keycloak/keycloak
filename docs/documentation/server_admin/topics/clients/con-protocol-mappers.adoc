...
[[_attribute-store-mapper]]
== Attribute Store Mapper

Administrators can set claims on access tokens generated by Keycloak with attributes from a configured `Attribute Store`.
When multiple mappers are configured for the same Attribute Store provider, attributes will be cached in the user session
so requests to the external store are only issued once for the duration of the user session.

.Configuration

. Go to `Client scopes`
. Click `Create client scope` to create a new client scope
. Select `OpenID Connect` as the `Protocol`
. Select the `Mappers` tab
. Click `Configure a new mapper` and select `Attribute Store`
. Configure the `JSON Pointer Expression` to extract the attribute value from the received attributes and specify the `Token Claim Name` to set on the token
. Click `Save`
image:images/create-attribute-store-mapper.png[]