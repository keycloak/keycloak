...
* Exchange the lightweight access token for a full access token using link:{securing_apps_token_exchange_link}[Token Exchange] (see *Converting a lightweight access token to a regular access token* above), then call UserInfo with the full token
+
