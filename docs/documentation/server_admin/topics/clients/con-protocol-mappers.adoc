...
However it is not strictly needed as the *Subject (sub)* protocol mapper is executed before *Pairwise subject identifier* mapper and hence the pairwise value will override the value added
by the Subject mapper. This is due the <<_protocol-mappers_priority, priority>> of the Subject mapper. So the only advantage of removing built-in *Subject (sub)* mapper might be to
save a little bit of performance by avoid using protocol mapper, which may not have any effect.
