...
*Encrypt Assertions*:: Encrypts the assertions in SAML documents with the specified client public key. The AES algorithm uses a key size of 128 bits by default. You can configure all the encryption details in the *Advanced* tab when *Encrypt Assertions* option is enabled.
...
*Encryption algorithm*:: Encryption algorithm used for the client. Default value is `AES_CBC_128`.

*Key transport algorithm*:: Key transport algorithm used for the client to encrypt the secret key used for encryption. Default value is `RSA-OAEP-MGF1P`.

*Digest method for RSA-OAEP*:: Digest method to use when RSA-OAEP is selected as the key transport algorithm. Only available if *Key transport algorithm* is set to any RSA-OAEP algorithm. Default value is `SHA-1` per specification.

*Mask generation function*:: Mask generation function to use when RSA-OAEP-11 is selected as the key transport algorithm. Only available if *Key transport algorithm* is set to any RSA-OAEP-11 algorithm. Default value is `mgf1sha1` per specification.

NOTE: The encryption options are only available if the *Encrypt Assertions* option is enabled in the *Keys* tab. For more information about SAML/XML encryption, please see the link:https://www.w3.org/TR/xmlenc-core1/[XML Encryption Syntax and Processing] specification.
