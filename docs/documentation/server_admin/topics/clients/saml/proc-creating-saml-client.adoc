...
*SAML signing key*:: Select a specific realm key to sign SAML responses and assertions for this client. If not set, the realm's active signing key is used. This setting is useful for <<_rotating-keys-gradually,rotating keys gradually>>.
+
[NOTE]
====
