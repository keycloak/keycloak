...
|The value overrides the realm option with the same name for this client. This option is only available when *Revoke Refresh Token* is set to *Enabled* for the client and is preset with the realm value. When *Revoke Refresh Token* is not set or *Disabled* for the client, the realm setting is used.
