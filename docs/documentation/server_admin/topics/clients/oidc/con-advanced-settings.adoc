...
|The value overrides the realm option <<_revoke-refresh-token,Revoke Refresh Token>> for this client. When set to *Enabled*, refresh tokens issued to this client are rotated on use and their reuse is limited by *Refresh Token Max Reuse*, regardless of the realm setting. When set to *Disabled*, refresh tokens issued to this client can be reused without restriction, even if revocation is enabled for the realm. When not set, the realm setting is used.

|Refresh Token Max Reuse
|The value overrides the realm option with the same name for this client. This option is only available when *Revoke Refresh Token* is set to *Enabled* for the client and is preset with the realm value. When *Revoke Refresh Token* is not set for the client, the realm value is used. When *Revoke Refresh Token* is *Disabled* for the client, refresh tokens can be reused without restriction and this option has no effect.
