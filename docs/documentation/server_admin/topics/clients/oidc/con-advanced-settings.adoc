...
|The value overrides the realm option <<_revoke-refresh-token,Revoke Refresh Token>> for this client. When set to *Enabled*, refresh tokens issued to this client are revoked when used, regardless of the realm setting. When set to *Disabled*, refresh tokens issued to this client can be reused, even if revocation is enabled for the realm. When not set, the realm setting is used.
