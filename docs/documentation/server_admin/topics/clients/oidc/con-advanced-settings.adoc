...
|The value overrides the realm option with the same name for this client. This option is only relevant when refresh token revocation is effective for the client, either through the *Revoke Refresh Token* client override or inherited from the realm. If not set, the realm setting is used.
