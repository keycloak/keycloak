...
A client that is disabled is never added to the `aud` claim of a token:

* The client roles of a disabled client are never added to the token, not even by the _Hardcoded Role_ and _Role Name_ protocol mappers.
Because such roles are not present, the _Audience Resolve_ protocol mapper does not resolve the disabled client as an audience.
