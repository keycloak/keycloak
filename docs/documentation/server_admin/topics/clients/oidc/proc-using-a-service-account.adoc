...
. Verify that the client role scope mappings include the roles required by the service account. Do not enable *Full Scope Allowed*: this switch is deprecated, so explicitly configure the required role scope mappings instead. Details are available in <<_role_scope_mappings, this section>> and <<_oidc_token_role_mappings, this section>>.
