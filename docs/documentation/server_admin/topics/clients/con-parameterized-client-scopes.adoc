...
:tech_feature_name: Parameterized scopes
:tech_feature_id: parameterized-scopes
...
| `user-delegation`
| Extends the `username` type with an additional delegation check. The parameter must be the username (or email address when the realm allows login with email) of an existing, enabled user. The user cannot target themselves, and the target user must be granted the `delegate` permission on the current user through <<_fine_grained_permissions, Fine-grained admin permissions version 2>>.
| No

| `client-delegation`
| Validates that the parameter value is the client ID of an existing, enabled client that has service accounts enabled. The service account of that client must be granted the `delegate` permission on the current user through <<_fine_grained_permissions, Fine-grained admin permissions version 2>>.
| No
...
However, some scope types do not allow this. For example, a `boolean` scope with both `scope:true` and `scope:false` in the same request is contradictory. Similarly, a `user-delegation` scope that identifies a single target user should not appear with multiple values.
...
+
This mapper is applicable when the parameter type is `username` or `user-delegation`. Access control depends on the type:
+
...
* `user-delegation` — the `delegate` permission is already validated during scope resolution, so no additional access control is applied and user property claims are always included.
