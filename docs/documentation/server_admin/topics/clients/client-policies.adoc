...
* *Exclude specific clients using a marker role with Negative logic* — instead of the *Any Client* condition, use the *Client Role* condition with *Negative logic* enabled. Clients that carry a designated marker role will then be excluded from the policy. The steps below describe this approach:
+
...
.. In the client policy, remove the *Any Client* condition and add a *Client Role* condition instead. Set the role name to `fsa-permitted-marker-role` and enable *Negative logic*. Save the condition.
+
With this setup, any client that has the `fsa-permitted-marker-role` client role is excluded from the policy and retains its *Full Scope Allowed* setting unchanged. All other clients, including newly registered ones, are subject to the policy.
