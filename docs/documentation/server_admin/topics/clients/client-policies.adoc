...
NOTE: There is often a use-case for require that particular client policy is applied for the specified clients like `my-client-1` and `my-client-2`. The best way to achieve this
is to use *Client Role* condition in your policy and then create client role of specified name to requested clients. This client role can be used as a "marker role" used solely
for marking that particular client policy should be used for particular clients.
