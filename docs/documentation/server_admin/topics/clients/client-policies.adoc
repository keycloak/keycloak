...
* Enforce that only configured protocol mapper types can be created or updated on clients through the Admin REST API using the `allowed-protocol-mappers` executor. Existing mapper types that are not configured can only be preserved with an unchanged type and configuration.
