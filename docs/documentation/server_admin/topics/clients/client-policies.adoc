...
NOTE: Client Policies are evaluated for protocol mapper create, update, and delete operations on clients through the Admin REST API. They are not evaluated for protocol mapper operations on client scopes.
...
* Enforce that only configured protocol mapper types can be created or updated on clients through the dedicated Admin REST API protocol mapper endpoints using the `allowed-protocol-mappers` executor. Existing mapper types that are not configured can only be preserved with an unchanged type and configuration.
