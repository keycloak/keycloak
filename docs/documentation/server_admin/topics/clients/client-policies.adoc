...
The client policy concept is independent of any specific protocol. {project_name} currently supports especially client profiles for link:{adapterguide_link}#_oidc[OpenID Connect (OIDC) protocol], but there is
also client profile available for the link:{adapterguide_link}#_saml[SAML protocol].
