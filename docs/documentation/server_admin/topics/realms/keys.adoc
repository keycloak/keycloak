...
. Optionally, set a value for the certificate validity in days.
. Click *Save*.

Changing the priority for a provider will not cause the keys to be re-generated, but if you want to change the keysize or reduce the certificate validity
you can edit the provider and new keys will be generated.
Increasing the certificate validity will not regenerate existing keys; the current certificate will simply expire earlier than the new setting.
