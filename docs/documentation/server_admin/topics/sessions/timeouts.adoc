...
|When *Enabled*, {project_name} revokes refresh tokens and issues another token that the client must use. This action applies to OIDC clients performing the refresh token flow. This setting and the related *Refresh Token Max Reuse* can be overridden per client in the <<_client_advanced_settings_oidc,Advanced Settings>> of the client.
