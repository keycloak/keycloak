...
|The maximum time before a user's action permission expires. This value should balance two competing objectives: On the one hand it should be short enough to mitigate a potential leak, while on the other hand it should allow for delayed email delivery, and potentially inexperienced users.
