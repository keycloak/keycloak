...
Your sessions with the application are ended, including any offline sessions, so the application can no longer access {project_name} on your behalf using previously issued tokens. If the application supports it, {project_name} also notifies the application. Because this does not sign you out from {project_name}, the application can sign you in again without asking for your credentials when you access it again. To fully sign out from all applications, use the *Sign out* action on the *Device activity* page instead.
