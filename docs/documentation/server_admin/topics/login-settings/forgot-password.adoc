...
=== Redirect to reset credentials screen directly from the application

Sometimes, application may want to directly redirect to the {project_name} "Reset credentials" screen instead of being redirected to the {project_name} login screen.
The direct redirect to the screen can be done for OIDC client by slightly changing the initial URL of OIDC authentication request as described in <<_registration-rc-client-flows,this section>>.
