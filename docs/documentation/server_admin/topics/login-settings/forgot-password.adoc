...
When users click the email link, {project_name} asks them to update their password, and if they have set up an OTP generator, {project_name} asks them to reconfigure the OTP generator. By default the flow maintains the user logged in if the same authentication session (same browser) was used. Depending on security requirements of your organization, you may not want users to reset their OTP generator through email and/or you may want to force them to re-login after the process.
...
+
If the user needs to re-login after the password change, click *Send Reset Email* settings icon in the flow, define an *Alias* and set *Force login after reset* to *On*.
. Click *Authentication* in the menu.
