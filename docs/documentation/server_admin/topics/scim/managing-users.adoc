...
The SCIM `/Users` endpoint allows you to create, read, update, and delete realm user resources in {project_name}. User resources
...
NOTE: The SCIM `/Users` endpoint only returns users that are stored locally in the {project_name} database. Users
that are managed by external user federation providers (such as LDAP or custom providers) and have not been imported
into {project_name} are not included in SCIM search results. Federated users that have been imported (that is,
synchronized into the local database) are included. This is because {project_name} acts as the identity authority
only for locally stored users - external identity sources should expose their own SCIM endpoints for managing
their users.
