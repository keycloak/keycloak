...
The following table summarizes all available and supported SCIM endpoints.
...
| `/ResourceTypes/{id}`
| GET
| Retrieve a specific resource type by its ID.
...
| `/Schemas/{id}`
| GET
| Retrieve a specific schema by its ID (URN).
|===
