...
If the maximum number secondary authentication failures is exceeded the account is permanently locked. This behavior prevents attacks on second factor authenticators such as OTP when attacker already guessed the password. The reasonable value might be set for `100`.
