...
==== Exclude specific {project_name} authenticator from participating in brute force detection

When authentication flow contains public key based authenticator such as Passkey(WebAuthn) or WebAuthn it might be not desirable to unlock user account and let the brute force attack continue.

An authenticator could be excluded from brute force detection via setting AMR (authentication method reference) on the authenticator and using brute force detection setting `Excluded AMRs`.
If more authenticators need to be excluded use comma separated list their AMRs. This setting is valid for all brute force modes.
