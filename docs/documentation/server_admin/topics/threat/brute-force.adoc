...
* the per-user counter, when the lock policy is *This user only*
* only the selected property whose current value matches the submitted identifier, when the lock policy is *Shared properties only*

*Max login failures* is the threshold for the per-user counter or each identifier counter. Reaching an identifier threshold blocks only that identifier, so a locked email does not prevent login with a phone number or username.

Users that share a property value (for example the same email) share that identifier counter. The existing user unlock action clears all current identifier counters for that user.
...
Property counters belong to the property value, not to an individual user. Changing a user's property does not delete the counter for the old value, and the user inherits any existing counter for the new value. Select `id` instead when protection must follow the stable user id.
====

Locking by user property does not create separate counters for authentication methods or delivery channels. For example, email OTP and SMS OTP attempts are not counted independently.

To configure this:

. Enable a brute force mode other than *Disabled*.
. Choose a *Lock policy*.
. If the policy is not *This user only*, select one or more property ids in *Lock by user properties*.
. Click *Save*.

==== Protect recovery authentication codes independently
Enable *Independent recovery code failures* to give recovery authentication codes their own attempt budget. Recovery code failures then use *Max login failures* without consuming the password or identifier budget. Reaching the limit blocks only recovery code authentication, so another configured authentication method remains available.
