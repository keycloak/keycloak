...
Blind SSRF attack is a type of <<ssrf,SSRF attack>> where the attacker induces an application ({project_name}, in this case) to make requests to an unintended destination but does not return a response body to the attacker. This can be used to map private network infrastructure and services.
