...
This allows an attacker to make {project_name} connect to internal services, including endpoints on the same host, enabling SSRF or internal network scanning depending on authentication and network controls.

To mitigate DNS rebinding attacks, configure the DNS resolver used by {project_name} to reject resolutions of public domain names to non-public/internal IP address ranges.
