...
==== Connecting to multiple LDAP servers for failover
...
The *Connection Timeout* setting controls how long {project_name} waits for a TCP connection to each URL before trying the next one. The default is 5000 milliseconds (5 seconds). If a server is unreachable (for example, due to a network partition or a firewall silently dropping packets), the full timeout is spent on each unreachable URL before failover proceeds. Adjust this value based on your network environment to balance between fast failover and tolerance for slow connections.

NOTE: If a server is cleanly stopped (the port is closed), the connection is refused immediately and failover to the next URL occurs with no noticeable delay.
...
The `Drop non existing LDAP roles` setting is off by default. When enabled, a sync from LDAP to {project_name} removes roles created by this role mapper that are no longer returned by LDAP. The mapper does not take ownership of roles that already existed in {project_name}, including roles with the same name. Roles imported before this setting was introduced are not marked for removal. A failed or incomplete LDAP search stops the sync before any roles are removed. Review changes to the LDAP search base and filter before enabling this setting, because roles excluded by a complete search are considered absent.
