...
Similarly, the `verifiable_credential_offer` action is a parameterized AIA used to trigger verifiable credential issuance. The parameter is a Base64Url-encoded JSON object containing the credential configuration ID and optional fields. For example: `kc_action=verifiable_credential_offer:eyJjcmVkZW50aWFsQ29uZmlndXJhdGlvbklkIjoibWVtYmVyc2hpcC1jcmVkZW50aWFsIn0`. This action is available when the `OID4VC_VCI` feature is enabled. See <<_oid4vci_credential_offer_aia,Credential Offer via Application Initiated Action>> for full details.
