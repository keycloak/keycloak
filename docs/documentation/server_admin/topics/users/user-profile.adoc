...
* Define converters on a per-attribute basis to normalize attribute values (for example, trim whitespace) before validation and storage
...
Converter::
In this section, you can define converters that preprocess the attribute value before validation.
{project_name} provides a set of built-in converters you can choose from with the possibility to add your own. For more details, look at
the <<_converting-attributes,Converting Attributes>> section.
...
[[_converting-attributes]]
== Converting Attributes

Converters preprocess an attribute value before validators run and before the value is stored. They are
useful to normalize user input, for example to strip leading or trailing whitespace that would otherwise
cause two values to be treated as distinct.

Converters are opt-in and must be configured explicitly on an attribute. No converters are applied by
default. They run in the order they are declared in the configuration. If multiple converters are
configured, the output of one converter is fed into the next.

Converters can be added or removed from the `Converters` settings when managing an attribute.

=== Built-in Converters

{project_name} provides built-in converters that you can choose from. You can also provide your own
converters by extending the `Converter SPI`.

The list below provides the built-in converters:
...
|trim-whitespace
|Removes leading and/or trailing whitespace from a string value. Non-string values are returned unchanged.
Recognizes the same whitespace characters as `String#strip()`.
|

*trim-leading*: a boolean to control whether leading whitespace is removed. Defaults to `true`.

*trim-trailing*: a boolean to control whether trailing whitespace is removed. Defaults to `true`.
...
The attribute-level `converters` property can be used to declare converters that preprocess the attribute value before validation runs. Converters are opt-in and applied in declaration order. Each entry is keyed by the converter id with an object containing its configuration.
