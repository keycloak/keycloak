...
|Pass subject
|Controls if {project_name} forwards a `login_hint` query parameter to the IDP. {project_name} adds this field's value to the login_hint parameter in the AuthnRequest's Subject as well as appends `login_hint` query parameter to the browser redirect so destination providers can pre-fill their login form.
