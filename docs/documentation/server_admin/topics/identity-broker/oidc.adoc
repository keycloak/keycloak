...
|Applies only when *Client Authentication* is *X509 Certificate (mutual TLS)*. Selects the realm key provider (for example a `java-keystore` provider) whose private key and certificate chain {project_name} presents as the client certificate on all backchannel calls to the external IDP (token, refresh, introspection, and user info endpoints). The external IDP's server certificate is validated using the {project_name} global truststore. Sourcing the certificate per identity provider allows different realms and identity providers to present different client certificates.
