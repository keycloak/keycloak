...
{project_name} resolves and persists the issuer URL and JWKS URL from the discovery document. Discovery is not
refreshed automatically after save.

When an issuer discovery URL is configured, Keycloak invokes its
`<DISCOVERY_URL>/.well-known/openid-configuration` endpoint when the provider is saved and persists the returned JWKS
...
|Optional issuer URL of service account tokens. If omitted, provide an issuer discovery URL so {project_name} can resolve and persist the issuer and JWKS URL when the provider is saved.

|Issuer discovery URL
|Optional Kubernetes API server or external OIDC discovery base URL. It is used only when the issuer URL is empty; discovery runs when the provider is saved and is not refreshed automatically.
