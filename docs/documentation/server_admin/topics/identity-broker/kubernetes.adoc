...
For managed Kubernetes clusters where service account tokens use an external issuer, leave the issuer URL empty and
provide the Kubernetes API server or another OIDC discovery URL instead. When the identity provider is saved,
{project_name} resolves and persists the issuer URL and JWKS URL from the discovery document. When the provider is
updated with a discovery URL, {project_name} resolves the discovery document again and updates the persisted values.

When an issuer discovery URL is configured, Keycloak invokes its
`<DISCOVERY_URL>/.well-known/openid-configuration` endpoint when the provider is saved or updated and persists the returned JWKS
URL. Otherwise, it discovers keys from `<ISSUER>/.well-known/openid-configuration` when needed. {project_name} uses the token from
`/var/run/secrets/kubernetes.io/serviceaccount/token` to access the in-cluster discovery endpoint. When loading JWKS,
it uses the token when its `iss` claim matches the configured issuer.
...
|Optional issuer URL of service account tokens. If omitted, {project_name} resolves it from the issuer discovery URL when configured; otherwise, it uses the default Kubernetes issuer.

|Issuer discovery URL
|Optional Kubernetes API server or external OIDC discovery base URL. When configured, {project_name} resolves and persists the issuer and JWKS URL when the provider is saved or updated.
