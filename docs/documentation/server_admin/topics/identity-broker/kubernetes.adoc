...
this value by decoding a service account token to retrieve the value of the `iss` claim. Managed Kubernetes clusters
can expose the discovery endpoint through the in-cluster Kubernetes API while issuing service account tokens with a
different issuer URL. In this case, enable automatic issuer discovery. {project_name} resolves the issuer from the
in-cluster discovery endpoint and stores the discovered issuer for client assertion validation.
...
|The issuer URL of service account tokens. The URL `<ISSUER>/.well-known/openid-configuration` must be available to {project_name} when automatic issuer discovery is disabled

|Automatic issuer discovery
|Resolve the service account token issuer from the in-cluster Kubernetes discovery endpoint. Use this option when a managed cluster exposes discovery through the Kubernetes API but uses a different issuer in service account tokens
