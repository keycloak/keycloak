...
The typical use of this authenticator is for instance the scenario like this one:
* You have for instance existing {project_name} user `john` with email `john@gmail.com`, who is already linked with identity provider `google` with `google` username being `john@gmail.com` .
* Then for instance {project_name} user `john` updates his email in {project_name} to `john-new@gmail.com`
* Then during login to {project_name}, the user authenticated to identity provider `google` with his new username like `john-new@gmail.com`, which is not linked to any {project_name} account yet (as {project_name} account `john` is still linked with `google` user `john@gmail.com`) and hence first-broker-login flow is triggered.
...
* Now with this authenticator in the authentication flow, it is possible to override existing IDP link to `google` identity provider of {project_name} user `john` with the new `google` link to `google` user `john-new@gmail.com` after user `john` confirms this.

When creating authentication flow with this authenticator, please make sure to add this authenticator once other authenticators already established {project_name} user by other means (either by re-authentication or after `Detect existing broker user` as mentioned above.
