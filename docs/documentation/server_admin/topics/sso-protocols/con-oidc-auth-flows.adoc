...
This is useful especially for the browser based applications. For example when the application is secured by the link:{securing_apps_javascript_adapter_link}[{securing_apps_javascript_adapter_name}]
then logout from one browser tab can trigger automatic logout from all other browser tabs with applications secured by the javascript adapter.
...
If even the Admin URL is not specified, {project_name} will not propagate logout to the client. This can be still sufficient for many client application deployments. For
instance when the client application is frontend javascript application, it may rely on the <<_oidc-logout-session-management, Session Management>> and hence there is no need to send
dedicated logout request to the client application. Other client applications may just rely on the short <<_timeouts,Access token lifespan timeout>>. This means that the session on the application
side will be still valid for the short time after {project_name} SSO session logout, however when the client application will try to refresh the access token, the token refresh request to {project_name}
will fail due the fact that {project_name} SSO session was already logged out.
...
When the client application supports backchannel logout, it is usually better alternative than the front-channel logout as communication happens directly between client and {project_name} without web browser
involved, which is usually more reliable. Also for example when the {project_name} logout of specified user is triggered by the administrator from the <<viewing-user-sessions,Admin console>> (or other use of admin REST API)
or by the user himself from <<_account-service,the Account console>>, then {project_name} is able to propagate backchannel logout request to the client applications attached to the session being
logged out. This is not supported in case of Front-Channel logout as {project_name} can propagate logout to the client via front-channel just when logout is triggered in the same browser
