...
For example it can look like this for `Reset credentials` flow:

image:images/authentication-user-session-limits-resetcred.png[Authentication User Session Limits Reset Credentials Flow]

For `Browser` flow, it can be beneficial to not add the Session Limits authenticator at the top level flow. This is due the `Cookie` authenticator, which automatically re-authenticates users based
on SSO cookie, is at top level and we do not want to check session limits during SSO re-authentication as user session already exists. So instead, it can be good to add separate ALTERNATIVE
