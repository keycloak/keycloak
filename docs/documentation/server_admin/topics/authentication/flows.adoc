...
NOTE: During very first authentication of the user, the first configured subflow with the *Conditional - Level Of Authentication* is always executed (regardless of the requested level) as
the user does not yet have any level. So it is encouraged that the first level subflow contains the authenticators, which are supposed to be the minimal required authenticators for the user to be
considered authenticated. Also note that for correct functionality, the subflows with different values of *Conditional - Level Of Authentication* should be ordered from the lowest as specified
in the example above. For instance if you configure subflow with level 2 and then additional subflow with level 1 afterwards, the level 2 will be always asked during 1st authentication, which may
