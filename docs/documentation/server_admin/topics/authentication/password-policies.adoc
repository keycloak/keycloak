...
[NOTE]
====
The new policy will not be effective for existing users. So, Make sure that you set the password policy from the beginning of the realm creation and/or add "Update password" to existing users and/or use "Expire password" to make sure that users update their passwords in next "N" days, which will in fact adjust to new password policies.
====
