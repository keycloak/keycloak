...
NOTE: The Maximum Authentication Age is configurable also when configuring required action *Update Password* in the *Required Actions* tab in the admin console. It is preferred to use rather
required action for the configuration as the _Maximum Authentication Age_ password policy might be deprecated/removed in the future.
