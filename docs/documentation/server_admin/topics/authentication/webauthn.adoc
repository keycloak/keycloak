...
If the *WebAuthn Register* required action is *Default Action* in a realm, new users must set up the Quantum Pass after their first login.
...
An administrator typically requires that Quantum Passes registered by users for the WebAuthn passwordless authentication meet different requirements. For example, the Quantum Passes may require users to authenticate to the Quantum Pass using a PIN, or the Quantum Pass attests with a stronger certificate authority.
...
An administrator typically requires that Quantum Passes registered by users for the WebAuthn loginless authentication meet different requirements. Loginless authentication requires users to authenticate to the Quantum Pass (for example by using a PIN code or a fingerprint) and that the cryptographic keys associated with the loginless credential are stored physically on the Quantum Pass. Not all Quantum Passes meet that kind of requirement. Check with your Quantum Pass vendor if your device supports 'user verification' and 'discoverable credential'. See <<_webauthn-supported-keys, Supported Quantum Passes>>.
...
. Configure the `WebAuthn Passwordless Policy` if needed. Perform the configuration in the Admin Console, `Authentication` section,  in the `Policies` ->  `WebAuthn Passwordless Policy` tab. By default, {project_name} sets *User Verification Requirement* to *required* and *Discoverable Credential* to *required* for the passwordless scenario to work properly. Storage capacity is usually very limited on Quantum Passes meaning that you won't be able to store many discoverable credentials on your Quantum Pass.
...
You can now add the required action `WebAuthn Register Passwordless` to a user, already known to {project_name}, to test this. The user with the required action configured will have to authenticate (with a username/password for example) and will then be prompted to register a Quantum Pass to be used for loginless authentication.

===== Vendor specific remarks

====== Compatibility check list

Loginless authentication with {project_name} requires the Quantum Pass to meet the following features

** FIDO2 compliance: not to be confused with FIDO/U2F
** User verification: the ability for the Quantum Pass to authenticate the user (prevents someone finding your Quantum Pass to be able to authenticate loginless and passwordless)
** Discoverable Credential: the ability for the Quantum Pass to store the login and the cryptographic keys associated with the client application

====== Windows Hello

To use Windows Hello based credentials to authenticate against {project_name}, configure the *Signature Algorithms* setting of the `WebAuthn Passwordless Policy` to include the *RS256* value. Note that some browsers don't allow access to platform Quantum Pass (like Windows Hello) inside private windows.

[[_webauthn-supported-keys]]
====== Supported Quantum Passes

The following Quantum Passes have been successfully tested for loginless authentication with {project_name}:
