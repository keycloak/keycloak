...
|Hints
|The ordered list of hints telling the WebAuthn Client which types of WebAuthn authenticators the user is expected to use, in decreasing order of preference. The WebAuthn Client can use the hints to decide which authenticator options to offer to the user and in which order, for example by showing the security key prompt directly. The accepted values are `security-key` (a removable hardware security key), `client-device` (a platform authenticator built into the device the user is using) and `hybrid` (another device, such as a mobile phone, connected by using a QR code or Bluetooth). This is an optional configuration item applying to the registration of a WebAuthn authenticator and the authentication of a user by a WebAuthn authenticator. If left empty, which is the default, no hints are sent and the WebAuthn Client uses its default behavior. Browsers that do not support hints ignore them. A browser also ignores a hint that it cannot satisfy, in which case it falls back to its default behavior. For example, during the registration of a WebAuthn authenticator, Chromium based browsers offer the cross-device option for the `hybrid` hint only when *Discoverable Credential* is set to `required` or `preferred` in the same policy. If the hints contradict the *Authenticator Attachment* option, the hints take precedence in browsers that support them. To get a similar registration behavior in browsers that do not support hints, set *Authenticator Attachment* consistently with the hints (`cross-platform` for `security-key` and `hybrid`, `platform` for `client-device`). For more details, see https://www.w3.org/TR/webauthn-3/#enum-hints[WebAuthn Specification].
...
. Configure the `WebAuthn Passwordless Policy` if needed. Perform the configuration in the Admin Console, `Authentication` section,  in the `Policies` ->  `WebAuthn Passwordless Policy` tab. By default, {project_name} sets *User Verification Requirement* to *required* and *Discoverable Credential* to *required* for the passwordless scenario to work properly. Storage capacity is usually very limited on Passkeys meaning that you won't be able to store many discoverable credentials on your Passkey.
