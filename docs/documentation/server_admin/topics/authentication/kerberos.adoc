...
Finding users is based on the LDAP storage provider option `Kerberos principal LDAP attribute`. When this is configured for instance with value like `userPrincipalName`, then
after SPNEGO authentication of user `john@A`, {project_name} will try to lookup LDAP user with attribute `userPrincipalName` equivalent to `john@A`. If `Kerberos principal LDAP attribute` is left
