...
[WARNING]
====
Credential delegation is deprecated and may be removed in a future release. Credential delegation has significant security implications as it allows credentials to be forwarded and reused. See the warning below for more details. Moreover, the use of protocol mappers and carry kerberos ticket within access token may also have security implications.
Finally, we are not able to properly test it and official support for this feature may not deserve the additional overhead as it seems that feature has not much usage within {project_name} users.
====
