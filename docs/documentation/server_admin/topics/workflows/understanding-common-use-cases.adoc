...
WARNING: When using attribute-based conditions with privileged steps like `grant-role` or `join-group`, ensure the attribute
is not writable by end users. See <<_workflow_security_considerations_>> for details.
