...
IMPORTANT: Only server administrators (with the `admin` role in the master realm) or realm administrators (with the `realm-admin` role) can assign admin roles to other users. Delegated administrators with individual admin roles such as `manage-users` cannot grant any admin roles, even the ones they hold themselves. Similarly, adding a user to a group that has admin roles assigned requires `realm-admin` or `admin` privileges.
