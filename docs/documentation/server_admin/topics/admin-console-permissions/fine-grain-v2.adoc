...
The Fine-Grained Admin Permissions feature allows server administrators to delegate realm management to other administrators, 
the realm administrators. Different than the regular Role-Based Access Control Mechanism provided through the Global and 
Realm specific roles (<<_master_realm_access_control, Master realm access control>>), this feature provides a more fine-grained 
control over how realm resources can be accessed and managed based on a well-defined set of operations that can be performed 
on them.
...
* Fine grained permissions are used to grant additional permissions. You cannot override the default behavior of the 
...
In a realm, you can manage different types of resources such as users, groups, clients, client scopes, roles, and etc. 
...
| `view`                    | Defines if a realm administrator can view the client’s configuration.
| `manage`                  | Defines if a realm administrator can view and manage the client’s configuration.
| `configure`               | (TODO: if we are removing transitive dependencies, what we do with this??) Grants limited 
                              privileges to manage the client, excluding protocol mappers, client templates, and client scopes.
| `map-roles`               | Defines if a realm administrator can assign any role defined by the client to a user.
| `map-roles-composite`     | Defines if a realm administrator can assign any role defined by the client as a composite to 
                              another role.
| `map-roles-client-scope`  | Defines if a realm administrator can assign any role defined by the client to the scope of 
                              another client.
|===
...
- An administrator must have explicit permission assigned for each operation to perform the corresponding action. For example, 
  assigning only `manage` without `view` will prevent the group from being visible.
- The `configure` operation provides a restricted set of management privileges compared to `manage`. It prevents 
  administrators from defining protocol mappers, changing the client template, or modifying the client’s scope.  
====

===== Roles Resource Type

TODO: review this section, I'm not sure how it should behave and if it's described correctly
...
| `map-role`               | Defines if a realm administrator can assign this role to a user. This only grants permission to 
                             map the role, not to perform user role mapping tasks. The administrator must also have `manage` 
                             or role mapping permissions.
| `map-role-composite`     | Defines if a realm administrator can assign this role as a composite to another role. Even if an 
                             administrator can manage client roles, they must have `map-role-composite` privileges for the role 
                             they want to add as a composite.
| `map-role-client-scope`  | Defines if a realm administrator can apply this role to a client’s scope. Even if they can manage 
                             the client, they cannot create tokens for that client containing this role unless this privilege 
                             is granted.
|===

[NOTE]
====
- The `map-role` operation does not grant the ability to manage users or assign roles arbitrarily; the administrator must also 
  have user role mapping permissions.  
- The `map-role-composite` operation ensures that administrators cannot add a role as a composite unless explicitly granted 
  permission.  
- The `map-role-client-scope` operation restricts the ability to apply roles to client scopes, even if the administrator has 
  client management privileges.
====

==== Enabling admin permissions to a realm

To enable fine-grained admin permissions to a realm, follow these steps:
...
Authorization services. Read more about :{authorizationguide_link}#_policy_overview[Managing policies] in the Authorization services 
documentation. 
...
database query to fetch the actual stream of resources. Currently, partial evaluation is only supported for non-federated 
resources, which is a limitation of the current implementation.
...
need to administer, along with realm-related roles such as `view-realm`:
...
If an administrator is responsible for multiple resource types (e.g., both users and groups), they must have all corresponding `query-*` 
roles assigned.
...
Beyond these foundational roles, **fine-grained admin permissions** can be used to define specific administrative capabilities. While fine-grained permissions allow for more granular control over access, they **cannot override the default behavior of built-in admin roles**.  
This means that if an additional admin role is assigned to a realm administrator, permission evaluation will be bypassed, and access will be granted.  
...
===== Allow manage users by group of admins
...
- Create any **Policy** that evaluates to deny (e.g., a user policy with no users selected).
...
- Assign a e.g. **Group Policy** targeting `test-admins` to it.
