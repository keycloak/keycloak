...
Only server and realm administrators can grant admin roles to other users, regardless of whether this feature is enabled. Delegated administrators with individual admin roles such as `manage-users` cannot assign any admin roles, even the ones they hold themselves. This restriction also applies to adding users to groups that have admin roles assigned.
