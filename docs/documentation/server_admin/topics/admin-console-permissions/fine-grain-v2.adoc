...
| *manage-members*          | Defines if a realm administrator can manage group members. Together with `manage-membership` and `view` on the group, also allows creating new users as members of the group.
                              This operation applies to any child group in the group hierarchy.
                              This can be prevented by explicitly denying permission for specific subgroups.
...
[NOTE]
====
To create a new user directly as a member of a group, the delegated administrator needs `manage-members`, `manage-membership`, and `view` scopes on the target group. The `view` scope is required because the group must be resolvable from queries during user creation.
====

====== Group hierarchy and permission inheritance

The `view-members`, `manage-members`, and `impersonate-members` scopes cascade through the group hierarchy. When you grant or deny one of these scopes on a group, the permission applies to all descendant groups unless a more specific permission exists on a descendant.

*Deny cascading:* A deny on a parent group prevents access to members of the parent and all its descendant groups (children, grandchildren, and so on).

*Allow carve-outs:* You can override a parent deny by granting an explicit allow for the same scope on a specific child group. The allow applies only to direct members of that child group. It does not cascade further to the child's own descendants. Members of deeper descendant groups remain denied unless they also have an explicit allow.

A direct `view` permission on a specific user does not override a group membership deny. Group membership denies take precedence over user-level permissions. The only way to restore access to a user in a denied group subtree is to grant an explicit allow for the member scope (such as `view-members`) on the specific group the user belongs to.

For example, given a hierarchy `Department > Team A > Subteam X`:

* Denying `view-members` on `Department` denies access to members of `Department`, `Team A`, and `Subteam X`.
* Adding an explicit allow for `view-members` on `Team A` restores access to direct members of `Team A` only. Members of `Subteam X` remain denied.
* To also restore access to `Subteam X` members, you must add a separate allow for `view-members` on `Subteam X`.
* Granting a direct `view` permission on a specific user in `Team A` does not restore access if the group deny is in effect.

====== Search visibility for group member permissions

When `view-members` is granted on a parent group, members of child groups are accessible via direct user lookup (for example, `GET /admin/realms/{realm}/users/{userId}`) but may not appear in user search or count results. The search mechanism uses a partial evaluation that matches users against the specific groups referenced in the permission, not against their descendants.

To ensure that members of child groups appear in search and count results, grant `view-members` on each child group explicitly.
...
To apply a permission to particular organizations, select *Specific Organizations* when creating the permission and pick them from the list.
With *All Organizations*, the permission applies to every organization in the realm.

A realm administrator who only has permissions on specific organizations also needs the `query-organizations` role to see and list them in the administration console.
For more details on these roles, see <<_realm_access_control, Accessing a Realm administration console as a Realm Administrator>>.
