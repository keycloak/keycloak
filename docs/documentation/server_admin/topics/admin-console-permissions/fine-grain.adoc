...
IMPORTANT: Fine-grained Admin Permissions V1 have been replaced by a new version. Version 1 of the feature is still available, 
but it may be deprecated and removed in future. To enable it start the server with `--features=admin-fine-grained-authz:v1`.
