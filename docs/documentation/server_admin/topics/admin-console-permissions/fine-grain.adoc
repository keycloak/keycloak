...
There's a button on this page called `Create client policy`.  There's a multitude of policies
...
There's one more thing we have to do.  Go to `Users`, select the `sales-admin` user, then go to the `Role Mappings` tab and assign the `query-clients` role to the user.
