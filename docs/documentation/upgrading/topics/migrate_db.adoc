...
On databases that support non-blocking index creation — PostgreSQL, Oracle, MySQL/MariaDB, and Microsoft SQL Server Enterprise, Developer, or Azure SQL editions — {project_name} automatically creates the skipped indexes in the background after startup using online index operations that do not lock the table. On databases without non-blocking support, a warning is logged with the SQL commands to apply manually.
