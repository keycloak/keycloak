...
For example, creating an index on tables with millions of records can be time-consuming and cause a major service disruption.  Therefore, a threshold of `300000` records exists for automatic migration.  If the number of records exceeds this threshold, the index is not created during schema migration.

On databases that support non-blocking index creation — PostgreSQL, Oracle Enterprise Edition, MySQL/MariaDB, and Microsoft SQL Server Enterprise, Developer, or Azure SQL editions — {project_name} automatically creates the skipped indexes in the background after startup using online index operations that allow concurrent reads and writes. On databases without non-blocking support, a warning is logged with the SQL commands to apply manually. To disable automatic creation and log the SQL instead, set `--spi-connections-jpa--quarkus--auto-create-missing-indexes=false`.
...
When you start the server with this configuration, the server checks if the database needs to be migrated. If migration is needed, the required changes are written to the `keycloak-database-update.sql` SQL file in the working directory from where {project_name} was started. You can review and manually run these commands against the database.
...
For an empty database the script also creates the two Liquibase bookkeeping tables, `DATABASECHANGELOG`, which it
records the applied changesets in, and `DATABASECHANGELOGLOCK`. A database that was set up purely by running the
script can therefore be started against by a database user that is not allowed to create tables.

Generating the script is different: even with the manual strategy, {project_name} creates both bookkeeping tables
itself on the database it generates the script against. Use a database user that is allowed to create tables there,
or have an administrator create the two tables up front. When they already exist before the server starts, the
generated script contains no `CREATE TABLE` for them, so it can also be applied to the database it was generated from.
