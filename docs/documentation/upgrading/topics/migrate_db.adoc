...
When you start the server with this configuration, the server checks if the database needs to be migrated. If migration is needed, the required changes are written to the `keycloak-database-update.sql` SQL file in the working directory from where keycloak was started. You can review and manually run these commands against the database.
