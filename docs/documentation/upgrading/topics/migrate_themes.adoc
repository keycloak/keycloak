...
=== Migrating the identity provider link email

The login templates now rely on FreeMarker auto-escaping for message summaries and headers. If a custom login theme disables auto-escaping with `?no_esc` or passes these values through `kcSanitize`, remove that handling so dynamic message text is rendered as text:
...
The base HTML email template now renders the localized message in two parts so the confirmation link stays at the translated `{3}` position. If a custom HTML email theme uses `identityProviderLinkBodyHtml` or `identityProviderLinkBody`, update its message bundles with the translated `identityProviderLinkBodyBeforeLink` and `identityProviderLinkBodyAfterLink` keys. Split the existing `identityProviderLinkBody` value around `{3}` to create those values, keeping the text before and after the placeholder in the corresponding key. Then render the link between them:
...
FreeMarker escapes the provider name, realm, username, URL attribute, and link label. Keep `identityProviderLinkBodyHtml` in custom English message bundles while translated bundles still define that key, so the theme verifier can validate translated HTML values. Do not use `kcSanitize` or `?no_esc` for these dynamic values.
