...
header, `messageBodyParam0` and `messageBodyParam1` for the confirmation and
result bodies, and `nestedIdpAlias` and `nestedIdpUsername` for nested broker
messages. Copy the corresponding marker-based rendering expressions from the
