...
In previous versions of Keycloak, when the `hostname-admin` or `hostname-admin-url` were not specified, the URL for the administration console was based on the incoming request.

In this release, the case is slightly different, as we encountered an inconsistency in claimed behavior in the documentation and CLI options description.
...
If there is a case when the frontend URLs are not specified, then the URL for the administration console is based on the incoming request.
