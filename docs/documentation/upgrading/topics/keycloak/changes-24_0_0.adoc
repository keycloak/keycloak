...
User profile feature is now enabled by default. The feature `declarative-user-profile` is now not available anymore as the user profile is always considered enabled now.
This means that realm switch `User Profile Enabled` is also not available and it was removed from the realm settings. Instead of `User Profile Enabled` switch, there is new switch `Unmanaged attributes` added to the realm settings.
When migrating from previous version, the behaviour is like this:

* For the deployments where the realm switch `User Profile Enabled` was set to ON, the `Unmanaged attributes` switch will be set to `Disabled` after the migration. This means that just user attributes supported explicitly
by user profile will be allowed.

* For the deployments where the realm switch `User Profile Enabled` was set to OFF (which was also the default settings for the deployments with `declarative-user-profile` feature disabled, which was the default),
the `Unmanaged attributes` switch will be `Enabled` after the migration. This means the behaviour should be pretty much the same as in previous versions with user profile disabled. There will be still
`Attributes` tab available in user details in the admin console. Also users themselves are able to set arbitrary attributes through UI pages like registration page, update profile page etc as long as
particular custom themes support it. The custom themes should work like before as well. However it is recommended to possibly update your themes to leverage user-profile (See below in the themes section) and
maybe even remove your custom themes at all if the only reason for them was adding custom attributes. It is also recommended to switch `Unmanaged attributes` to `Disabled` or enabled just for
administrators if possible and rather rely on managed attributes as much as possible.
