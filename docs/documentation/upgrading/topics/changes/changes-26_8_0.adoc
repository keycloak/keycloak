...
=== `EventBuilder` reuse after a terminal operation logs a warning

Calling `success()`, `error()`, `clone()`, or any setter on an `EventBuilder` that has already been dispatched via `success()` or `error()` now logs a warning with a stack trace. The warning is logged once per call site per JVM lifetime. Reusing a builder after a terminal operation can cause event listeners to observe the wrong event type or details. The `Event` passed to `EventListenerProvider.onEvent()` should be treated as read-only.

The correct pattern is to clone the builder *before* firing a side event:
...
In a future version of {project_name}, reusing an `EventBuilder` after a terminal operation will throw an exception.
...
=== Reusing an `EventBuilder` after calling `success()` or `error()`

Calling `success()`, `error()`, or `clone()` on an `EventBuilder` that has already been dispatched is now deprecated and will throw an exception in a future version of {project_name}. Clone the builder before each terminal call. See the notable change above for the correct pattern.
