...
=== OID4VCI feature promoted from experimental to preview

The `oid4vc-vci` feature has been promoted from experimental to preview. If you previously enabled this feature with `--features=oid4vc-vci`, the feature continues to work as before. With the preview status, the feature can also be enabled with `--features=preview`.

The pre-authorized code grant (`oid4vc-vci-preauth-code`), REST credential offer endpoint (`oid4vc-vci-rest-credential-offer`), mdoc format (`oid4vc-mdoc`), and OpenID4VP (`oid4vc-vp`) remain experimental and must still be enabled explicitly.
