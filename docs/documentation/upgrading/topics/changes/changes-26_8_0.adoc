...
A new client policy executor, `secure-client-node-hostname`, is available to address server-side request forgery (SSRF) via the legacy adapter cluster node registration endpoint (`/clients-management/register-node`).
...
Hostnames must be supplied without a port number. IPv6 addresses must use bracket notation (for example, `[2001:db8::1]`). Registrations that include a port suffix (for example, `app.example.com:8443`) are always rejected regardless of the configured patterns.

If you plan to enable this executor, audit the `registeredNodes` map for any affected clients before activating the policy. Any entry that contains a port suffix or uses bare IPv6 notation will no longer be accepted at registration time and will not match the allowed patterns. Re-register those nodes with a bare hostname that satisfies at least one configured pattern, or update the patterns to match the corrected hostnames, before enabling the executor.
