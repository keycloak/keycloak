...
Following the changes made to the X509 client authenticator, in this release the user counterparts also require the **Certificate Authority subject DN** option. The new configuration attribute is multi-valued and identifies the trusted anchor CA (or CAs) for user certificates. The new option is now compulsory in the admin console. Additionally, the option **Revalidate Client Certificate** should always be enabled to get the root certificate for the chain and check its subject DN, so it is now deprecated and no longer presented in the console. Previous configurations still work to maintain backwards compatibility, but the new behavior will be enforced in the next major version.

Note that {project_name}'s trust store (or mTLS store) is shared across all realms. The new configuration allows specifying the correct CAs for user certificates and avoids interference with other unrelated CAs.
...
The experimental AuthZEN endpoints now require the caller to authenticate with a confidential client's access token. Previously, a user's access token was also accepted.
...
The `show-config` command now masks all SPI option values. The command is not currently aware of provider factory configuration property metadata, so it cannot determine whether a property is marked as `isSecret`.
...
{project_name} now runs on Quarkus 3.40 (up from 3.33 in 26.7.0). This upgrade brings internal improvements across the Quarkus platform. While no user-facing configuration changes are required, custom extensions or providers that depend on Quarkus-internal APIs should be retested.
...
The proprietary {project_name} extension for invoking the introspection endpoint with the HTTP header `Accept: application/jwt` instead of `Accept: application/json` is now deprecated. If you have a lightweight access token that you want to exchange for a full access token, use link:{securing_apps_token_exchange_link}[Token Exchange] instead.
...
Several client switches in the admin console under **OpenID Connect Compatibility Modes** were introduced for backwards compatibility with older OIDC/OAuth2 client adapters. These switches are now deprecated and will be removed in a future version. Some were deprecated in previous versions. The newly deprecated switches are:
...
As described in the breaking changes section, the option **Revalidate Client Certificate** is deprecated for the X509 user authenticators. This option was used to revalidate the certificate chain presented by the user against the server trust store. In the future, revalidation will always be performed and the trust store should be configured accordingly.
