...
=== `--optimized` startup flag deprecated in favour of `optimized` configuration option

The `--optimized` flag on the `start` command is deprecated and will be removed in a future release.
...
The preferred approach is now to set the `optimized` option to `true` in your configuration file or via the `KC_OPTIMIZED` environment variable. This has the same effect — {project_name} will reuse the current build — but the setting is persistent and does not need to be repeated on every invocation:
...
# then set the optimized option so that start commands using this image don't rebuild
