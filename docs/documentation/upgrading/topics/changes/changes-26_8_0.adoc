...
Hostnames must be supplied as a DNS name or IP address without a port number. IPv6 addresses must use bracket notation (for example, `[2001:db8::1]`); bare IPv6 without brackets is not accepted. Port suffixes (for example, `app.example.com:8443`) are always rejected regardless of the configured patterns.

Patterns are matched against the address without brackets: an IPv6 address registered as `[2001:db8::1]` must be matched by a pattern written without brackets, for example `^2001:db8::1$`.

If you plan to enable this executor, audit the `registeredNodes` map for any affected clients before activating the policy. Any entry that contains a port suffix or uses bare IPv6 notation will no longer be accepted at registration time. Before enabling the executor:
