...
When two requests change the same entity at the same time, one of them can fail with an optimistic locking conflict in the JPA store.
Before, this returned an HTTP `500 Internal Server Error`. Now it returns an HTTP `409 Conflict` with the error code `conflict`, which means you can retry the request.

This affects write requests in the Admin and Account REST APIs. The most common case is resetting a user's password from more than one request at the same time (`PUT /admin/realms/\{realm}/users/\{id}/reset-password`). See the https://github.com/keycloak/keycloak/issues/50924[GitHub issue].
