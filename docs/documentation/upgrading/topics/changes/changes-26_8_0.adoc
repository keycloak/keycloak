...
When deploying with the {project_name} Operator, `KC_CACHE_EMBEDDED_NODE_NAME` is now set automatically to the Kubernetes pod name (for example, `keycloak-0`) via the downward API.
No additional configuration is needed.
If you previously set the node name manually through `additionalOptions` or the unsupported pod template, you can remove that configuration.
If your configuration explicitly sets `cache-embedded-node-name` or `KC_CACHE_EMBEDDED_NODE_NAME`, your value takes precedence over the Operator default.
