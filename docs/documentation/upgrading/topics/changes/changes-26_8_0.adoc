...
PostgreSQL connections now set `ApplicationName` to `Keycloak`, making them identifiable in `pg_stat_activity`, server logs, and monitoring tools. You can override this via `db-url` or `db-url-properties`.

When using the AWS JDBC Wrapper for Amazon Aurora PostgreSQL, {project_name} now also applies `connectTimeout`, `logServerErrorDetail`, `ApplicationName`, and `assumeMinServerVersion` to the connection. Previously, these properties were skipped for all custom JDBC drivers. The `targetServerType` property remains excluded when using the AWS JDBC Wrapper, as the wrapper has its own failover routing mechanism.
