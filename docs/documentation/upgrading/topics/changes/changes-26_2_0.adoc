...
Built-in X509 client certificate lookup providers will only be able to use request header values when `proxy-trusted-addresses` is unset, or the proxy is trusted. 
