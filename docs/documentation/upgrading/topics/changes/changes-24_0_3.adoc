...
The Account REST endpoint for removing the credential of the user is deprecated. Account console does not use this endpoint from this version. It is replaced by the `Delete Credential` application-initiated
action, which is triggered by Account console when user want to remove the credential of some user.
