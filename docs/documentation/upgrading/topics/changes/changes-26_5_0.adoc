...
When extending the "`Failure reset time`" for your realm, any existing entries will still expire with their original shorter "`Failure reset time`".
When shorting it, the setting will be honored at the time of login, but the entries in the cache still expire only with their original "`Failure reset time`".
Existing entries will still expire if you change to "`Lockout permanently".
Only updated and new entries will use the new setting.
