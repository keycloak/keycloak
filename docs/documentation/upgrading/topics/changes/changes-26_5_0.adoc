...
==== Truststore initialization moved to the server; Operator CRD field deprecated
...
This behavior is enabled by default and can be controlled with the new server options:
...
=== Deprecation of `automountServiceAccountToken` field in Keycloak CRD

Deprecated: The `automountServiceAccountToken` field in the Keycloak CRD is now deprecated and has no effect on the server truststore configuration. To prevent trusting cluster CAs, use the server options above instead. If your policies require not mounting the service account token, manage that at the Kubernetes Pod/ServiceAccount level; note that the truststore behavior is governed by the server options.
