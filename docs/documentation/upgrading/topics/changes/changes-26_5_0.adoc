...
Previously, a cluster-aware scheduled task triggered the removal, where only a single instance would check and remove expired entries.
Each {project_name} node triggered this by default every 15 minutes.
