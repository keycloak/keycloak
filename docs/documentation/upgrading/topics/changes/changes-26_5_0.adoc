...
=== Not recommended to use UserCredentialManager directly in your extensions

If you have user storage extension and you reference the class `UserCredentialManager` from your providers, it is recommended to avoid using this class directly as it might be
...
@Override
public SubjectCredentialManager credentialManager() {
    return keycloakSession.getUserCredentialManager(this);
}
```
