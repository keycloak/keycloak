...
=== Infinispan state-transfer disabled for session caches with persistence

When persistent sessions are enabled, session data is available in the database and Infinispan session caches are
configured to have `num_owners=1`. Consequently, Infinispan's state-transfer provides no additional availability
for session data, whilst adding significant overhead when {project_name} nodes leave or join the cluster. Therefore, we
now disable state-transfer when the persistent-user-session feature is enabled. Similarly, we now also disable state-transfer
for offline session caches when the persistent-users-ssion feature is disabled.
