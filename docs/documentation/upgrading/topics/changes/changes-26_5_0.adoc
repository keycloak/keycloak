...
=== Added Index on `client_id` in `offline_client_session` table

An index has been added on the `client_id` column in the `offline_client_session` table to improve query performance when retrieving offline client sessions by client ID.
