...
To improve failover behavior and startup resilience during network issues, {project_name} now sets a default database connection timeout for selected database vendors (for example MySQL, MSSQL, MariaDB, and Oracle).
For Postgres, the connection timeout already defaults to 10s internally.
