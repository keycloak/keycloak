...
== Updated HTTP status code for invalid_grant errors

Authentication failures resulting in an invalid_grant now correctly return an HTTP 400 status code instead of 401 for Direct Grant, X.509, and UMA Permission Grant flows.
