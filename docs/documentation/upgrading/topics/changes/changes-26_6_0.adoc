...
The `UserSessionProvider` interface receives new methods for efficient streaming of user sessions, in both speed and memory.
The streaming instances are read-only, meaning they will not change.
In addition, the implementation **should** avoid keeping them in memory, in case the invoker needs to analyze all data.
