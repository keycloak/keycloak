...
The 'secure-client-uris' executor has been updated to enforce stricter validation on Client URIs. This aligns with FAPI 2.0 security requirements by mandating TLS (HTTPS) for client endpoints.
...
'Post logout redirect URIs', 'Sector Identifier URI' (Pairwise Subject Identifier mapper configuration), 'Logo URL', 'Policy URL', and 'Terms of Service URL'.
