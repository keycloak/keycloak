...
To improve failover behavior and startup resilience during network issues, {project_name} now sets a default database connection timeout of 10 seconds for selected database vendors (for example MySQL, MSSQL, MariaDB, and Oracle).
For PostgreSQL, the connection timeout already defaults to 10s internally.
You can override these defaults by specifying the corresponding JDBC driver properties in `db-url` or `db-url-properties`.
