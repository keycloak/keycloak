...
=== Database connection timeout no longer causes server startup warning

The `--db-connect-timeout` option now also automatically configures the Agroal connection pool acquisition timeout to twice the connect timeout value.
This prevents the Agroal startup warning:
`WARN [io.agroal.pool] (JPA Startup Thread) Datasource '': Login timeout should be smaller than acquisition timeout .`
