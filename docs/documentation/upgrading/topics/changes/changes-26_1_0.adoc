...
The robots.txt file, previously included by default, is now removed. The desired default behaviour is for Keycloak login/signup pages to not show up in search engine results, and this is accomplished by the `<meta name="robots" content="noindex, nofollow">` tags included in the default Freemarker templates for all themes.
