...
Starting with version {project_name} 26.1, the default changes to `jdbc-ping-udp` configuration of JDBC_PING2 which uses {project_name}'s database to discover other nodes.
As this removes the need for multicast network capabilities, this is a simplification and a drop-in replacement.
In the next major version `jdbc-ping` is planned to be the new default, therefore `jdbc-ping-udp` is now deprecated.
...
The `tcp`, `azure`, `ec2` and `google` transport stacks have been deprecated.
Users should use the TCP based `jdbc-ping` stack as a direct replacement.

The `udp` and `jdbc-ping-udp` transport stacks have been deprecated.
Users should use the TCP based `jdbc-ping` stack.
