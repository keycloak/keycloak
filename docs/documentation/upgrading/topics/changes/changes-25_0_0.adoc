...
{project_name} now does not display the message _You are already logged in_ to the end user when authentication session expires and user is already logged-in. Instead it redirects the error
about expired authentication session to the client application, so client can act on it and restart authentication as described in the link:{adminguide_link}#_authentication-sessions[{adminguide_name} authentication sessions chapter].
