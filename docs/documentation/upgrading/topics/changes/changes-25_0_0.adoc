...
The {project_name} builder image sets the `KC_MANAGEMENT_ENABLED` to `true`, which turn on the management interface for every custom images.
This is the default behavior for now as it comply the secure-by-default principle.
Even {project_name} Operator assumes it is enabled for the custom image.

The `/health` and `/metrics` endpoints are accessible on port `9000`, or you can change it via setting property `--management-port`.
You can also specify the relative path for the management interface via setting property `--management-relative-path`.

The management interface uses different HTTP server than the default {project_name} HTTP server and provides a new set of options.

For more information, check the Management port guide.
