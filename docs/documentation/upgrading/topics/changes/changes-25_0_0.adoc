...
Migrating in-memory sessions for {project_name} 24 is not supported as all {project_name} instances need to be shut down before the upgrade due to a major version upgrade of the embedded cache.

[WARNING]
====
The migration of user sessions only works when the persistent user sessions is enabled when upgrade to {project_name} 25.
...
* If the remote {jdgserver_name} is used in a multi-site setup, update the cache config as outlined in https://www.keycloak.org/high-availability/deploy-infinispan-kubernetes-crossdc[Deploy Infinispan for HA with the Infinispan Operator] {section} with the number of owners and the maximum memory size to reduce the memory usage.
--
