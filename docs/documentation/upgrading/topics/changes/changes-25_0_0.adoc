...
In {project_name} 24 release, we had a change in the password hashing algorithm which resulted in a increased CPU usage. To address that we opted to a different hashing algorithm Argon2 which brings the CPU usage back to where it was prior to the {project_name} 24 release.
