...
{project_name} now defines all cache defaults programmatically. Consequently, the `conf/cache-ispn.xml` file no longer
contains the default cache configurations. It is still possible to modify the cache configurations used by {project_name}
in this file, however a warning will now be logged if the `--cache-config-mutate=true` option is not set. Custom caches
can still be configured without setting this option. To modify a default cache, prefer using the `--cache-...` options instead.

When upgrading an existing deployment, it is recommended to remove all default cache configurations from your existing `conf/cache-ispn.xml`.
...
All {project_name} cache configurations have been removed from `conf/cache-ispn.xml`. Configuration of the default
cache configurations in `conf/cache-ispn.xml`, or a custom file via `--cache-config-file`, has been deprecated and will
result in a warning log unless `--cache-config-mutate=true` is explicitly passed.
