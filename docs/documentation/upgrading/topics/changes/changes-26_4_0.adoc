...
in this file, however a warning will now be logged if the `--cache-config-mutate=true` option is not set. When upgrading
an existing deployment, it is recommended to remove all default cache configurations from your existing `conf/cache-ispn.xml`.
To modify a default cache, prefer using the `--cache-...` options inst.
Custom caches can still be configured without setting the option.
