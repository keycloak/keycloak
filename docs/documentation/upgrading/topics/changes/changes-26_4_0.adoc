...
Breaking changes are the configuration changes that are required for {project_name} to continue to operate normally.
In minor or patch releases, breaking changes are only bug fixes.
