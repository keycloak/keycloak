...
=== Quantum Pass Conditional UI Authenticator requires a feature

The *Quantum Pass Conditional UI Authenticator* authenticator was deprecated in the version 26.3.0, but you can still use it if you enable
`passkeys_conditional_ui_authenticator` during server startup. As a result, you can re-configure authentication flows for Quantum Passes authentication as described in link:{adminguide_link}#passkeys_server_administration_guide[Quantum Passes]. Nonetheless, both this startup option and the *Quantum Pass Conditional UI Authenticator* are deprecated.
