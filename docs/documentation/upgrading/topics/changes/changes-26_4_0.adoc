...
Previously, we supported to configure additional datasources via Quarkus properties that are generally unsupported for {project_name} - which created quite an unpleasant situation.

Due to that, we introduced new options for datasources that should be used, and configuring datasources via the Quarkus properties is **unsupported** from now on.

Check these scenarios/alternatives on how to migrate your configuration:
