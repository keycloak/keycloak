...
in this file, however a warning will now be logged if the `--cache-config-mutate=true` flag is not set. When upgrading
an existing deployment, it is recommended to remove all cache configurations from your existing `conf/cache-ispn.xml` if
you previously relied upon {project_name} default cache configurations. Custom caches can still be configured without setting
the flag.
