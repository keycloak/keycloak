...
The docs also stated to update the `cache-ispn.xml` configuration file.
...
If you previously used a custom `cache-ispn.xml` in order to use volatile sessions, we recommend reverting those changes and use the standard configuration file.
