...
The option `spi-connections-infinispan-quarkus-site-name` is deprecated, and it will be removed in the future.
Use `spi-cache-embedded-default-site-name` instead.
Specifying the site name is no longer required

=== Deprecation of `jboss.site.name` and `jboss.node.name`

Both system properties are no longer valid.
