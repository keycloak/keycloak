...
When a user changes their password, they have a checkbox *Sign out from other devices* which is checked by default.

In previous versions, this logged out only regular sessions.
Starting with this release, it logs out also offline sessions as this is what users would expect to happen given the current screen's design.
