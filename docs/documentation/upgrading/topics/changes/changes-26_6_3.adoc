...
=== Token endpoint error responses for client authentication failures now consistently use 401

The token endpoint now returns `401 Unauthorized` with `invalid_client` for all client authentication failures, consistent with link:https://datatracker.ietf.org/doc/html/rfc6749#section-5.2[RFC 6749 Section 5.2].
