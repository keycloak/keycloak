...
=== Two token endpoint client authentication failure cases now use 401

The token endpoint now returns `401 Unauthorized` with `invalid_client` when client credentials are not configured or when authentication raises an unexpected error, consistent with link:https://datatracker.ietf.org/doc/html/rfc6749#section-5.2[RFC 6749 Section 5.2].

Previously, two cases returned `400 Bad Request` with `unauthorized_client` instead: when a client had no credentials configured, and when an unexpected error occurred during authentication. Both now return `401` with `invalid_client`.

==== Migration

If your application checks for a `400` status code or `unauthorized_client` error in these specific failure scenarios, update it to expect `401` and `invalid_client`.
