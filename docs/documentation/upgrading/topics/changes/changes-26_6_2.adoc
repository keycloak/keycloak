...
NOTE: Refresh and offline tokens are not subject to the audience check, because their `aud` claim is the realm rather than a set of clients, and audience protocol mappers do not apply to them. Such a token can be introspected only by the client it was issued for (`azp`), and the compatibility options above do not relax that rule. {project_name} detects refresh and offline tokens from the token itself, so this applies whether or not `token_type_hint` is provided.
