...
Due to the fact that some web platform features that Keycloak relies on are https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts/features_restricted_to_secure_contexts[only available in secure contexts], and changes made to the cookie behavior, we are officially dropping support for running Keycloak outside of a https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts[secure context].
