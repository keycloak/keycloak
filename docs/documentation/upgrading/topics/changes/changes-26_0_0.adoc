...
To protect users from using it accidentally via a manual configuration in Keycloak's cache XML or via the CLI options, this is now guarded with a feature flag `remote-store-cross-dc`. It is marked as experimental and is therefore not supported. {project_name} 26 will not start with such a configuration and show an error instead unless this experimental feature is enabled.
...
The experimental feature `remote-store-cross-dc` *will be removed in a future minor release*.
