...
{project_name} by default does not use XA transactions. However, this is considered unsafe if more than one datasource is used.
Starting with this release, you need to use XA transactions if you are adding additional datasources to {project_name}.
For the default datasource, you can do this by setting the `--transaction-xa-enabled=true` option. For additional datasources, you need to use
