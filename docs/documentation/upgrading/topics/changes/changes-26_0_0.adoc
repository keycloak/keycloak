...
After upgrading from {project_name} versions 24 or 25, {project_name} will not start because the architecture is no longer supported.
Users need to enable `multi-site` feature and configure the {jdgserver_name} endpoint and credentials using the `cache-remote-*` CLI options.

The experimental feature `remote-store-cross-dc` allows {project_name} to start with the previous architecture but support *will be removed with the next release*.

WARNING: Using an external {jdgserver_name} in single-site deployment to achieve persistent sessions was never supported and {project_name} 26 will not start with such configuration.
The `persistent-user-sessions` feature is enabled by default and the external {jdgserver_name} can be decommissioned.
