...
WARNING: make sure to call `init()` once during application bootstrapping, because otherwise you can end up in the redirect loop.
...
WARNING: if you are calling this method with `onLoad: 'login-required'` make sure to call it once because it sets authenticated state to `false` and you may end up in the redirect loop.
