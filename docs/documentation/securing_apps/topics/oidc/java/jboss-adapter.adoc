...
* xref:jboss7_adapter_rpm[Installing JBoss EAP Adapters from an RPM]
...
ifeval::[{project_product}==true]
* Install on JBoss EAP 7.x if the server *is not* running.
+
[source]
----
$ ./bin/jboss-cli.sh --file=bin/adapter-elytron-install-offline.cli
...
* Install on JBoss EAP 7.x if the server *is* running.
+
[source]
----
$ ./bin/jboss-cli.sh -c --file=bin/adapter-elytron-install.cli
