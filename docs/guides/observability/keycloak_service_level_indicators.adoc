...
* Metrics need to be enabled for {project_name}, and the `http-metrics-slos` option needs to be set to latency to be measured for the SLO defined below. Follow <@links.server id="configuration-metrics"/> {section} for more details.
