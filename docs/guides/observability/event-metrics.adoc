...
By default, the `clientId` and `error` tags are limited to 10000 unique entries to avoid a metrics explosion which can lead to out-of-memory problems in {project_name} or resource exhaustion in downstream observability tools.
