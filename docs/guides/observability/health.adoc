...
* `/health/started` - allows startup probes that are used for initial verification of the application before the liveness probe takes over. This is useful for applications which require additional startup time on their first initialization.
* `/health/live` - if the application is running. If this procedure fails the application can be discarded (terminated, shutdown).
* `/health/ready` - if the application is ready to process requests or not. If this procedure fails the application may be made unavailable to serve new requests.
