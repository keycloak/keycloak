...
When the `health-on-main` option is `true` you may also access the `/health/ready` endpoint on the main HTTP or HTTPS port. When this option is `true` make sure to block unwanted traffic to `/health` at your proxy.
