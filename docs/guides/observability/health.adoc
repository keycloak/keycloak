...
Once the health endpoint is enabled, async startup is enabled by default as well where the HTTP ports open while the boostrap is in progress. See <@links.server id="configuration-production" anchor="server-boostrap-behavior" /> for more details.
