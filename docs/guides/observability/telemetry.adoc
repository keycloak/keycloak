...
WARNING: OpenTelemetry Logs and Metrics are preview/experimental features.
...
The `telemetry-logs-level` option filters which logs are exported to OpenTelemetry, while `log-level` and `log-level-<category>` control which logs {project_name} generates.
