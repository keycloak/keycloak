...
You can change the telemetry configuration via Keycloak CR when using the Operator.
It will help to simplify telemetry settings in your deployment by using the first-class citizen configuration options.

You can change telemetry configuration via Keycloak CR fields as follows:
...
These fields should reflect 1:1 association with `telemetry-*` options that contain more information.
