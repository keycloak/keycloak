...
| Log Sequence Number (LSN) is a number representing a position in the Write-Ahead Log (WAL) stream.

The `Current LSN` shows the latest position recorded by a particular instance.

The `Sent`, `Write`, `Flush` and `Replay LSN` show the latest position for a particular standby which has been sent to it by the primary, written to the file system, safely written to a storage device (flushed from cache), and replayed by the database. Differences between values indicate a lag between individual phases in terms of bytes.

The difference between a standby's `Replay LSN` and the primary's `Current LSN` indicates the overall replication lag for the particular standby in terms of bytes.
...
Monitor CloudNativePG clusters leveraging https://prometheus.io/[Prometheus] and https://grafana.com/[Grafana].

=== Set up monitoring through Prometheus and Grafana 
...
If backups are enabled, use the https://cloudnative-pg.io/plugin-barman-cloud/docs/observability/[metrics exposed by the Barman Cloud Plugin] to monitor the status:
