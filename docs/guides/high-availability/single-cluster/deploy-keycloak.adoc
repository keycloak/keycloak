...
** <@links.ha id="single-cluster-deploy-cnpg-with-backup" /> {section}.
** <@links.ha id="single-cluster-deploy-cnpg-recovery" /> {section}.
...
<4> Specify the URL to your custom {project_name} image. If your image is optimized, set the `startOptimized` flag to `true`.
...
. Create a `ConfigMap` containing the generated CA certificate of the CloudNativePG cluster so that {project_name} can secure the connection to the database.
+
[source,bash]
----
