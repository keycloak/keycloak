...
** <@links.ha id="single-cluster-deploy-aurora" /> {section},
** <@links.ha id="single-cluster-deploy-cnpg" /> {section}.

[#single-cluster-deploy-keycloak-procedure]
== Procedure

Depending on the database you deployed use the relevant procedure for deploying {project_name}.

[#single-cluster-deploy-keycloak-with-aurora-procedure]
=== Procedure for deploying {project_name} with AWS Aurora
...
spec:
  ...
  db:
    vendor: postgres
    url: jdbc:aws-wrapper:postgresql://<AWS_AURORA_URL_HERE>:5432/keycloak?sslmode=verify-full&sslfactory=org.postgresql.ssl.DefaultJavaSSLFactory # <1>
    poolMinSize: 30 # <2>
    poolInitialSize: 30
    poolMaxSize: 30
    usernameSecret:
...
----
<1> Secure the database connection with the `sslmode=verify-full` and `sslfactory` properties.
...
[#single-cluster-deploy-keycloak-with-cnpg-procedure]
=== Procedure for deploying {project_name} with CloudNativePG
. Determine the sizing of the deployment using the <@links.ha id="single-cluster-concepts-memory-and-cpu-sizing" /> {section}.

. Install the {project_name} Operator as described in the <@links.operator id="installation" /> {section}.

. Notice the configuration file below contains options relevant for connecting to the CloudNativePG database from <@links.ha id="single-cluster-deploy-cnpg" anchor="single-cluster-cnpg-connecting" />
...
spec:
  ...
  db:
    vendor: postgres
    url: jdbc:postgresql://cnpg-keycloak-rw.cnpg-keycloak.svc.cluster.local:5432/keycloak?sslmode=verify-full&sslfactory=org.postgresql.ssl.DefaultJavaSSLFactory <1>
    poolMinSize: 30 <2>
    poolInitialSize: 30
    poolMaxSize: 30
    usernameSecret:
...
<4> Specify the `ConfigMap` name that contains the CloudNativePG CA certificate.
The Operator will automatically mount the file in directory `/opt/keycloak/conf/truststores/configmap-<config map name>/<file-name>`
