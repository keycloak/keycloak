...
This document describes the high availability architectures in which to deploy {project_name}.
...
=== Multi-cluster deployments (v1) — deprecated

WARNING: Multi-cluster v1 (the `multi-site` feature) is deprecated and will be removed in a future major release. Use <@links.ha id="multi-cluster-v2-introduction" /> instead. See <@links.ha id="multi-cluster-v2-migrate-from-v1-to-v2" /> for migration instructions.
...
Connect two or more {project_name} clusters without an external {jdgserver_name} cluster using <@links.ha id="multi-cluster-v2-introduction" />.

Advantages::
* Tolerate availability-zone failure
* Tolerate cluster failure
* Simpler architecture: no external {jdgserver_name} cluster required
...
* <@links.ha id="multi-cluster-v2-introduction" />
</@profile.ifCommunity>
