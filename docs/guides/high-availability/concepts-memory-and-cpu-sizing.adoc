...
* By default {project_name} will store user sessions in the database. For the database, allocate per 100 requests/second that update the database (login, logout, refresh token) 0.35-0.7 vCPU and 1400 WriteIOPS on an Aurora PostgreSQL multi-AZ database base on a 2 vCPU `db.t4g.large` instance type (tested up to 400 requests per second).
+
