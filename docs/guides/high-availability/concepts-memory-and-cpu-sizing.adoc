...
+
The database may become a bottleneck when performing many concurrent client credential grant workflows.
With this in mind, it is recommended to size the `users` and `realms` cache accordingly.
Tests show this workflow may take up to 2 entries in the `users` cache and up to 4 entries in the `realms` cache per client, to perform the workflow without database accesses.
