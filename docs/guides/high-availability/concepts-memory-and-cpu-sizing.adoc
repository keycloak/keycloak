...
This section details the use of the user event metrics such as `login` , `refresh_token` ,
`client_login`  to monitor and analyze activities related to user logins,
refresh token requests, and client credential grants respectively.
...
This is a preview feature and can be enabled with a feature flag.
See the link:{links_observability_event-metrics_url}[{observability_event-metrics_name}] for more information.

==== PromQL queries:

Below is an example
on how to use the PromQL queries to calculate the User login request rate and User login request error rate.
These steps can be extrapolated further to measure the other key metrics that are important for your sizing.

NOTE: Please note that, the current user event `login` wouldn't differentiate between password-based logins and cookie-based logins.
...
irate(
  keycloak_user_events_total{
        namespace="runner-keycloak", <1>
        realm="realm-0", <2>
        event="login" <3>
     }[$__rate_interval]
  )
----
<1> Filter by additional tags to identify namespace of {project_name}
<2> Filter by the specific realm in which the user logins rate is calculated
<3> User the `login` event tag in the metric to filter only the login events
...
<2> Filter by the specific realm in which the user logins rate is calculated
<3> User the `login` event tag in the metric to filter only the login events
<4> Fetch all the events which have error tag filled with a non-empty value
