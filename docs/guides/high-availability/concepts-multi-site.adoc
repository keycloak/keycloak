...
be taken offline by the stonith Lambda and will remain offline until the connection is restored and the session data is re-synchronized between the two sites.
| No data loss^3^
| Less than one minute
...
Some requests might receive an error message util the loadbalancer detects Site A has failed.
| No data loss^3^
| Less than one minute
