...
This describes advanced {jdgserver_name} configurations for {project_name} on Kubernetes.
...
* All the memory, resource and database configurations are skipped from the CR below as they have been described in <@links.ha id="deploy-keycloak-kubernetes.adoc" /> {section} already.
...
If you are using multiple sites for {project_name} in a cross-DC setup like <@links.ha id="deploy-infinispan-kubernetes-crossdc" />, the site name must be different in each site.
