...
[NOTE]
====
Alternatively there are CLI configuration options `cache-remote-host`, `cache-remote-port`, `cache-remote-username`, `cache-remote-password` to configure <2> and <3>. 
In that case the `persistence` tag <1> is expected not to be present in the file (default values will be used)
====
