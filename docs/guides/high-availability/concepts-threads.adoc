...
NOTE: This currently applies if virtual threads are disabled.
Since {project_name} 26.1, virtual threads are enabled in both Infinispan and JGroups if running on JDK 21 or higher.
Running {project_name} 26.0 in JDK 21, virtual thread can be enabled by setting the Java system property `-Dorg.infinispan.threads.virtual=true`.
