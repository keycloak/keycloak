...
This guide describes how to deploy an AWS Global Accelerator instance to handle {project_name} client connection failover for multiple
...
DNS caching on the client-side, the implementation ideally needs to provide a static IP address that remains the same
when routing clients to both availability-zones.

In this guide we describe how to route all {project_name} client requests via an AWS Global Accelerator LoadBalancer.
...
----
+
. Update/Create {project_name} Deployment
+
Perform the following on each of the {project_name} clusters:
+
.. Login to the ROSA cluster
...
`DnsName` or `DualStackDnsName` in your browser, and you should be presented with the {project_name} admin console.
