...
WARNING: Multi-cluster v1 (the `multi-site` feature) is deprecated and will be removed in a future major release. Use <@links.ha id="multi-cluster-v2-introduction" /> instead, which simplifies the deployment architecture by removing the external {jdgserver_name} cluster. See <@links.ha id="multi-cluster-v2-migrate-from-v1-to-v2" /> for migration instructions.
