...
<@tmpl.guide
title="Multi-cluster deployments (v1)"
summary="Connect multiple {project_name} deployments in independent {kubernetes} clusters" >

WARNING: Multi-cluster v1 (the `multi-site` feature) is deprecated and will be removed in a future release. Use <@links.ha id="multi-cluster-v2-introduction" /> instead, which simplifies the deployment architecture by removing the external {jdgserver_name} cluster. See <@links.ha id="multi-cluster-v2-migrate-from-v1-to-v2" /> for migration instructions.
...
The following environment is used for regular functional, failure, and performance validation. It is an engineering reference environment, not a prescribed or certified HA architecture.
<@profile.ifProduct>
It is not the only environment in which the documented Red Hat build of Keycloak HA capabilities can be used.
</@profile.ifProduct>
<@profile.ifCommunity>
It is not the only environment in which the documented {project_name} HA capabilities can be used.
</@profile.ifCommunity>
