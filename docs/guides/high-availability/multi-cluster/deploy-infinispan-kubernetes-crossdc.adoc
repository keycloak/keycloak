...
.Wait until the {jdgserver_name} caches are ready
[source,bash,subs="+attributes"]
----
kubectl wait --for condition=Ready --timeout=300s cache.infinispan.org -n {ns} actionTokens
kubectl wait --for condition=Ready --timeout=300s cache.infinispan.org -n {ns} authenticationSessions
kubectl wait --for condition=Ready --timeout=300s cache.infinispan.org -n {ns} loginFailures
kubectl wait --for condition=Ready --timeout=300s cache.infinispan.org -n {ns} work
----
