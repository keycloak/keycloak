...
== Architecture
In the event of a network communication failure between the two sites in a Multi-AZ deployment, it is no
...
Typically in such scenarios a quorum is used to determine which sites are marked as online or offline, however as multi-site
...
include::examples/generated/stonith_lambda.py[tag=stonith-start]
    expected_user = 'keycloak' # <1>
    secret_name = 'webhook-password' # <2>
    secret_region = 'eu-west-1' # <3>
include::examples/generated/stonith_lambda.py[tag=stonith-end]
...
include::examples/generated/ispn-site-a.yaml[tag=stonith-secret]
</#noparse>
---
include::examples/generated/ispn-site-a.yaml[tag=stonith-alert-manager-config]
---
include::examples/generated/ispn-site-a.yaml[tag=stonith-prometheus-rule]
----
<1> The username required to authenticate Lambda requests
