...
WARNING: make sure the `automountServiceAccountToken` is not set to `false` in the {project_name} Custom Resource.
This is required for the {project_name} Operator to be able to access the Kubernetes API in HA setup
