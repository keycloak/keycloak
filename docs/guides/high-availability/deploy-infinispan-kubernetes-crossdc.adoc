...
+
Deadlocks may occur in an active-active setup and the `transaction.mode: NON_XA` ensures that the transaction is rolled back keeping the data consistent.
The `backup.failurePolicy: FAIL` is required in this case.
It will throw an error that allows the transaction to be safely rolled back.
The `transaction.locking` possibles values are `PESSIMISTIC` and `OPTIMISTIC`; `OPTIMISTIC` is not recommended due to its network costs.
+
...
+
It is possible to achieve higher availability at the cost of data consistency.
Set `transaction.mode: NONE` to disable transaction and `backup.failurePolicy` to `IGNORE` or `WARN`, it does not make the request fail when the cross-site replication fails.
The `backup.failurePolicy: IGNORE` will fail silently while `backup.failurePolicy: WARN` also fails silently, but a warning is logged.
+
WARNING: The `backup.strategy: ASYNC` also achieves high availability with a weaker data consistency.
It is deadlock free but conflicts are resolved by dropping updates (one site update wins, the other is dropped).
Although it achieves a better response time, it is not recommended in active-active deployment.
It active-passive deployment, it may cause data lost if a full cluster crashes, since the data is replicated in the background.
+
