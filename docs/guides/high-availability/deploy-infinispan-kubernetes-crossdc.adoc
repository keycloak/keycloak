...
+
====
*Background information and alternative configurations*
...
This allows one site to be updated while the other site is unreachable.
...
Although it achieves a better response time, it is not recommended in active-active deployments as conflicts are likely to occur.
It active-passive deployment, it may cause losing the latest data changes if a full cluster crashes, since the data is replicated asynchronously in the background.
====
+
