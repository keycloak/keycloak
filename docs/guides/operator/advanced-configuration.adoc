...
This is useful if some security policies require that the service account token is not mounted in the pod, but it cannot be `false` if you plan to use an external infinispan cluster or if plan to use the Kubernetes service accounts identity provider or if you have some custom provider logic which requires use of the Kubernetes API.
