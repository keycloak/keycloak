...
{project_name} now supports rolling updates for patch releases by default and the feature is essential. It performs a rolling update if the {project_name} version is the same or when upgrading to a newer patch version in the same `+major.minor+` release stream.
Future versions of {project_name} will use additional information from the configuration, the image and the version to determine if a rolling update is possible to reduce downtimes.
...
Rolling updates for patch releases are now fully supported and enabled by default.

When upgrading to a newer patch version in the same `+major.minor+` release stream using the `Auto` update strategy, the Operator will automatically perform a rolling update without downtime.

[NOTE]
====
The feature flag `rolling-updates:v2` is no longer required as the functionality is enabled by default and is an essential feature.
====
