...
In order for rolling updates to work as expected, check if {project_name} nodes perform a graceful shutdown when receiving a `TERM` signal.

Check the log file of the node shutting down for the following log entry.
If it shows, the shutdown was graceful.

----
INFO  [io.quarkus] (Shutdown thread) Keycloak stopped in ...s
----

See the <@links.server id="containers" anchor="custom-entrypoint-shell-scripts" /> {section} for details for possible causes when it does not show.

=== Verify the readiness probe in the load balancer

Your load balancer should honor the {project_name} readiness probe to stop forwarding traffic to nodes shutting down.

See <@links.observability id="health" /> and <@links.server id="reverseproxy" /> {sections} for details.
