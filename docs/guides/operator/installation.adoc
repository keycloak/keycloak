...
* OLM currently ships a ClusterServiceVersion (CSV) that only declares `OwnNamespace` and `SingleNamespace` as supported install modes. To use `MultiNamespace` or `AllNamespaces`, patch the CSV as described below.
====
...
The CSV published with the Operator currently enables only `OwnNamespace` and `SingleNamespace`. To install in `MultiNamespace` or `AllNamespaces` mode, create the OperatorGroup with the desired namespace selection, subscribe to the Operator, patch the CSV install modes, and then approve the install plan.
...
Patch the installed CSV to enable the `MultiNamespace` install mode. Replace `<csv-name>` with the CSV created for your Operator version, for example `keycloak-operator.v26.2.0`:

[source,bash]
----
kubectl patch csv <csv-name> -n <operator-namespace> --type merge --patch \
  '{"spec":{"installModes":[{"type":"OwnNamespace","supported":true},{"type":"SingleNamespace","supported":true},{"type":"MultiNamespace","supported":true},{"type":"AllNamespaces","supported":false}]}}'
----
...
Patch the CSV to enable the `AllNamespaces` install mode:

[source,bash]
----
kubectl patch csv <csv-name> -n <operator-namespace> --type merge --patch \
  '{"spec":{"installModes":[{"type":"OwnNamespace","supported":true},{"type":"SingleNamespace","supported":true},{"type":"MultiNamespace","supported":false},{"type":"AllNamespaces","supported":true}]}}'
----

If the Operator is already installed in single-namespace mode, you can switch to cluster-wide mode by patching the CSV and removing `spec.targetNamespaces` from the OperatorGroup:
