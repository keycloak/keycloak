...
*Major version upgrades* present the highest risk:
...
* Breaking changes in the Operator's handling of Custom Resources

*Minor version upgrades* generally have lower risk but still require caution:

* Changes in Operator logic might affect how your deployment is managed
* New fields or behavioral changes could impact existing custom configurations

*Recommendation:*

*We strongly recommend using manual approval mode for the Keycloak Operator.* This ensures you can:

1. Review release notes and understand changes before approving upgrades
2. Schedule maintenance windows for upgrades
3. Test upgrades in a non-production environment first
4. Back up the database to allow downgrading to the previous {project_version} in case of issues
====
