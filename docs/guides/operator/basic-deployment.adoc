...
When using the operator-provided ingress (`spec.ingress.enabled: true`) there are a lot of Keycloak CR options (like service and ingress labels and annotations) which allow you to customize your ingress behaviour. This section shows how to achieve the following TLS Termination when using OpenShift:
