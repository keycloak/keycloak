...
[source,bash]
----
openssl req -subj '/CN=test.keycloak.org/O=Test Keycloak./C=US' -newkey rsa:2048 -nodes -keyout key.pem -x509 -days 365 -addext "basicConstraints=critical,CA:FALSE" -out certificate.pem
----
...
:check-lb-forwarded-header: pass:quotes[Check your load balancer or reverse proxy configuration to determine whether it utilizes the `Forwarded` (RFC 7239) or `X-Forwarded-*` (e.g., `X-Forwarded-For`) mechanism for header propagation.]
...
[WARNING]
====
Anyone with the ability to create or edit the CRs used by the operator (Keycloak, KeycloakRealmImport, KeycloakOIDCClient, etc.) should be a namespace-level admin.
====
...
Similarly, the unsupported `podTemplate` gives the ability to deploy alternative workloads and requires a high degree of trust. In a single-namespace operator deployment those workloads may be granted the same permissions as the operator itself - which includes the ability to access Secrets in the namespace.
