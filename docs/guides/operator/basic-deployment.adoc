...
The {project_name} deployment is exposed through a basic Ingress and is accessible through the provided hostname.  

First, you create a reencrypt route using the Ingress certificate. The Ingress Controller negotiates its connection to the application independently of the connection from the client.

[source,yaml]
----
$ oc create route reencrypt --service=<service-name> --cert=<configured-certificate> --key=<certificate-key> --dest-ca-cert=<ca-certificate> --ca-cert=<ca-certificate> --hostname=<hostname>
----

The operator by default creates a passthrough route, so the route creation must be disabled on the Keycloak CR, setting `.spec.ingress.enabled: false:`
