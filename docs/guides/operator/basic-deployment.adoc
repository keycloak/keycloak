...
When using the operator-provided ingress (`spec.ingress.enabled: true`) there are a lot of Keycloak CR options (like service and ingress labels and annotations) which allows you to customize your ingress behaviour. This section show how to achive below TLS Termination when using Openshift:
