...
With passthrough, the client IP address is not visible to {project_name} because the default ingress controller does not support PROXY protocol.
It is the best option when you want to use mTLS (mutual TLS), as the TLS connection is established directly between the client and {project_name}, preserving the client certificate for authentication.
