...
For this reason TLS reencryption is not yet considered supported by basic Ingress, but you may be able to specify the tlsSecret on both the http and ingress as a starting point.
...
TLS passthrough is shown in the preceding example-kc example. It is enabled when you associate a tlsSecret with the http configuration and leave ingress enabled without specifying a tslSecret on it. 

TLS termination, or edge mode, is enabled by associating a tlsSecret with the ingress and by enabling http access.
