...
You can use `hostname-path` or `hostname-url` in these cases, or alternatively you can also change the context path of Keycloak itself to match the context path for the reverse proxy using the `http-relative-path` option. e.g. to configure it using `http-relative-path`, if you want to set Keyclock to host from subpath `/auth` then set `http-relative-path` to `/auth`, see details https://www.keycloak.org/server/all-config?f=build
