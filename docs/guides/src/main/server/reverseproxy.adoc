...
== Hosting keyclock on subpath

When Keycloak is hosted behind a reverse proxy, it is usually served from a subpath (not from rool url), for this to work properly Keyclock should be configured to serve resources from a subpath. Use `KC_HTTP_RELATIVE_PATH` for this, e.g. if you want to set Keyclock to host from subpath `/auth` then set this environment variable to `/auth`, see details https://www.keycloak.org/server/all-config?f=build
