...
As usual, the configuration options can also be set in `conf/keycloak.conf` with follow-up start `build` option.
...
To setup the JPA migrationStrategy you should run the `build` command as follows:

.Setting the `migration-strategy` for the `quarkus` provider of the `connections-jpa` SPI
<@kc.build parameters="--spi-connections-jpa-quarkus-migration-strategy=manual"/>

If you want to get a SQL file for DB initialization, too, you have to add this additional initializeEmpty SPI:
...
<@kc.start parameters="--spi-connections-jpa-quarkus-migration-export=/path_to_file/my_sql_file.sql"/>
