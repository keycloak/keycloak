...
<@kc.start parameters="log-json=true"/>
...
.Configuring the JSON timestamp Date format
To set the format for the timestamp used in JSON Logs, ...

.Configuring the JSON timestamp timezone
To set the timezone for the timestamp, ... (default: system)

.Configuring the JSON exception details
You can log exceptions on different detail levels by ...

.Configuring more detailed logging
On cost of performance, you can configure more detailed logging, including source class name, source file name, source method name and source line number, by...
