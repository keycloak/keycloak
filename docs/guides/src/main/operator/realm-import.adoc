...
.Note:
[NOTE]
The inlined realm definition is limited in the nesting level of `subGroups` and `subComponents`. +
The Kubernetes API doesn't allow full cyclic recursion and those fields are limited to 10 levels of nesting.
