...
|link:https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0-16.html[OpenID for Verifiable Credential Issuance] (OID4VCI)|Preview|| See link:{adminguide_link}#_oid4vci[Configuring {project_name} as a Verifiable Credential Issuer]
...
|===
|Specification|Status|Conformity|Comments
|link:https://datatracker.ietf.org/doc/html/rfc7643[System for Cross-domain Identity Management: Core Schema (RFC 7643)]|Tech Preview||Supports User and Group resource types, including the Enterprise User extension (Section 4.3).
|link:https://datatracker.ietf.org/doc/html/rfc7644[System for Cross-domain Identity Management: Protocol (RFC 7644)]|Tech Preview||Supports CRUD operations, filtering, pagination, PATCH, and schema discovery. Bulk operations are not yet supported.
|===
