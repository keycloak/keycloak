...
WARNING: Client Registration Policies apply to requests made through the Client Registration Service. They are not executed for arbitrary Admin REST API requests.
...
Client Policies are more flexible and support more use case.
For example the case of enforcing client URIs like the JWKS to link:{adminguide_link}#ssrf[prevent SSRF attacks] allowing only trusted patterns or domains.
