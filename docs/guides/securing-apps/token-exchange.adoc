...
Delegation addresses all of these. The delegated token is issued with the actor's own audience and scopes, narrowing it to only what the actor needs. The `act` claim provides a clear audit trail, and the user must explicitly consent to the delegation.
