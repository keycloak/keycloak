...
s|Scope | `delegation:user` | `delegation:client`
s|Actor | User (administrator) | Client application (e.g. AI agent)
s|Use case | Administrator acts on behalf of user | Application acts on behalf of user
s|`may_act` contains | `sub` (admin user ID) | `sub` (service account user ID) + `client_id`
|===

When the feature is enabled, the delegation client scopes (`delegation:user` and `delegation:client`) are automatically created in all realms (new and existing) as *Optional* client scopes, making them available to all clients by default. No manual client scope assignment is needed. The client requesting the delegation scope must have *Consent required* enabled, as the delegation scopes enforce consent on every login. The consent for delegation is not stored permanently, so the user must approve delegation each login session.

Delegation also requires granting a delegation permission through link:{adminguide_link}#_fine_grained_permissions[Fine-grained admin permissions version 2] (FGAP V2). Without a matching permission granting the `delegate` scope, the delegation scope is silently dropped from the token and the `may_act` claim is not included.
...
. Assign a policy that targets the actor that should be allowed to delegate. For admin delegation, use a *User* or *Client* policy targeting the administrator. For client delegation, use a *Client* policy targeting the actor client (for example, `actor-app`).

NOTE: To restrict delegation to specific users, select individual users when creating the permission.
...
. Assign a policy targeting the actor (administrator or client) that should be allowed to delegate for members of that group.
...
==== Why use client delegation instead of sharing the subject token?

Passing the user's original access token directly to an AI agent or service may seem simpler, but it has significant drawbacks:

* *No audience control.* The subject token's `aud` claim targets the original client, not the services the agent needs to call. Resource servers that validate the audience will reject it.
* *No scope restriction.* The subject token carries the user's full permissions. If the agent is compromised, the attacker inherits all of them.
* *No audit trail.* Resource servers cannot distinguish requests made by the user from requests made by the agent on their behalf.
* *No user consent.* The user has no opportunity to approve or deny what the agent can do.

Client delegation addresses all of these. The delegated token is issued with the actor client's audience and scopes, narrowing it to only what the agent needs. The `act` claim provides a clear audit trail, and the user must explicitly consent to the delegation.
...
. {project_name} validates that the specified client exists, is confidential, and has a service account.
...
. The actor client receives the subject's token (containing `may_act`) and performs a token exchange at the token endpoint. It authenticates with its own client credentials and provides both the subject's token as the `subject_token` and its own service account token as the `actor_token`.
. {project_name} validates that `may_act.sub` matches the actor token's `sub` claim and that `may_act.client_id` matches the actor client's `client_id`.
...
By default, the `may_act` claim includes `sub` (service account user ID) and `client_id`. Additional claims can be added by configuring extra parameterized protocol mappers on the `delegation:client` client scope in the Admin Console, following the same approach as described in the admin delegation section above.
