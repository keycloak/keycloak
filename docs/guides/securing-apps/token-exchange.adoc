...
subject_token_type::
    ...
    The value could also be  `urn:ietf:params:oauth:token-type:refresh_token` only if the `Allow refresh token as subject in Standard Token Exchange` client configuration option is enabled as specified in the <<_standard-token-exchange-details,standard token exchange>> section.
    {project_name} does not support other types for the standard token exchange.
...
* Using a refresh token as subject token is allowed only if the client has the switch *Allow refresh token as subject in Standard Token Exchange* set to On.

.Enabling refresh token as subject in Token Exchange
image::token-exchange-switch-subject-refresh.png[Enabling refresh token as subject in Token Exchange]
