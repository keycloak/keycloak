...
. The claim `aud` (audience) should identify the {project_name} server (issuer or token endpoint URL) and must contain only a single value. For security reasons and in line with recent OIDC recommendations, {project_name} rejects JWTs with multiple values in the `aud` claim.
