...
* Executor **jwt-authorization-grant-audience**. The executor allows to configure different and exclusive valid audiences per client for the assertion. The JWT Authorization Grant processing ensures, by specification, that the audience of the assertion is the {project_name} issuer or token endpoint URL. You can also use the Client ID as valid audience (option **Allows Client ID as audience for assertions**) for the OpenID Connect identity providers. Using this executor, you can configure one or more valid audiences for a client instead of the default ones for the requests processed by the client policy.
+
...
To address this issue, you can use link:{adminguide_link}#_client_policies[Client Policies] with the `jwt-authorization-grant-audience` executor to explicitly set the Google ID Token audience as the trusted audience for the specified client, replacing the default audience validation configured for the Identity Provider.

**Example setup:**

Assume you have an OIDC client named `foo`, which is a mobile application receiving a Google ID token with the audience `my-mobile-app.apps.googleusercontent.com`, and you have an already configured Google Identity Provider with a different client-id.

Follow these steps to configure the validation of the audience:

. **Create a Client Policy Profile:** Create a profile and add the `jwt-authorization-grant-audience` executor. Configure the allowed audience in the executor for client `foo` to `my-mobile-app.apps.googleusercontent.com`.
. **Create a Client Policy:** Create a policy that uses this profile. For the condition, you can use the **identity-provider-alias** condition applied to the google Identity Provider.
. **Validate:** When you send the JWT authorization grant request with your Google ID token from client `foo`, the request will now be accepted, provided that `my-mobile-app.apps.googleusercontent.com` is present as a valid audience in the token for that client.
