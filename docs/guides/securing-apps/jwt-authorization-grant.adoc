...
. The claim `aud` (audience) should identify the {project_name} server (issuer or token endpoint URL). The single `aud` is allowed to be present in the JWT. For security reasons and recent OIDC recommendations, {project_name} does not accept JWT with multiple values of the `aud` claim.
