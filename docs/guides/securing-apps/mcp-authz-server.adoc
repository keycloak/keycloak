...
==== Setting up the client profile for Claude Code desktop
...
* *Restrict same domain*: `OFF` (Claude Code desktop uses a localhost redirect URI such as `+http://localhost:<port>/callback+`, which is not on the same domain as `claude.ai`)
* *Only Allow Confidential Client*: `OFF` (Claude Code desktop is a public client)
+
. Click *Save*.

==== Setting up the client policy for Claude Code desktop
...
With this configuration, when Claude Code desktop sends an authorization request, {project_name} recognizes the `client_id` as a URL on `claude.ai`, fetches the Client ID Metadata Document, and uses a localhost callback to complete the OAuth flow.
