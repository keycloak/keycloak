...
NOTE: A build must be performed in order to update the server when JAR file(s) are added or removed. For an already optimized build, this will require running the `build` command again.
