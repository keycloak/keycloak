...
You can visit the http://localhost:16686/[Jaeger UI] (`localhost:16686`) to see the tracing information.
...
When the tracing is enabled, the trace information is included in the log messages of all enabled log handlers (see more in the logging https://www.keycloak.org/server/logging[guide]).
It means the format of the log records is changed and may start like follows:
...
NOTE: When you explicitly override the log format for the particular log handlers, the `*-include-trace` options do not have any effect, and no tracing is printed out.

== Sampling

Sampler decides whether a trace should be discarded or forwarded, effectively managing noise and reducing overhead by limiting the number of collected traces sent to the collector.
...
The `parentbased_traceidratio` sampler could be the preferred default type, but it would require additional aspects to ensure safety.

There might be potential security risks related to information exposure or DoS attacks - proper HTTP filtering and adequate measures of caller trust would need to be assessed.
