...
This requires {project_name} to be configured with `--proxy-protocol-enabled=true` (see <<keycloak-configuration-traefik-passthrough>>).
...
These values affect how quickly Traefik detects that a {project_name} instance is shutting down (see <<keycloak-configuration-traefik-passthrough>>).
+
`port: 9000`::
Directs health checks to the management port (`9000`), which is separate from the main application port (`8443`).
The management port is configured to use plain HTTP via `--http-management-scheme=http` (see <<keycloak-configuration-traefik-passthrough>>), which is required because Traefik cannot perform an HTTPS health check on a TCP passthrough service.
...
[[keycloak-configuration-traefik-passthrough]]
...
[[graceful-shutdown-considerations-traefik-passthrough]]
...
<@kc.start parameters="--proxy-protocol-enabled true --health-enabled true --metrics-enabled true --http-management-scheme=http --shutdown-delay=16s"/>
