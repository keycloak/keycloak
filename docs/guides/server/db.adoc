...
{project_name} stores sensitive information in the database.
...
For databases that do not offer built-in Transparent Data Encryption, such as PostgreSQL, use filesystem-level encryption instead.
...
* Write-ahead logs (WAL) or redo logs are stored on encrypted storage.
