...
# It is recommended to use optimized images
...
The configuration can be set via environment variables, config file, CLI, etc. The CLI is not recommended for build time, or sensitive options.

. Using the config file
+
Add the minimum settings needed to connect to the database in `conf/keycloak.conf`:
+
----
# The database vendor.
db=postgres
...
Transactions that update only ephemeral tables like persisted user sessions, client sessions, login failures, or events will use PostgreSQL's async commit feature (`synchronous_commit = off`).
...
Transactions that update only ephemeral tables like persisted user sessions, client sessions, login failures, or events can use MS SQL Server's delayed durability feature.
...
Transactions that update only ephemeral tables like persisted user sessions, client sessions, login failures, or events will use Oracle's asynchronous redo log write (`COMMIT WRITE BATCH NOWAIT`).
Logouts will still force a synchronous commit.
...
If you wish to use the XA transaction support offered by your driver, set the build time `transaction-xa-enabled` option to `true`.
