...
(for example, `public` in PostgreSQL, `dbo` in Microsoft SQL Server).
You can override this by using the `db-schema` configuration option.

NOTE: In PostgreSQL, `keycloak` in the examples above refers to the *database name* (set via `db-url-database`), not the schema.
The database and schema are separate concepts in PostgreSQL.
To use a dedicated schema such as `keycloak`, you must first create it in your database and then configure
`db-schema=keycloak` along with granting the appropriate permissions to the database user.
