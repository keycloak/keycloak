...
{project_name} also uses this value to automatically derive the connection pool acquisition timeout (`db-pool-acquisition-timeout`) , which is set to twice the value of db-connect-timeout.
This ensures that the Agroal connection pool acquisition timeout is always greater than the JDBC connection timeout, preventing startup warnings.
