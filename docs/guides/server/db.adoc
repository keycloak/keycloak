...
By default, {project_name} sets the `=== Unicode support for a Microsoft SQL Server databaseStringParametersAsUnicode` JDBC driver property to `false` when using Microsoft SQL Server.
This significantly improves performance because otherwise Microsoft SQL Server might be unable to use indexes.
