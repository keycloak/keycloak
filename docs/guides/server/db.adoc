...
{project_name} allows you to specify multiple datasources in the case you need to access your own databases where your data lives.
It means that you can connect to these databases to handle your data and execute required actions.
It is very useful in situations where you have a database of your own users with a custom schema, and you cannot migrate to the {project_name} database due to multiple reasons.
...
The additional datasource properties might be specified via several config sources like CLI, `keycloak.conf`, or environment variables.

The most important database options that configure the default datasource contain the **Named key** parameter in the list of relevant options that reference a key for the additional named datasources at the end of this guide.

==== 1. JPA `persistence.xml` file

The `persistence.xml` configures information for Jakarta Persistence API (JPA) like what entities it should manage, the datasource name, JDBC settings, JPA/Hibernate custom settings, and more.
The file needs to lie in the `META-INF/persistence.xml` location of your custom {project_name} extension.
...
After specifying the DB kind for the datasource, the same properties as for the default datasource are automatically set based on the DB kind.
...
For the link:{server_developer_guide}#_user-storage-spi[User Storage SPI] datasource, we navigated users to use plain Quarkus properties to specify the datasource settings.
However, as using Quarkus properties in the `conf/quarkus.properties` file is **unsupported** and should be used only in special cases, it is strongly recommended to use the new datasource options.

However, for backwards compatibility, you can still specify the datasource settings via the Quarkus properties as follows:
