...
`start --optimized` will use database type and other build-time values associated with the current state of the optimized image. 
If you need to use a different database type, then running a command without `--optimized` or another `build` will allow you to use a different one.
