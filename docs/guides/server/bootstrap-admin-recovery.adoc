...
A temporary admin account can also be created with the Welcome Screen that is displayed at the first start of {project_name}. In this case, the temporary account behavior is controlled either by the `KC_BOOTSTRAP_ADMIN_IS_TEMPORARY` environment variable or by the `kcBootstrapAdminIsTemporary` setting in `theme.properties`. If the variable or setting is not defined, the account will be created as a temporary one.
...
Both commands also support using the `--is-temporary:env` parameter and enables using a different environment variable to control whether the created account is temporary or permanent.
