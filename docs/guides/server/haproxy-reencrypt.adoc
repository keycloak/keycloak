...
includedOptions="proxy-headers health-enabled metrics-enabled https-certificate-file https-certificate-key-file https-client-auth https-trust-store-file"
tileVisible="false">
...
HAProxy operates at Layer 7 (HTTP) and has direct visibility into the HTTP content.
End-to-end TLS encryption between client and {project_name} is not preserved.
...
[[haproxy-configuration-for-reencrypt]]
...
backend keycloak_back
    ...
    server keycloak1 keycloak1:8443 ssl verify required crt /path/to/haproxy-internal-certificate ca-file /path/to/keycloak-1-certificate check port 9000 check-ssl verify none inter 5s fall 3 rise 2 # <7>
...
The example shows removing of the `Forwarded` and `x-forwarded-` headers in order to prevent the client from providing misleading information to the backend service.
...
<7> Defines a backend {project_name} server.
...
[[keycloak-configuration-for-reencrypt]]
== {project_name} configuration

With TLS re-encrypt, {project_name} requires the following configuration:

<@kc.start parameters="--proxy-headers forwarded --health-enabled true --metrics-enabled true --https-certificate-file=/path/to/certificate --https-certificate-key-file=/path/to/key --https-client-auth=required --https-trust-store-file=/path/to/https-truststore"/>
...
See the https://www.keycloak.org/server/enabletls[Configuring TLS] guide for additional details.
...
See the https://www.keycloak.org/server/mutual-tls[Configuring trusted certificates for mTLS] guide for additional details.

[[graceful-shutdown-considerations-for-reencrypt]]
...
<@kc.start parameters="--proxy-headers forwarded --health-enabled true --metrics-enabled true --https-certificate-file=/path/to/certificate --https-certificate-key-file=/path/to/key --https-client-auth=required --https-trust-store-file=/path/to/https-truststore --shutdown-delay=16s"/>
