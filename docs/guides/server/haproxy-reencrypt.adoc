...
HAProxy operates at the Layer 7 (HTTP) and has direct visibility into the HTTP content.
End-to-end TLS encryption between client and and {project_name} is not preserved.
...
<6> This option adds a `Forwarded` HTTP header with the correct information about the client.
...
`ca-file /path/to/keycloak-N-certificate`:: Configures the certificate used to authenticate the particular Keycloak server to HAProxy.
