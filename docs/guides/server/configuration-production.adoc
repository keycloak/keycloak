...
For an overview of supported platforms, Java versions, databases, and other system requirements for {project_name}, see <@links.server id="supported-configurations"/>.
...
When {project_name} is configured to expose health endpoints, the server will open its HTTP(S) and Management endpoints while initialization progresses in the background.
