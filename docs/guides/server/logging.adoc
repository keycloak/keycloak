...
Logging is done on a per-category and per-handler basis in {project_name}.
You can configure logging for the root log level or more specific categories such as `org.hibernate` or `org.keycloak`.
...
It is possible to specify different log levels for each available log handler in the same way as for the parent `log-level` property.
The format of specifying these levels is the same as for the `log-level` property.

To override log levels for particular handlers, properties in format `log-<handler>-level` (where `<handler>` is available log handler) were introduced.
...
If these properties are not set, the root log level and category values are inherited from the parent `log-level` property.
...
=== Log level collisions

When the parent `log-level` property already contains root level or levels for particular categories, it may collide with the levels specified for each handler.

The *more concrete log handlers take precedence* over the parent one.

.Example
<@kc.start parameters="--log-level=info,org.keycloak.timer:debug --log-console-level=debug,org.keycloak.timer:trace"/>

As you can see, the `--log-console-level` property takes precedence over the parent `--log-level` property, so the default log level will be `debug` and logs from the `org.keycloak.timer` package will have log level `trace`.

=== Examples

.Example: `debug` logs and `trace` logs for category `org.keycloak.events` in Syslog
<@kc.start parameters="--log=console,syslog --log-syslog-level=debug,org.keycloak.events:trace"/>

.Example:All handlers contains only `warn` logs and File log handler `debug` logs:
<@kc.start parameters="--log=console,file,syslog --log-level=warn --log-file-level=debug"/>

.Example: Syslog handler contains only `org.keycloak.events` logs:
<@kc.start parameters="--log=console,syslog --log-syslog-level=off,org.keycloak.events:all"/>
...
<@kc.start parameters="--log=console,file --log-file-level=warn"/>
...
<@kc.start parameters="--log=console,syslog --log-syslog-level=warn"/>
