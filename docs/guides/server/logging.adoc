...
The `Authorization` header and selected sensitive cookies are automatically masked in the HTTP Access log. However, the list of masked items might not be complete. Be careful with using the `long` pattern or printing the headers by the custom format - you should use it only for development purposes. To extend the list of the masked items (e.g. to accomodate for headers and cookies used by your custom extensions), configure the `http-access-log-masked-cookies` and `http-access-log-masked-cookies` options.
