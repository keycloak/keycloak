...
== Running Import / Export in a Container

When running {project_name} via a container image, you can execute the `export` or `import` commands by mounting a local directory into the container.

=== Exporting a Realm using a Container

To export realms using the container image:

.Exporting realms to a directory with a container
[source,bash]
----
docker run --rm -it \
  -e KC_DB=postgres \
  ...
  -v /tmp/export-dir:/opt/keycloak/export \
  quay.io/keycloak/keycloak:latest \
  export --dir /opt/keycloak/export
----

=== Importing a Realm using a Container

To import realms using the container image:

.Importing realms from a directory with a container
[source,bash]
----
docker run --rm -it \
  -e KC_DB=postgres \
  ...
  -e KC_DB_USERNAME=keycloak \
  -e KC_DB_PASSWORD=password \
  -v /tmp/export-dir:/opt/keycloak/import \
  quay.io/keycloak/keycloak:latest \
