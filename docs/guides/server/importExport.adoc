...
[NOTE]
====
The import / export process is not suitable for use as a backup and restore mechanism.
Consistency of an export is not guaranteed unless all {project_name} nodes are stopped prior to running the export and exported data does not include admin events, offline sessions, or revoked tokens.
====
