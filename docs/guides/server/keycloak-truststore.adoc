...
includedOptions="truststore-paths truststore-kubernetes-enabled tls-hostname-verifier">
...
When `truststore-kubernetes-enabled` is set to `true`, {project_name} will also discover and include the OpenShift Service CA if it is present on the filesystem. To disable automatically trusting the cluster CAs entirely:
