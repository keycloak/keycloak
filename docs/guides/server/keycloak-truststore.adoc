...
includedOptions="truststore-paths truststore-kubernetes-enabled truststore-openshift-enabled tls-hostname-verifier">
...
This behavior is enabled by default and does not require the Operator. It can be controlled with the following options:

* `--truststore-kubernetes-enabled=true|false` (default: `true`)
* `--truststore-openshift-enabled=true|false` (default: `true`)

For example, to disable automatically trusting the cluster CAs:

<@kc.start parameters="--truststore-kubernetes-enabled=false --truststore-openshift-enabled=false" />
