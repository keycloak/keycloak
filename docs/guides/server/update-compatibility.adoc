...
It is possible to configure the {project_name} compatibility command to allow rolling updates when updating from a version to the following patch version, while staying on the same major and minor version.
