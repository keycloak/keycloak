...
It is possible to configure the {project_name} compatibility command to allow rolling updates when updating from a version to a different patch version from the same `major.minor` release stream.
...
* OpenID Connect clients can perform all operations like, for example, refreshing tokens and querying the user info endpoint.

Known limitations during or after rolling updates:
