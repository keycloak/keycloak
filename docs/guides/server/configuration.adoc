...
Env variables are converted back to normal option keys by lower-casing the name and replacing `\_` with `-`. Logging wildcards are the exception as `_` in the category is replaced with period `.` instead. Logging categories are commonly class / package names, which are more likely to use `.` rather than `-`.
