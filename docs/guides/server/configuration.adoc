...
=== Environment variables keys with special characters

Non-alphanumeric characters in your configuration key must be converted to _ in your environment variable key. For some configuration keys, such as those for logging, they may contain characters such as `_ ` or `$` - e.g. `kc.log-level-package.class_name`.
...
- create an entry in your `keycloak.conf` file that references an environment variable of your choosing. e.g. `kc.log-level-package.class_name=${r"${CLASS_NAME_LEVEL}"}`. See more on referencing environment variables in <<Formats for environment variables>>.
- or in situations where modifying the `keycloak.conf` may not be easy, you can use a pair of environment variables `KC_UNIQUEIFIER=value` and `KCKEY_UNIQUEIFIER=key`, e.g. `KC_MYKEY=debug` and `KCKEY_MYKEY=log-level-package.class_name`.
