...
However {project_name} cannot distinguish at startup what quarkus options are build vs runtime. If you are running unoptimized, a change in the raw value from the quarkus.properties will trigger a re-build.
If you are using `--optimized` or have set auto-build to `false`, changes to those values are ignored.
...
{project_name} distinguishes **build options**, that are usable when running the `build` command.
...
==== Second step: Set the `auto-build` option

Using either the config file or an environment variable set the `auto-build` option to `false`.
...
If a build option has a different value than the value used when a build was run, an error occurs. For this value to take effect you can run a new `build` to refine the optimized build.

Sample shows the creation of an optimized build.
