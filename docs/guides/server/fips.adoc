...
PKCS12 keystore can be generated with OpenJDK on RHEL 9 in the standard way. For instance, the following command can be used to generate the keystore:
...
Adding this security provider should work well. In fact, it is FIPS compliant and is already added by default in the OpenJDK 21 and later, or newer versions of OpenJDK 17.
...
* Security file `kc.java.security` with added provider for SAML (Not needed with OpenJDK 21 and later, or newer OpenJDK 17)
