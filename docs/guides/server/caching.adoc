...
It is possible to configure {project_name} to store regular user sessions in the cache only and minimize the database utilization.
...
Using volatile user sessions also disables an improved handling of offline sessions, where only a limited number of offline sessions needs to be loaded into memory.
Due to this, apply the settings below not only to the regular session caches but also the offline session caches.
...
The default number of two owners is the minimum number is necessary to survive one node (owner) failure or a rolling restart in a cluster setup with at least two nodes. When one of the owners becomes unavailable, the data is restored from the remaining owner and rebalanced across the remaining nodes.
A higher number could increase the availability of the data, but would lead to slower writes as more nodes need to be updated.
