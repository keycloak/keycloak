...
The default stack is set to `jdbc-ping-udp` when distributed caches are enabled, which is backwards compatible with the 26.x release stream of {project_name}.

Starting with the 27.x release stream, {project_name}'s new default will change to `jdbc-ping` as it is considered simpler to configure and as it works also environments that do not support the UDP protocol.
...
|`tcp` (deprecated)|TCP|IP multicast - `MPING` protocol. See below on how to configure a unique `jgroups.mcast_addr` for each cluster.
|`udp` (deprecated)|UDP|IP multicast - `PING` protocol. See below on how to configure a unique `jgroups.mcast_addr` for each cluster.
|===
...
Each cluster must use a different address and/or multicast port so that their nodes form distinct clusters.
