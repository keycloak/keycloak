...
Due to this, it is not recommended to use volatile user sessions when using offline sessions extensively. apply the settings below not only to the regular session caches but also the offline session caches.

When using volatile user sessions, the cache is the source of truth for user and client sessions. Therefore, it is necessary to not define an upper limit on the total number of entries that can be stored. Similarly, it's crucial that we replicate each entry to at least two nodes to prevent data loss on node failures and rolling restarts.
