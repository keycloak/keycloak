...
WARNING: While overriding the default cache configurations via XML is technically possible, it is not supported. The only supported way to change the default cache configurations is via the `+cache-...+` options.
