...
If you are running behind a firewall or with port forwarding, the IP address and port that other nodes of a {project_name} cluster should use to connect to an instance might be different from the IP address and port your {project_name} instance binds to.
In such a case {project_name} needs additional hints to advertise the IP address and port other nodes should use to connect to this instance.
For this scenario, use the following properties:
