...
To support advanced use cases, it is possible to disable infinispan caching entirely by disabling the respective connection providers.

<@kc.start parameters="--spi-connections-infinispan-default-enabled false --spi-connections-infinispan-quarkus-enabled false"/>

Since caching is required by other parts of the server, this also requires custom implementations of at least
