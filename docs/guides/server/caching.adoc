...
Because {project_name} uses the database for node discovery by default, clusters that share the same database but should remain separate will merge unless they have distinct cluster names.
