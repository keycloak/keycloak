...
[WARNING]
====
Enabling this feature via a rolling configuration upgrade can result in an unknown behavior of {project_name}.
To avoid this state, it is necessary to remove all online user and client sessions before the first node is started with this feature enabled.
This means all {project_name} nodes need to be stopped and, if used, Infinispan remote cache store and Infinispan JDBC persistence need to be cleared.
====
