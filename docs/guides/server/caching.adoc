...
To set different sizes for the caches, edit {project_name}'s cache config file (`conf/cache-ispn.xml`) to set a `+<memory max-count="..."/>+` for those caches.
...
1. Configure caches to not limit number of entries, as the cache will be the only source of truth for user and client sessions. To do so, edit {project_name}'s cache config file (`conf/cache-ispn.xml`) to remove the `+<memory max-count="..."/>+` for the `sessions` and `clientSessions` caches.
