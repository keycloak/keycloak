...
{project_name} automatically creates all required caches with the expected configurations. It is not recommended to modify
the default cache configurations in the majority of cases, however if required you can specify custom configurations for
the default caches in `conf/cache-ispn.xml` or your own file provided via `--cache-config-file`.

In order to prevent a warning being logged when a modified configuration is detected, enter this command:

<@kc.start parameters="--cache-config-mutate=true"/>

NOTE: No warning is issued when creating custom caches for extensions or custom providers that {project_name} does not
rely upon for its core functionality.
