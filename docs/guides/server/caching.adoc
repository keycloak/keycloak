...
[[cache-topology]]
== Topology Aware Data Availability

Configuring the topology information of your cluster increases the data availability in case of hardware failure.
The hardware where the {project_name} instance runs is necessary to compute this topology information.
This section is more relevant for bare-metal users, who have control over the hardware and where it is running.
...
It ensures if Pods are scheduled in the same node, the data is safely replicated into another node.
We recommend to set up anti-affinity rules and/or topology spread constraints to prevent multiple Pods from being scheduled on the same node, further reducing the risk of a single node failure incur a data loss.
====
