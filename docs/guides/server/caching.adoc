...
[WARNING]
====
When using volatile user sessions, you must not use a `max-count` setting for the offline session caches as this leads to a loss of data due to the current implementation even though the session data is saved in the database.
