...
Encryption using TLS is enabled by default for the transport stack.
No additional CLI options or modifications of the cache XML are required as long as you are using a TCP-based transport stack, which is also the default configuration.

[NOTE]
====
If you are using transport stack based on `UDP` or `TCP_NIO2`, proceed as follows to configure the encryption of the transport stack:
