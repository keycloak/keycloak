...
|`tcp`|TCP|IP multicast - `MPING` protocol.
|`udp`|UDP|IP multicast - `PING` protocol.
|`jdbc-ping-udp`|UDP|Database registry - `JDBC_PING2` protocol.
...
|`kubernetes`|TCP|Dns resolution - `DNS_PING` protocol. It requires `-Djgroups.dns.query=<headless-service-FQDN>` to be added to JAVA_OPTS or JAVA_OPTS_APPEND environment variable.
|===
...
The table below shows the ports that need to be open for your {project_name} server and describe what it is used for.

|===
|Port |Stack |Property | Description
...
m|46655
|`tcp` and `udp`
m|jgroups.mcast_port
|Multicast data transmission and initial discovery.
By default, {project_name} uses `239.6.7.8` multicast address and can be modified with property `jgroups.mcast_addr`.
...
NOTE: Use `-D<property>=<value` to modify the ports above in your `JAVA_OPTS_APPEND` or in your CLI command.

=== Removing `FD_SOCK2`

As well as `FD_SOCK2`, the default JGroups stack has another failure detection protocol, `FD_ALL`.
`FD_ALL` uses heartbeats and can be used standalone; however, it may take more time to detect a {project_name} server fail.

If it is desired to reduce the number of ports open in your Firewall, `FD_SOCK2` can be safely removed from the stack.
The example below, shows how to remove it from the `tcp` stack.

[source,xml]
----
<infinispan>
    <jgroups>
        <stack name="my-custom-stack" extends="tcp">
...
Then, start {project_name} with the following CLI option:

<@kc.start parameters="--cache-stack=my-custom-stack"/>

WARNING: It is not possible to remove `FD_SOCK2` from the `jdbc-ping` or `jdbc-ping-udp` stacks.
