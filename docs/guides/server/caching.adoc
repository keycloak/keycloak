...
The generated certificate and associated keys are securely stored within the database of each {project_name} instance.

To enable zero-configuration TLS encryption, set the `cache-embedded-mtls-enabled` option to true.
It's important to note that this disables any custom certificate configurations specified using other cache-embedded-mtls-* options.
Therefore, for zero-configuration encryption, you should not set any other cache-embedded-mtls-* options.
====
