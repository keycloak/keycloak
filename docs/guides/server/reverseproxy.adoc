...
If private backchannel access is required it is recommended to deploy an internal proxy in addition to the external proxy. In this case there are additional considerations:

* Restrict network access so that {project_name} accepts connections only from the two proxies.

* Both proxies need to use the same mode. Either both need to use TLS passthrough, or both need to use TLS re-encrypt.
