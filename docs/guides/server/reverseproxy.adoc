...
Here is an example nginx reverse proxy configuration to expose only needed paths. In this example, `/admin/` is exposed as a separated host (`https://admin.my.keycloak.org`) and accesses to `https://admin.my.keycloak.org` is restricted. We assume you run {project_name} at `https://internal.keycloak.org:8443`:
[source]
----
server {
  listen 443;
  listen [::]:443;
  server_name my.keycloak.org;
  ...
  # You need to add TLS configurations here

  location /realms/ {
    proxy_pass https://internal.keycloak.org:8443/realms/;
    # You need to add TLS configurations here and for each additional proxy_pass
  }
  location /resources/ {
    proxy_pass https://internal.keycloak.org:8443/resources/;
  }
  location /robots.txt {
    proxy_pass https://internal.keycloak.org:8443/robots;
  }
}
...
server {
  listen 443;
  listen [::]:443;
  server_name admin.my.keycloak.org;
  ...
  # You can restrict accessable clients
  # allow 192.168.0.0/24;
  # deny all;

  location /admin/ {
    proxy_pass https://internal.keycloak.org:8443/admin/;
    # You need to add TLS configurations here and for each additional proxy_pass
  }
  location /resources/ {
    proxy_pass https://internal.keycloak.org:8443/resources/;
...
In this example, we assume that {project_name} is started like the following:

<@kc.start parameters="--hostname https://my.keycloak.org --hostname-admin https://admin.keycloak.org --proxy-headers xforwarded ..."/>

See also: <@links.server id="hostname"/>
