...
| all (defaults to the most likely name for the provider)
...
| `nginx` (defaults to true), `traefik` (defaults to false)
...
Traefik sends the client certificate and any intermediate CA certificates as optionally URL-encoded PEM blocks in a single `X-Forwarded-Tls-Client-Cert` header, separated by commas.
The `traefik` provider automatically URL-decodes and parses all certificates from this header.

If you stick to the default header name used by Traefik, you do not need to configure any additional options after selecting the `traefik` provider.
The default values of the options are as follows:
