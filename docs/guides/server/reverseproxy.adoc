...
It is necessary as the regular `health` endpoints are only accessible in the internal cluster, but not an external loadbalancer.
This is a non-blocking endpoint, and it will still return "200 OK" if the system is overloaded and in load-shedding mode.
This ensures that the load balancer will not take a cluster out of rotation under high load as that would reduce the overall capacity of the system that it would make it difficult to recover from an overload scenario.
