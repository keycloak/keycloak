...
`8443` (`8080`) is used for frontend, backend and administration. See also: <@links.server id="hostname"/>

`9000` is used for management. See also: <@links.server id="management-interface"/>

You only need to proxy `8443` (`8080`) even when you use different host names for frontend/backend and administration as described at <@links.server id="configure-production"/>. You don't need to proxy `9000`.
