...
|10s
|1-10s
|Allow 1-2 poll cycles (typically 5s intervals) for load balancer to detect shutdown

|TLS passthrough configuration
|1s
|10-30s
|Longer timeout allows keepalive connections to drain naturally and receive connection close signals
...
|Deployment orchestration notifies proxy before pod termination
|0s
|1-10s
|No pre-shutdown delay needed if proxy is already reconfigured

|Combined: TLS passthrough + polled readiness
|10s
|30s
...
[source,bash]
----
bin/kc.sh start --shutdown-delay=5s --shutdown-timeout=30s
----
...
[source,bash]
----
bin/kc.sh start --shutdown-delay=10s --shutdown-timeout=10s
----
...
[source,bash]
----
bin/kc.sh start-dev --shutdown-delay=0s --shutdown-timeout=500ms
----
