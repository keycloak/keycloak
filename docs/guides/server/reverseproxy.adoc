...
| -
|No
|Although `/realms/` is exposed, the `master` realm is used for administrating {project_name} and is not needed for user-facing authentication.
Restrict `/realms/master/` to internal or administrative access toreduce the attack surface.
...
As a security best practice, do not expose `/realms/master/`  the public internet.
...
* Combine this with serving the AdminUI and Admin API on a dedicated hostname and port.
