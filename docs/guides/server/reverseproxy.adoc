...
* When the PROXY protocol is used for TLS passthrough, clients cannot connect directly to {project_name} without the PROXY protocol. Any direct connection would also allow spoofing of IP source addresses.

* When using TLS re-encrypt, it might be impossible to restrict proxy traffic to specific source IP addresses when the proxy's IP addresses are dynamic. Not restricting the proxy traffic would allow clients to spoof proxy headers.

In these setups, it might be better to stick with a single proxy and no backchannel access, or to deploy an internal proxy in addition to the external proxy.

When deploying a second proxy, there are additional considerations:

* Restrict network access so that {project_name} accepts connections only from the two proxies, for example, by adding network filtering.

* Both proxies need to use the same mode.
Either both need to use TLS passthrough, or both need to use TLS re-encrypt.

* If using TLS passthrough mode, both proxies can forward requests using the PROXY protocol.
As {project_name} presents a single TLS certificate for both paths, it needs to include a subject alternative name (SAN) for the second path so callers can validate TLS on both paths.
Note that this SAN will be visible on both paths.
While this is not a technical problem, it still leaks an internal hostname on the public path.

* If using TLS re-encrypt mode, both proxies would filter incoming HTTP headers, and add trusted HTTP headers.
...
Similarly, if your deployment uses fully dynamic URLs (`--hostname-strict=false`), rogue clients can forge the request's host headers. This exposes {project_name} to Host header injection, allowing attackers to manipulate dynamically generated URLs.
...
Depending on your security setup, you might want to allow some of the tracing headers through — either as-is or after sanitizing their values — for requests from trusted callers.
This assessment depends on your trust boundaries and security trade-offs and is out of scope for this guide.
...
|Proxy identity
|Clients can spoof their IP address, protocol, or host, affecting access control and audit logging. Forging the host allows Host header injection, enabling attackers to manipulate dynamically generated URLs.
...
|/realms/master/
|Only internally
|Assuming the administrative realm is called `master`, restrict `/realms/master/` to internal access. See <<isolating-master-realm, Isolating authentication for the administrative realm>> for more information.
...
|/lb-check
|Yes
|Endpoint to be used for an external load balancer to check the availability of a {project_name} cluster.
...
The `lb-check` endpoint is only available when the `stateless` or the `multi-site` feature is enabled.
It is intended for external load balancers when you do not want to expose the management interface and its `/health` endpoints publicly.
This is a non-blocking endpoint and should remain responsive under high load, so the load balancer does not remove a healthy cluster from rotation during transient overload.

[[isolating-master-realm]]
=== Isolating authentication for the administrative realm

As a security best practice, restrict access to the authentication protocol endpoints of the administrative realm from public IP addresses.
The following description assumes {project_name}'s default settings and that this realm is called `master`.

To do that, do not expose the path `/realms/master/` to the public internet.

Depending on your topology, you can combine this with serving the Admin UI and Admin API on a dedicated hostname and port. See <@links.server id="hostname" anchor="administration-console-on-a-separate-hostname" /> for details.
...
In previous versions of {project_name}, sticky sessions could improve performance by routing all requests for a browser session to the same backend node, avoiding remote cache lookups for session data.
{project_name} appended the owning node's route to the `AUTH_SESSION_ID` cookie (for example, `123.node2`) so that the load balancer could parse it for routing decisions.
...
The options `+spi-sticky-session-encoder--infinispan--should-attach-route+` and `+spi-sticky-session-encoder--remote--should-attach-route+` are deprecated since {project_name} 26.8 and will be removed in a future release.
To suppress the deprecation warning at startup, set those SPI options to `false`:
...
NOTE: The provider selection (`spi-x509cert-lookup--provider`) is a build time option and should be set via config file, environment variable. See <@links.server id="configuration"/> for more on handling build time options.
...
|ssl-cert-chain
| The name of the header holding the full certificate chain as a single value.
All chain certificates are contained in one header rather than split across indexed headers.
| `haproxy`, `rfc9440`
...
==== Configuring the HAProxy provider

HAProxy can forward client certificates as base64-encoded DER using its `ssl_c_der,base64` and `ssl_c_chain_der,base64` sample fetches.
The `haproxy` provider reads the client certificate from the `ssl-client-cert` header and the certificate chain from the `ssl-cert-chain` header.

The options and defaults specific to `haproxy` are as follows:
...
|ssl-cert-chain
| The name of the header holding the full certificate chain as a single base64-encoded DER value. HAProxy's `ssl_c_chain_der,base64` produces the concatenated DER encoding of all CA certificates in one value.
| (none)

|certificate-chain-length
| The maximum number of certificates to load from the chain header.
| 1
...
Corresponding {project_name} configuration:
