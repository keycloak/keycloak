...
includedOptions="https-trust-store-* https-client-auth https-management-client-auth https-management-trust-store-*">
...
NOTE: Management interface properties are inherited from the main HTTP server, including mTLS settings and the truststore used for client certificate validation.
It means when mTLS is set, it is also enabled for the management interface.
To override client authentication for the management interface, use the `https-management-client-auth` property.
To use a different truststore on the management interface, use the `https-management-trust-store-*` properties.
...
If your truststore does not have an extension matching its file type, you will also need to set the `https-trust-store-type` option.
