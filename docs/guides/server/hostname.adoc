...
You can set a specific host or base URL by using the `hostname-admin` and `hostname-admin-url` options to restrict access to the administration console.
If the options are not specified, the frontend URLs are used.
Otherwise, the URLs for the administration console are based on the incoming request.
