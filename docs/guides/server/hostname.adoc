...
WARNING: Using fully dynamic URLs (`--hostname-strict=false`) disables hostname verification. {project_name} will trust the request's host headers (such as `Host` or `X-Forwarded-Host`) to generate URLs. If untrusted clients can control these headers, attackers can forge them to execute Host header injection attacks, manipulating the URLs generated by the server. You must ensure your network environment guarantees these headers always contain trustworthy values, for example by blocking untrusted direct access and ensuring your reverse proxy strictly filters or overwrites them.
...
[#administration-console-on-a-separate-hostname]
