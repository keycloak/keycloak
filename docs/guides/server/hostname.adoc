...
When a proxy is in use for edge, reencrypt, or http passthrough, the `proxy-headers` should be set. Depending on the hostname settings, some or all of the URL, may be dynamically determined.
