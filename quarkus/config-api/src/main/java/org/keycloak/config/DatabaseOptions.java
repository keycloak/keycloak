...
public class DatabaseOptions {
    ...
    /**
     * Check whether the {@param option} have mapping from DB --> Datasource option listed in {@code DATASOURCES_OVERRIDES_SUFFIX} list
     */
    public static boolean isValidDatasourceOption(String option) {
        return OPTIONS_DATASOURCES.stream()
                .map(Option::getKey)
                .map(DatabaseOptions::getKeyForDatasource)
     ...
     */
    public static Optional<String> getResultNamedKey(Option<?> option, String namedProperty, boolean useKcNamespace) {
        return getKeyForDatasource(option)
                .map(key -> key.substring(0, key.indexOf("<")))
                .map(key -> key.concat(namedProperty))
                .map(key -> useKcNamespace ? "kc.".concat(key) : key);
    }

    public static Optional<String> getResultNamedKey(Option<?> option, String namedProperty) {
        return getResultNamedKey(option, namedProperty, true);
    }
}
