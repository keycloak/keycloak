...
public class OpenApiSwaggerDistTest {

  private static final String SWAGGER_UI_ENDPOINT = "/swagger-ui";
  private static final String Q_SWAGGER_UI_ENDPOINT = "/q/swagger-ui";
  private static final String OPENAPI_ENDPOINT = "/openapi";
  private static final String Q_OPENAPI_ENDPOINT = "/q/openapi";
  ...
  void testOpenApiEndpointNotEnabled(KeycloakDistribution distribution) {
    ...
    assertThrows(IOException.class, () -> when().get(Q_SWAGGER_UI_ENDPOINT), "Connection refused must be thrown");

    distribution.setRequestPort(8080);

    when().get(SWAGGER_UI_ENDPOINT).then()
        .statusCode(404);
    when().get(Q_SWAGGER_UI_ENDPOINT).then()
        .statusCode(404);
  }
  ...
  void testOpenApiEndpointEnabled(KeycloakDistribution distribution) {
    ...
    when().get(OPENAPI_ENDPOINT).then()
        .statusCode(200);
    when().get(Q_OPENAPI_ENDPOINT).then()
        .statusCode(200);
  }
//    @Test
