...
public class CustomJpaEntityProviderDistTest {
    ...
    void dbKindSpecifiedInBuildTime(KeycloakDistribution dist) {
        ...
        result.assertMessage("Multiple datasources are specified: <default>, client-store, user-store");
        result.assertBuild();

        result = dist.run("start", "--optimized", "--http-enabled=true", "--hostname-strict=false");
        result.assertWarning("You have set DB kind for 'client-store' datasource via Quarkus property. This approach is deprecated and you should use the Keycloak 'db-kind-client-store' property.");
    ...
    void notSpecifiedDbKind(CLIResult cliResult) {
        ...
        cliResult.assertWarning("You have set DB kind for 'client-store' datasource via Quarkus property. This approach is deprecated and you should use the Keycloak 'db-kind-client-store' property.");
    ...
    @Test
    @Launch({"start-dev", "--log-level=org.hibernate.jpa.internal.util.LogHelper:debug,org.keycloak.quarkus.deployment.KeycloakProcessor:debug", "--db-kind-user-store=dev-mem", "--db-kind-client-store=dev-mem"})
    void testUserManagedEntityNotAddedToDefaultPU(CLIResult cliResult) {
        cliResult.assertMessage("Multiple datasources are specified: <default>, client-store, user-store");
