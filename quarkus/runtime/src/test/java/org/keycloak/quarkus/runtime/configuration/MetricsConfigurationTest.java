...
public class MetricsConfigurationTest extends AbstractConfigurationTest{
    ...
    public void priorities() {
        ConfigArgsConfigSource.setCliArgs("--features=opentelemetry-metrics", "--metrics-enabled=true", "--metrics-export-enabled=true", "--metrics-export-endpoint=localhost:2000", "--metrics-export-protocol=http/protobuf");
        initConfig();
        assertConfig(Map.of(
        ...
        ConfigArgsConfigSource.setCliArgs("--features=opentelemetry-metrics", "--metrics-enabled=true", "--telemetry-endpoint=http://keycloak.org:1234", "--telemetry-protocol=grpc", "--metrics-export-enabled=true", "--metrics-export-endpoint=my-domain:2001", "--metrics-export-protocol=http/protobuf");
        initConfig();
        assertConfig(Map.of(
                "metrics-export-enabled", "true",
