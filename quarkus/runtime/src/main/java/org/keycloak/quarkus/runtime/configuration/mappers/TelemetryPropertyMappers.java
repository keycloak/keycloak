...
public class TelemetryPropertyMappers implements PropertyMapperGrouping{
    ...
    static String transformTelemetryHeaders(Option<String> singleOption, String value) {
        return transformTelemetryHeaders(singleOption, value, log);
    }

    static String transformTelemetryHeaders(Option<String> singleOption, String value, Logger logger) {
        if (StringUtil.isNotBlank(value)) {
            logger.debug("HTTP headers for the exporter with option '%s' are overridden by the parent property".formatted(singleOption));
            return value;
        }
    ...
    private static Map<Option<?>, Map<String, String>> getHeaders() {
        if (TELEMETRY_HEADERS_CACHE == null) {
            final List<Option<?>> OPTIONS = List.of(TELEMETRY_HEADER, TELEMETRY_LOGS_HEADER, TELEMETRY_METRICS_HEADER, TRACING_HEADER);
            final Map<Option<?>, String> PREFIXES = OPTIONS.stream().collect(Collectors.toMap(f -> f, f -> NS_KEYCLOAK_PREFIX + getWildcardPrefix(f.getKey())));
            TELEMETRY_HEADERS_CACHE = new HashMap<>();

            Configuration.getPropertyNames().forEach(key -> {
                if (key.startsWith(NS_KEYCLOAK_PREFIX)) {
                    PREFIXES.forEach((option, prefix) -> {
