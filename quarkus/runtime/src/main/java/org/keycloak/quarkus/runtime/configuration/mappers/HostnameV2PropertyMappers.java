...
public final class HostnameV2PropertyMappers {
    ...
    private static final String CONTEXT_WARNING = "the server running in an insecure context. Secure contexts are required for full functionality, including cross-origin cookies.";
    ...
    public static void validateConfig(Picocli picocli) {
        ...
        if (isProd) {
            String warning = CONTEXT_WARNING + " Also if you are using a proxy, requests from the proxy to the server will fail CORS checks with 403s because the wrong origin will be determined.";
            if (host == null) {
