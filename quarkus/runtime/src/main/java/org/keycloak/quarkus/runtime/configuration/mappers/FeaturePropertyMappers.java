...
public final class FeaturePropertyMappers implements PropertyMapperGrouping {
    ...
    public static void validateSingleFeature(String feature, String value) {
        ...
        Matcher matcher = VERSION_SUFFIX_PATTERN.matcher(value.toLowerCase());
        if (matcher.matches()) {
            int version = Integer.parseInt(matcher.group(1));
            validateFeatureVersions(feature, version);
        } else if (!value.equals("enabled") && !value.equals("disabled")) {
            throw new PropertyException("Wrong value for feature '%s': %s. You can specify either 'enabled', 'disabled', or specific version that will be enabled".formatted(feature, value));
        }
    }
