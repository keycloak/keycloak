...
public class MigrateTo2_2_0 implements Migration {
    ...
    private void addIdentityProviderAuthenticator(RealmModel realm) {
        ...
        execution.setParentFlow(DefaultAuthenticationFlows.BROWSER_FLOW);
        ...
        if (defaultProvider != null) {
            ...
            configModel.setAlias("default-provider-config");
            ...
            LOG.debugf("Added identity provider authenticator to %s, with defaultProvider %s", realm.getName(), defaultProvider);
        } else {
            LOG.debugf("Added identity provider authenticator to %s", realm.getName());
        }
