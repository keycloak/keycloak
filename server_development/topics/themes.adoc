...
{project_name} uses http://freemarker.org[Freemarker Templates] in order to generate HTML. You can override individual templates in your own theme by
...
The deploy the archive to {project_name} you can either manually create a module in `modules` or use the `jboss-cli` command. It's simplest to use
