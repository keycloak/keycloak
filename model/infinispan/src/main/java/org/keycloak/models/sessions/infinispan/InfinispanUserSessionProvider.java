...
public class InfinispanUserSessionProvider implements UserSessionProvider {
    ...
    @Override
    public UserSessionModel createUserSession(RootAuthenticationSessionModel parentSession, RealmModel realm, UserModel user, String loginUsername, String ipAddress, String authMethod, boolean rememberMe, String brokerSessionId, String brokerUserId) {
        String id = parentSession.getId();
        UserSessionModel session = createUserSession(id, realm, user, loginUsername, ipAddress, authMethod, rememberMe,
                brokerSessionId, brokerUserId);
        
        session.setDeviceInfo(this.session.getContext().getConnection().getDeviceInfo());
