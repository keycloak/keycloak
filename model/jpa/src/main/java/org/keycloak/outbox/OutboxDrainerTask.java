...
public class OutboxDrainerTask implements ScheduledTask {
    ...
    protected void processOne(KeycloakSession session, OutboxStore store, OutboxEntryEntity row) {
        ...
        try {
            outcome = handler.deliver(session, row);
            if (outcome == null) {
                outcome = OutboxDeliveryOutcome.RETRY;
                errorMessage = "delivery handler returned null outcome";
            }
        ...
        switch (outcome) {
            ...
            case RETRY -> handleRetry(store, row, errorMessage);
            case DEAD_LETTER -> {
                String reason = errorMessage != null ? errorMessage
