...
public class DefaultMigrationManager implements MigrationManager {
    ...
    private final boolean allowMigrateExistingDBToSnapshot;

    public DefaultMigrationManager(KeycloakSession session, boolean allowMigrateExistingDBToSnapshot) {
        this.session = session;
        this.allowMigrateExistingDBToSnapshot = allowMigrateExistingDBToSnapshot;
    }
    ...
    public void migrate() {
        ...
        if (SNAPSHOT_VERSION.equals(currentVersion) && databaseVersion != null && databaseVersion.lessThan(SNAPSHOT_VERSION) && !allowMigrateExistingDBToSnapshot) {
            throw new ModelException("Incorrect state of migration. You are trying to run nightly server version '" + currentVersion + "' against a database, which was previously migrated to version '" + databaseVersion +
                    ". This indicates that you are trying to run development server version against production database, which can result in a loss or corruption of data. If it is intended, please " +
                    "use the option allow-migrate-existing-db-to-snapshot of datastore provider when starting the server and explicitly set it to true.");
        }
