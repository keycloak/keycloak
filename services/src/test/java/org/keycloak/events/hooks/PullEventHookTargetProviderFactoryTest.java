...
public class PullEventHookTargetProviderFactoryTest {
    ...
    public void shouldReturnReadyTestResult() {
        ...
        EventHookDeliveryResult result = factory.test(null, (RealmModel) null, target);

        assertTrue(result.isSuccess());
        assertEquals("PULL_READY", result.getStatusCode());
        assertEquals("PULL: /realms/realm-1/event-hooks/target-1/consume", result.getDetails());
    }
    ...
    public void shouldReturnWaitingDeliveryResult() throws Exception {
        ...
        assertEquals("Waiting for consumption through /realms/realm-1/event-hooks/target-1/consume", result.getDetails());
    }
