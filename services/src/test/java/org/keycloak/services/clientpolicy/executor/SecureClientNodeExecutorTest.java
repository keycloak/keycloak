...
public class SecureClientNodeExecutorTest {
    ...
    @Test
    public void registerNode_mixedCaseHostname_normalisedToLowercase_rejectedByLowercasePattern() throws ClientPolicyException {
        configure(List.of(PATTERN_INTERNAL));
        assertRegisterPasses("App-Node-1.Internal.Example.Com");
    }
    ...
    @Test
    public void registerNode_bareIpv6Address_passes() throws ClientPolicyException {
        configure(List.of("^::1$"));
        assertRegisterPasses("::1");
    }

    // Bare and bracketed forms of same address both produce same canonical → both match same pattern
    @Test
    public void registerNode_bareAndBracketedIpv6_samePattern_bothPass() throws ClientPolicyException {
        configure(List.of("^2001:db8::1$"));
        assertRegisterPasses("2001:db8::1");
        assertRegisterPasses("[2001:db8::1]");
    }

    // IPv4-mapped bare IPv6
    @Test
    public void registerNode_bareIpv4MappedIpv6_matchesPattern_passes() throws ClientPolicyException {
        configure(List.of("^::ffff:192\\.0\\.2\\.1$"));
        assertRegisterPasses("::ffff:192.0.2.1");
    }

    // Bare IPv6 where last group is :8443 — accepted as IPv6, not as port
    @Test
    public void registerNode_bareIpv6WithColonGroup8443_notTreatedAsPort_accepted() throws ClientPolicyException {
        configure(List.of("^2001:db8::1:8443$"));
        assertRegisterPasses("2001:db8::1:8443");
    }

    // Invalid bare IPv6 — bad hex chars
    @Test
    public void registerNode_bareIpv6InvalidHex_rejected() {
        configure(List.of(".*"));
        assertRegisterThrows("2001:zzzz::1", OAuthErrorException.INVALID_REQUEST,
    ...
    public void executeOnEvent_nonRegisterNodeEvent_ignored() throws ClientPolicyException {
        configure(Collections.emptyList());
        executor.executeOnEvent(new ClientNodeRegistrationContext(null, List.of("evil.host.com"),
    ...
    @Test
    public void registerNodes_multipleValidIpv4Addresses_allPass() throws ClientPolicyException {
        configure(List.of("^192\\.168\\.1\\.\\d+$"));
        executor.executeOnEvent(new ClientNodeRegistrationContext(null,
                List.of("192.168.1.10", "192.168.1.11", "192.168.1.12"),
                ClientPolicyEvent.REGISTER_NODE));
        // No exception = all three passed.
    }

    @Test
    public void registerNodes_mixedValidAndInvalidIpv4_wholeBatchRejected() {
        configure(List.of("^192\\.168\\.1\\.\\d+$"));
        assertRegisterThrowsList(
                List.of("192.168.1.10", "evil.attacker.com", "192.168.1.11"),
    ...
    private void assertRegisterPasses(String nodeHost) throws ClientPolicyException {
        executor.executeOnEvent(new ClientNodeRegistrationContext(null, Collections.singletonList(nodeHost), ClientPolicyEvent.REGISTER_NODE));
    }
    ...
    private void assertRegisterThrowsList(List<String> nodeHosts, String expectedError, String expectedDetail) {
        try {
            executor.executeOnEvent(new ClientNodeRegistrationContext(null, nodeHosts, ClientPolicyEvent.REGISTER_NODE));
            Assertions.fail("Expected ClientPolicyException for hostnames: " + nodeHosts);
        } catch (ClientPolicyException cpe) {
            Assertions.assertEquals(expectedError, cpe.getError());
            Assertions.assertEquals(expectedDetail, cpe.getErrorDetail());
        }
