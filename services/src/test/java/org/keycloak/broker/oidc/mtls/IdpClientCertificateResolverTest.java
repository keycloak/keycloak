...
public class IdpClientCertificateResolverTest {
    ...
    @Test
    public void selectsUsableKeyWhenEarlierEnabledKeyIsUnusable() {
        // A provider can expose more than one enabled key. The first enabled key here has no private key
        // (unusable for mTLS); the resolver must skip it and select the later, fully usable key rather than
        // failing on the first entry.
        KeyWrapper unusable = new KeyWrapper();
        unusable.setProviderId("kp-1");
        unusable.setStatus(KeyStatus.from(true, true)); // enabled but no private key -> unusable
        unusable.setCertificate(dummyCert());

        KeyWrapper usable = new KeyWrapper();
        usable.setProviderId("kp-1");
        usable.setStatus(KeyStatus.from(true, true));
        usable.setPrivateKey(new java.security.PrivateKey() {
            public String getAlgorithm() { return "RSA"; }
            public String getFormat() { return "PKCS8"; }
            public byte[] getEncoded() { return new byte[0]; }
        });
        usable.setCertificate(dummyCert());

        RealmModel realm = realmStub();
        KeycloakSession session = sessionReturning(Stream.of(unusable, usable));

        KeyWrapper result = new IdpClientCertificateResolver(session).resolve(realm, "kp-1");
        assertSame(usable, result);
    }

    @Test
    public void skipsKeyWhosePrivateKeyIsNotAPrivateKey() {
        // getPrivateKey() is typed as java.security.Key; a non-PrivateKey value is unusable because
        // IdpMtlsSslContextProvider.buildKeyManagers requires an actual PrivateKey. The resolver must treat
        // such an entry as unusable and select a later, genuinely usable key instead of failing at request time.
        KeyWrapper nonPrivate = new KeyWrapper();
        nonPrivate.setProviderId("kp-1");
        nonPrivate.setStatus(KeyStatus.from(true, true));
        nonPrivate.setPrivateKey(new java.security.Key() { // a Key that is NOT a PrivateKey
            public String getAlgorithm() { return "RAW"; }
            public String getFormat() { return "RAW"; }
            public byte[] getEncoded() { return new byte[0]; }
        });
        nonPrivate.setCertificate(dummyCert());
        ...
        RealmModel realm = realmStub();
        KeycloakSession session = sessionReturning(Stream.of(nonPrivate, usable));
    ...
    @Test
    public void throwsDedicatedExceptionWhenNoUsableKey() {
        // The expected "no usable key" outcome must be signalled with the dedicated exception type so that
        // config validation can translate only this case into an invalid-configuration error and let
        // operational failures of the key-storage path propagate as server errors.
        RealmModel realm = realmStub();
        KeycloakSession session = sessionReturning(Stream.empty());
        IdpClientCertificateResolver resolver = new IdpClientCertificateResolver(session);
        assertThrows(ClientCertificateKeyNotFoundException.class, () -> resolver.resolve(realm, "missing"));
    }
