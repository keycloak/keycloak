...
public class OAuth2IdentityProviderConfigTest {
    ...
    @Test
    public void tlsClientAuthOperationalKeyProviderFailureIsNotReportedAsBadConfig() {
        OAuth2IdentityProviderConfig c = config();
        c.setAuthorizationUrl("https://idp/auth");
        c.setTokenUrl("https://idp/token");
        c.setClientAuthMethod(OIDCLoginProtocol.TLS_CLIENT_AUTH);
        ...
        // The referenced key provider exists and is of the right type, but loading it fails operationally
        // (e.g. HSM/keystore access error). DefaultKeyManager.getProviders would swallow this and the resolver
        // would then see an empty key stream and throw ClientCertificateKeyNotFoundException -> 400. Validation
        // must instead surface it as an operational failure (a non-IllegalArgumentException RuntimeException ->
        // 5xx) so it is not misreported to Admin REST clients as an invalid IdP configuration.
        RuntimeException operational = new IllegalStateException("HSM offline");
        KeycloakSession session = sessionWithKeys(realm, operational);

        RuntimeException thrown = org.junit.Assert.assertThrows(RuntimeException.class,
                () -> c.validate(session, realm));
        assertFalse("operational key-provider load failure must not be classified as invalid config (400)",
    ...
    /**
     * Stubs a {@link KeycloakSession} whose {@code keys().getKeysStream(realm)} returns the given keys, and
     * whose {@code getKeycloakSessionFactory().getProviderFactory(KeyProvider.class, ...)} yields a
     * {@link org.keycloak.keys.KeyProviderFactory} that loads (creates) successfully.
     */
    private static KeycloakSession sessionWithKeys(org.keycloak.models.RealmModel realm, KeyWrapper... keys) {
        return sessionWithKeys(realm, null, keys);
    }

    /**
     * Variant that lets the referenced key provider's factory {@code create(session, component)} fail with an
     * operational error, mirroring an infra/provider-load failure that {@code DefaultKeyManager.getProviders}
     * would otherwise swallow. When {@code factoryFailure} is null the factory loads a benign provider.
     */
    private static KeycloakSession sessionWithKeys(org.keycloak.models.RealmModel realm,
            RuntimeException factoryFailure, KeyWrapper... keys) {
        ...
        KeyProvider keyProvider = (KeyProvider) Proxy.newProxyInstance(
                OAuth2IdentityProviderConfigTest.class.getClassLoader(),
                new Class<?>[] { KeyProvider.class },
                (proxy, method, args) -> {
                    if ("getKeysStream".equals(method.getName())) {
                        return Stream.of(keys);
                    }
        ...
        org.keycloak.keys.KeyProviderFactory<?> keyProviderFactory =
                (org.keycloak.keys.KeyProviderFactory<?>) Proxy.newProxyInstance(
                        OAuth2IdentityProviderConfigTest.class.getClassLoader(),
                        new Class<?>[] { org.keycloak.keys.KeyProviderFactory.class },
                        (proxy, method, args) -> {
        ...
        org.keycloak.models.KeycloakSessionFactory sessionFactory =
                (org.keycloak.models.KeycloakSessionFactory) Proxy.newProxyInstance(
                        OAuth2IdentityProviderConfigTest.class.getClassLoader(),
                        new Class<?>[] { org.keycloak.models.KeycloakSessionFactory.class },
                        (proxy, method, args) -> {
        ...
        return (KeycloakSession) Proxy.newProxyInstance(
                ...
                (proxy, method, args) -> {
                    switch (method.getName()) {
                        case "keys":
                            return keyManager;
                        case "getKeycloakSessionFactory":
