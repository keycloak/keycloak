...
public class GoogleIdentityProviderConfigTest {
    ...
    /**
     * Google must still run the session-dependent mTLS validation even though it replaces the generic OIDC
     * checks. A Google IdP with tls_client_auth but no client-certificate key provider must be rejected at
     * create/update time rather than deterministically failing on the first backchannel request.
     */
    @Test(expected = IllegalArgumentException.class)
    public void tlsClientAuthWithoutKeyProviderIsRejected() {
        GoogleIdentityProviderConfig c = config();
        c.setClientAuthMethod(org.keycloak.protocol.oidc.OIDCLoginProtocol.TLS_CLIENT_AUTH);
        // deliberately no client-certificate key provider configured
