...
public class AbstractPermissionService {
    ...
    private String createPermissionTicket(List<PermissionRequest> request) {
        ...
        SignatureProvider signatureProvider = this.authorization.getKeycloakSession().getProvider(SignatureProvider.class, TokenSignatureUtil.getAccessTokenSignatureAlgorithm(this.authorization.getKeycloakSession()));
        SignatureContext signer = signatureProvider.signer();

        return new JWSBuilder().jsonContent(token)
                .sign(signer);
    }
}