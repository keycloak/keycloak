...
public class DefaultHttpClientFactory implements HttpClientFactory {
    ...
    CloseableHttpClient buildHttpClient(KeycloakSession session, KeyManager[] keyManagers) {
        ...
        // Dedicated per-IdP mTLS clients (keyManagers != null) are built on every backchannel request. The
        // trust-configuration warnings ("TruststoreProvider is disabled" / "TrustManager is disabled") describe
        // a server-wide state that is already logged once on the shared lazy-init path, so suppress them here to
        // avoid flooding the log with one warning per token/user-info/introspection request.
        boolean logTrustConfig = keyManagers == null;
        configureBuilder(builder, session, logTrustConfig);
        if (keyManagers != null) {
