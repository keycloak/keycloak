...
public class DefaultHttpClientFactory implements HttpClientFactory {
    ...
    /**
     * Latches the once-per-factory logging of the insecure trust-configuration warnings. Gating on the shared
     * client's lazy init would hide these warnings entirely for an mTLS-only deployment (where the shared
     * client may never be built), so the first client build of any kind (shared or dedicated mTLS) logs them.
     */
    private final java.util.concurrent.atomic.AtomicBoolean trustConfigLogged =
    ...
    public HttpClientProvider create(KeycloakSession session) {
        // Do NOT build the shared client here: shared-client init loads the server-wide client keystore, and
        // dedicated per-IdP mTLS clients (createHttpClient(keyManagers)) supply their own key material and must
        // remain buildable even when that global keystore is unusable. Shared-client init is deferred to the
        // first shared-client operation via getSharedClient(session).
        return new HttpClientProvider() {
            ...
            public CloseableHttpClient createHttpClient(KeyManager[] keyManagers) {
                return buildHttpClient(session, keyManagers);
            }
            ...
            public int postText(String uri, String text) throws IOException {
                CloseableHttpClient httpClient = getSharedClient(session);
    ...
    /**
     * Returns the process-wide shared HTTP client, building it lazily on first use. The shared client is a
     * single server-wide instance and captures the truststore of whichever session first triggers its build
     * (identical to the prior eager behavior — only the trigger point moved from create() to first use, so a
     * dedicated mTLS client can be built even when the global client keystore is unusable).
     */
    private CloseableHttpClient getSharedClient(KeycloakSession session) {
        lazyInit(session);
        return httpClient;
    }
     ...
     * @return a configured client; the caller owns and must close clients built with key managers
     */
    CloseableHttpClient buildHttpClient(KeycloakSession session, KeyManager[] keyManagers) {
        HttpClientBuilder builder = newHttpClientBuilder(session);
        // The insecure trust-configuration warnings ("TruststoreProvider is disabled" / "TrustManager is
        // disabled") describe a server-wide state. Log them once per factory on the FIRST client build of any
        // kind (shared or dedicated mTLS) so they stay visible for an mTLS-only deployment where the shared
        // client may never be built, while never flooding the log with one warning per backchannel request.
        boolean logTrustConfig = trustConfigLogged.compareAndSet(false, true);
        configureBuilder(builder, session, logTrustConfig, keyManagers != null);
        if (keyManagers != null) {
    ...
    private void configureBuilder(HttpClientBuilder builder, KeycloakSession session, boolean logTrustConfig,
            boolean skipGlobalClientKeystore) {
        ...
        // Skip loading the server-wide client keystore for dedicated mTLS clients (key managers supplied):
        // HttpClientBuilder.resolveSslContext takes the key-managers branch and never reads this keystore, so
        // loading it would only be wasted per-request I/O and could fail an otherwise-valid mTLS request when the
        // global key material is absent or unusable.
        if (clientKeystore != null && !skipGlobalClientKeystore) {
