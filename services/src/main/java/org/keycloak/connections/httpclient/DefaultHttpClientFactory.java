...
import org.keycloak.truststore.TruststoreReloadEvent;
...
public class DefaultHttpClientFactory implements HttpClientFactory {
    ...
    // grace period before closing a client replaced on truststore reload, longer than a request lifetime so in-flight requests finish
    ...
    private void scheduleClose(CloseableHttpClient client) {
        ...
        long delayMillis = Math.max(RELOADED_CLIENT_CLOSE_DELAY_MILLIS, config.getLong("socket-timeout-millis", 5000L));
    ...
    private void lazyInit(KeycloakSession session) {
        if (httpClient.delegate == null) {
            synchronized(this) {
                if (httpClient.delegate == null) {
                    httpClient.delegate = buildClient(session);
                }
            }
        }
    }

    private CloseableHttpClient buildClient(KeycloakSession session) {
    ...
    public void postInit(KeycloakSessionFactory factory) {
        ...
        factory.register(event -> {
            if (event instanceof TruststoreReloadEvent truststoreReloadEvent) {
                synchronized (DefaultHttpClientFactory.this) {
                    CloseableHttpClient old = httpClient.delegate;
                    httpClient.delegate = buildClient(truststoreReloadEvent.getKeycloakSession());
                    if (old != null) {
                        scheduleClose(old);
                    }
                }
    ...
    private static final class ReloadableCloseableHttpClient extends CloseableHttpClient {
