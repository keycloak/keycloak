...
public class DefaultEmailSenderProvider implements EmailSenderProvider {
    ...
    /**
     * @deprecated This way creating the provider will be removed.
     * The pre-initialized version of authenticators allows the reuse of valid tokens.
     * @param session - current session
     */
    @Deprecated(forRemoval = true)
    public DefaultEmailSenderProvider(KeycloakSession session) {
        this.authenticators = new ConcurrentHashMap<>();
        authenticators.put(EmailAuthenticator.AuthenticatorType.NONE, new DefaultEmailAuthenticator());
        authenticators.put(EmailAuthenticator.AuthenticatorType.BASIC, new PasswordAuthEmailAuthenticator());
        authenticators.put(EmailAuthenticator.AuthenticatorType.TOKEN, new TokenAuthEmailAuthenticator());
    ...
    public void send(Map<String, String> config, String address, String subject, String textBody, String htmlBody) throws EmailException {
        ...
        try(Transport transport = session.getTransport("smtp")) {
            ...
            var selectedAuthenticator = selectAuthenticatorBasedOnConfig(config);
    ...
    private EmailAuthenticator selectAuthenticatorBasedOnConfig(Map<String, String> config) {
        if(isAuthConfigured(config)) {
            var authType = config.getOrDefault("authType", "basic");
