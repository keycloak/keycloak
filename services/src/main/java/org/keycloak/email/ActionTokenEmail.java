 ...
 * {@code InviteUserStepProvider}) do not have to reimplement the individual steps:
 * <ul>
 ...
 *   <li>invoking {@link EmailTemplateProvider#sendExecuteActions(String, long)}.</li>
 * </ul>
 * It is HTTP-agnostic: {@link #resolveParams} returns a {@link Result} (either {@link Params} or
 * an {@link Ineligibility} reason) and {@link #send} throws {@link EmailException}, so callers can
...
public final class ActionTokenEmail {
    ...
    public static void send(KeycloakSession session, RealmModel realm, UserModel user,
        ...
        session.getProvider(EmailTemplateProvider.class)
                ...
                .setRealm(realm)
                .setUser(user)
                .sendExecuteActions(link, Math.max(1, TimeUnit.SECONDS.toMinutes(params.lifespanSeconds)));
    }
}
