...
public class OID4VCLoginProtocolFactory implements LoginProtocolFactory, OID4VCEnvironmentProviderFactory {
    ...
    public void createDefaultClientScopes(RealmModel newRealm, boolean addScopesToExistingClients) {
        ...
        for (String format : Format.SUPPORTED_FORMATS) {
            String scopeName = CREDENTIAL_TYPE_NATURAL_PERSON + Format.getScopeSuffix(format);
    ...
    public void addClientScopeDefaults(ClientScopeRepresentation clientScope) {
        ...
        clientScope.getAttributes().computeIfAbsent(FORMAT, k -> getFormatFromScope(scopeName));
        String format = clientScope.getAttributes().get(FORMAT);

        int idx = scopeName.lastIndexOf(Format.getScopeSuffix(format));
    ...
    private String getFormatFromScope(String scope) {
        String format = SD_JWT_VC; // default format
        if (scope.toLowerCase().endsWith("_jwt")) format = Format.JWT_VC;
        if (scope.toLowerCase().endsWith("_ld")) format = Format.LDP_VC;
        return format;
    }
}
