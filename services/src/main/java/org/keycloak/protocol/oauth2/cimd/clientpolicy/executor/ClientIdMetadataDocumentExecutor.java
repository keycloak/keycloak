...
public class ClientIdMetadataDocumentExecutor extends AbstractClientIdMetadataDocumentExecutor<ClientIdMetadataDocumentExecutor.Configuration> {
    ...
    protected void validateClientMetadata(final URI clientIdURI, final URI redirectUriURI, final OIDCClientRepresentation clientOIDC) throws ClientPolicyException {
        ...
        if (configuration.isAcceptPublicClientWithConfidentialClientOnlyGrant()) {
            if (clientOIDC.getTokenEndpointAuthMethod() == null || "none".equals(clientOIDC.getTokenEndpointAuthMethod())) {
                List<String> grantTypes = new ArrayList<>(Optional.ofNullable(clientOIDC.getGrantTypes()).orElse(List.of()));
                Set<String> confidentialOnlyGrantTypes = getConfidentialOnlyGrantTypes();
                if (grantTypes.removeAll(confidentialOnlyGrantTypes)) {
    ...
    private Set<String> getConfidentialOnlyGrantTypes() {
        ...
        session.getKeycloakSessionFactory().getProviderFactoriesStream(OAuth2GrantType.class)
                ...
                .filter(factory -> {
                    OAuth2GrantType grantType = session.getProvider(OAuth2GrantType.class, factory.getId());
                    return grantType != null && grantType.isConfidentialOnlyGrantType();
                })
                .map(OAuth2GrantTypeFactory::getId)
                .forEach(confidentialOnlyGrantTypes::add);
        return confidentialOnlyGrantTypes;
    }
}
