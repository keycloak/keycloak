...
/**
 * Maps a note stored on the {@link org.keycloak.models.AuthenticatedClientSessionModel} to a token claim.
 * <p>
 * The note to read is selected via the {@link ProtocolMapperUtils#CLIENT_SESSION_NOTE} config property, and the
 * resulting claim can be emitted into the access token, ID token, access token response, userinfo and token
 * introspection responses. If the configured note is absent on the client session, no claim is added.
 */
