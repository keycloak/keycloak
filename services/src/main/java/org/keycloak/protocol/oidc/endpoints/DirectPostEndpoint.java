...
public class DirectPostEndpoint {
    ...
    static final String AUTHORIZATION_REQUEST_KEY = AuthorizationEndpointRequest.class.getSimpleName();

    // [TODO #44657] Remove DirectPostEndpoint.authSessionHolder hack
    ...
    public Response process() {
        ...
        IDTokenResponse idTokenRes;
        try {
            idTokenRes = new IDTokenResponse(signedJwt);
        } catch (JWSInputException e) {
            throw new AuthenticationFlowException("Failed to parse IDToken from: " + signedJwt, e, INVALID_CREDENTIALS);
        }

        logger.infof("Received IDToken: %s", idTokenRes.tokenJwt);
        ...
        if (!idTokenRes.subject.equals(authRequest.getClientId()))
        ...
        String userDid = idTokenRes.subject;
        UserModel userModel = session.users()
                .searchForUserByUserAttributeStream(realm, USER_ATTRIBUTE_NAME_DID, userDid)
                .findFirst()
                .orElse(null);
        if (userModel == null)
