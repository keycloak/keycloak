...
public class IDJWTTokenExchangeProvider extends StandardTokenExchangeProvider {
    ...
    protected Response tokenExchange() {
        ...
        // Enforce the strict Not-Before revocation policy. 
        // This ensures that ID tokens issued prior to an administrative revocation event 
        // (at the realm, client, or user level) are strictly rejected, even if they have not yet expired.
    ...
    protected String getRequestedScope(JsonWebToken token, List<ClientModel> targetAudienceClients) {
        ...
        if (attrValue != null && !attrValue.isBlank()) {
            ...
            throw new CorsErrorResponseException(
                cors,
                OAuthErrorException.INVALID_SCOPE, // 400 invalid_scope
                errorMessage,
    ...
    protected Response exchangeClientToOIDCClient(UserModel targetUser, UserSessionModel targetUserSession, String requestedTokenType,
        ...
        try {
            ...
            responseBuilder.getAccessToken().audience(params.getAudience().get(0));
    ...
    protected List<ClientModel> getTargetAudienceClients() {
        ...
        if (audienceParams != null) {
            if (audienceParams.size() == 0) {
                ...
                List<ClientModel> matchingClients = session.clients().getClientsStream(realm)
                ...
                if (matchingClients.isEmpty()) {
                    ...
                    event.error(Errors.NOT_ALLOWED);
                    throw new CorsErrorResponseException(cors, OAuthErrorException.INVALID_REQUEST, "Client not found for audience identifier: " + audienceParameterString, Response.Status.BAD_REQUEST);
                }
                
                if (matchingClients.size() > 1) {
                    event.detail(Details.REASON, "Ambiguous clients found for audience identifier: " + audienceParameterString);
                    event.error(Errors.INVALID_REQUEST);
                    throw new CorsErrorResponseException(cors, OAuthErrorException.INVALID_REQUEST,
                        "Ambiguous target client. Multiple clients match the requested resource identifier: " + audienceParameterString,
                        Response.Status.BAD_REQUEST);
                }
                
                if (matchingClients.get(0).getRegistrationToken() != null) {
                ...
                targetAudienceClients.add(matchingClients.get(0));
