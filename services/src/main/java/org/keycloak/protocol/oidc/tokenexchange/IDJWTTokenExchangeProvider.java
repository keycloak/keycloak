...
public class IDJWTTokenExchangeProvider extends StandardTokenExchangeProvider {
    ...
    protected Response tokenExchange() {
        ...
        if (context.getParams().getActorToken() != null || context.getParams().getActorTokenType() != null) {
        ...
        keycloakContext.setBearerToken(token);
        keycloakContext.setUserSession(userSession);
    ...
    protected Response buildTokenExchangeResponse(ClientSessionContext clientSessionCtx, String requestedTokenType, List<ClientModel> targetAudienceClients) {
        ...
        AccessToken tokenForTransform = new AccessToken();
        tokenForTransform.setScope(clientSessionCtx.getScopeString());
        AccessToken transformedToken = tokenManager.transformAccessToken(session, tokenForTransform, userSession, clientSessionCtx);
        ...
        idjag.subject(transformedToken.getSubject());
        idjag.issuedNow();
        idjag.nbf(Time.currentTimeSeconds());
        ...
        idjag.audience(transformedToken.getAudience());
        ...
        if (clientLifespanStr != null && !clientLifespanStr.trim().isEmpty()) {
            try {
                ...
                if (clientLifespan > 0 || clientLifespan == -1) {
        ...
        if (lifespan == -1) {
            ...
            expiration = Time.currentTimeMillis() + TimeUnit.SECONDS.toMillis(lifespan);
        }
        long sessionExpires = SessionExpirationUtils.calculateClientSessionMaxLifespanTimestamp(
            ...
            client);
        if (sessionExpires > 0) {
            expiration = Math.min(expiration, sessionExpires);
        }
        idjag.exp(TimeUnit.MILLISECONDS.toSeconds(expiration));

        // client_id,auth_time should be set in idjag assertion here.
        if (transformedToken.getOtherClaims() != null) {
            idjag.getOtherClaims().putAll(transformedToken.getOtherClaims());
        }

        Object targetClientId = idjag.getOtherClaims().get(OAuth2Constants.CLIENT_ID);
        ...
        String grantedScope = transformedToken.getScope();
        if (grantedScope != null) {
            idjag.setOtherClaims("scope", grantedScope);
        }    
        
        checkRequestedAudiences(idjag);

        // sign the ID-JAG using the realm/server-configured signature algorithm for ID tokens
        ...
        res.setScope(grantedScope);

        if (params.getAudience() != null) {
            event.detail(Details.AUDIENCE, String.join(" ", params.getAudience()));
        }
        event.success();
