...
public class OID4VPIdentityProvider extends AbstractIdentityProvider<OID4VPIdentityProviderConfig>
    ...
    // The verifier signs request objects with the realm's active ES256 key, whose certificate also
    // yields the client identifier. Configure it through a realm key provider, as OID4VCI does.
    // TODO revisit using the realm-wide signing key, since the verifier certificate must be
    // pre-registered in the wallet ecosystem and may need a lifecycle independent of realm keys.
    public KeyWrapper signingKey() {
        KeyWrapper key = session.keys().getActiveKey(session.getContext().getRealm(), KeyUse.SIG, Algorithm.ES256);
        if (key == null) {
            throw new IdentityBrokerException("No active ES256 realm signing key found for the OID4VP verifier");
        }
        if (key.getCertificate() == null) {
            throw new IdentityBrokerException("The active ES256 realm signing key has no certificate");
        }
        return key;
    }
