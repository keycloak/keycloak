...
public class OID4VPIdentityProviderEndpoint {
    ...
    public Response requestObject(@PathParam("state") String state) {
        ...
        if (flow == null) {
            return loginError(Response.Status.NOT_FOUND, OAuthErrorException.INVALID_REQUEST,
        ...
        return Response.ok(requestObject).type(OID4VCConstants.REQUEST_OBJECT_MEDIA_TYPE).build();
    }
