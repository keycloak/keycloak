...
public abstract class AbstractOAuth2IdentityProvider<C extends OAuth2IdentityProviderConfig> extends AbstractIdentityProvider<C> implements ExchangeTokenToIdentityProviderToken, ExchangeExternalToken {
    ...
    protected SimpleHttp createBackchannelHttp() {
        // Default backchannel calls target the RFC 8705 endpoints (token, user-info, introspection), which are
        // the only endpoints in scope for the client certificate. Callers that hit an endpoint outside that
        // scope (e.g. logout, which is not an advertised mtls_endpoint_alias) must use createBackchannelHttp(false).
        return createBackchannelHttp(getConfig().isTlsClientAuth());
    }

    /**
     * Backchannel {@link SimpleHttp} whose transport is chosen explicitly. Pass {@code true} for the RFC 8705
     * endpoints (token / user-info / introspection) so a {@code tls_client_auth} IdP presents the realm's client
     * certificate; pass {@code false} to force the shared non-mTLS client for calls that are outside the
     * certificate-bound scope, so the client certificate is neither resolved nor presented there.
     *
     * @param tlsClientAuth whether this specific call is an mTLS-scoped endpoint
     */
    protected SimpleHttp createBackchannelHttp(boolean tlsClientAuth) {
        if (!tlsClientAuth) {
