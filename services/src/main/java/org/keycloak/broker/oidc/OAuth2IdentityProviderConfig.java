...
import org.keycloak.broker.oidc.mtls.ClientCertificateKeyNotFoundException;
...
public class OAuth2IdentityProviderConfig extends IdentityProviderModel {
    ...
    public void validate(RealmModel realm) {
        // Realm-only validations that do not need a KeycloakSession live here so that direct callers of the
        // legacy validate(RealmModel) hook (the base contract, and subclasses/extensions that invoke
        // super.validate(realm)) keep getting the URL, PKCE and mTLS-alias checks. The session overload below
        // delegates to this method before running the session-dependent key-resolution validation.
    ...
    /**
     * Realm-only tls_client_auth validation: the preferred mTLS endpoint aliases must honor the realm SSL
     * requirement and a client-certificate key provider of the correct type must exist. Extracted so provider
     * configs that intentionally replace the generic URL checks (e.g. Google) can still run the mTLS checks
     * without inheriting those generic checks. Callers guard on {@link #isTlsClientAuth()}.
     */
    protected void validateTlsClientAuth(RealmModel realm) {
        SslRequired sslRequired = realm.getSslRequired();
        ...
        // receives the client certificate and backchannel credentials. Blank values mean "no alias" (a
        // discovery import without mtls_endpoint_aliases stores empty strings to clear stale aliases), so
        // they are skipped here exactly like selectEndpointForClientAuth treats them.
        checkUrl(sslRequired, emptyToNull(getMtlsTokenUrl()), "mtls_token_url");
        checkUrl(sslRequired, emptyToNull(getMtlsUserInfoUrl()), "mtls_userinfo_url");
        checkUrl(sslRequired, emptyToNull(getMtlsTokenIntrospectionUrl()), "mtls_tokenIntrospection_url");
    ...
    public void validate(KeycloakSession session, RealmModel realm) {
        // Run the realm-only checks first. Delegating through the base overload (which calls validate(realm))
        // preserves the base contract and keeps provider-specific checks registered by subclasses that only
        // override the (RealmModel) hook running (e.g. the generic OAuth provider's User Info/claim checks in
        // OAuth2IdentityProviderFactory).
    ...
    /**
     * Session-dependent tls_client_auth validation: resolve the actual client-certificate key material now
     * that a session is available, so create/update enforces the same requirements as the runtime backchannel
     * calls (an enabled key with a private key and a certificate). Extracted so provider configs that replace
     * the generic {@code validate(session, realm)} chain (e.g. Google) can still run it. Callers guard on
     * {@link #isTlsClientAuth()}.
     */
    protected void validateTlsClientAuthKeyResolution(KeycloakSession session, RealmModel realm) {
        // The referenced key provider's existence/type was already checked in validateTlsClientAuth(realm);
        // otherwise creation would succeed but IdpClientCertificateResolver would then deterministically fail
        // on the first mTLS request.
        String keyProviderId = getClientCertKeyProviderId();

        // Probe the referenced key provider by loading it directly, mirroring DefaultKeyManager.getProviders but
        // WITHOUT swallowing load failures. DefaultKeyManager drops a provider whose factory.create(...) throws
        // (logs and returns null), so an operational load failure would otherwise reach the resolver as an empty
        // key stream and be misreported as an invalid configuration (400). Letting it propagate here keeps it a
        // non-IllegalArgumentException RuntimeException, so Admin REST maps it to a server error instead.
        ComponentModel component = realm.getComponent(keyProviderId);
        ProviderFactory<KeyProvider> f = session.getKeycloakSessionFactory()
                .getProviderFactory(KeyProvider.class, component.getProviderId());
        KeyProviderFactory<?> factory = (KeyProviderFactory<?>) f;
        KeyProvider provider = factory.create(session, component);
        session.enlistForClose(provider);

        try {
            new IdpClientCertificateResolver(session).resolve(realm, keyProviderId);
        } catch (ClientCertificateKeyNotFoundException e) {
            // Only the expected "no usable key" outcome is an invalid configuration. Other RuntimeExceptions
            // (transient/internal failures of the key-storage path) must propagate as server errors rather
            // than being reported to Admin REST clients as a bad IdP configuration.
            throw new IllegalArgumentException(
