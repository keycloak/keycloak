...
public class OAuth2IdentityProviderConfig extends IdentityProviderModel {
    ...
    public void validate(KeycloakSession session, RealmModel realm) {
        ...
        if (isTlsClientAuth()) {
            // The referenced key provider's existence/type was already checked in validate(realm); here we
            // additionally resolve the actual key material now that a session is available, so that
            // create/update enforces the same requirements as the runtime backchannel calls: the provider
            // must expose an enabled key with a private key and a certificate. Otherwise creation would
            // succeed but IdpClientCertificateResolver would then deterministically fail on the first mTLS
            // request.
            String keyProviderId = getClientCertKeyProviderId();
            try {
