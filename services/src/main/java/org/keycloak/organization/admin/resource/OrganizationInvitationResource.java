...
public class OrganizationInvitationResource {
    ...
    private Response sendInvitation(UserModel user, InvitationTarget invitationTarget, List<String> roles) {
        ...
        // only implementations storing roles are asked to, so a plain invite works with any of them
        if (!roleIds.isEmpty()) {
    ...
    public Response resendInvitation(@PathParam("id") String id) {
        ...
        // resending grants the roles again, so the permissions are checked against the administrator resending the
        // invitation rather than the one who sent it, whose rights may have been revoked in the meantime
        List<String> roles = List.copyOf(invitation.getRoleIds());
        invitationManager.remove(id);
    ...
    private OrganizationInvitationRepresentation toRepresentation(OrganizationInvitationModel model) {
        ...
        rep.setRoles(toRoleRepresentations(model));

        return rep;
    }

    /**
     * Returns the roles granted by an invitation, or {@code null} if it grants none. Roles removed since the invitation
     * was sent are skipped.
     */
    private List<RoleRepresentation> toRoleRepresentations(OrganizationInvitationModel model) {
        Set<String> roleIds = model.getRoleIds();
