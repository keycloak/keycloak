...
public class OrganizationAuthenticator extends IdentityProviderAuthenticator {
    ...
    private boolean redirect(AuthenticationFlowContext context, OrganizationModel organization, String username, String domain) {
        ...
        idp = organization.getIdentityProviders().filter(broker -> IdentityProviderRedirectMode.EMAIL_MATCH.isSet(broker) &&
                ANY_DOMAIN.equalsIgnoreCase(broker.getConfig().get(OrganizationModel.ORGANIZATION_DOMAIN_ATTRIBUTE)) &&
                organization.getDomains().map(OrganizationDomainModel::getName).anyMatch(domain::equals)).findFirst().orElse(null);
