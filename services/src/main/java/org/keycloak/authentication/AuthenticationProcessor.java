    ...
    public Response handleClientAuthException(Exception failure) {
        if (failure instanceof AuthenticationFlowException) {
            ...
            if (e.getError() == AuthenticationFlowError.CLIENT_NOT_FOUND) {
                ...
                return ClientAuthUtil.errorResponse(Response.Status.BAD_REQUEST.getStatusCode(), "unauthorized_client", "Invalid client or Invalid client credentials");
                ...
                event.error(Errors.INVALID_CLIENT_CREDENTIALS);
                return ClientAuthUtil.errorResponse(Response.Status.BAD_REQUEST.getStatusCode(), "invalid_client", "Invalid client or Invalid client credentials");
            }
        } else {
