...
public class UpdateEmail implements RequiredActionProvider, RequiredActionFactory {
    ...
    public void processAction(RequiredActionContext context) {
        ...
        UserProfileValidationResult emailUpdateValidationResult = validateEmailUpdate(context.getSession(), user, newEmail);
    ...
    private void updateEmailWithoutConfirmation(RequiredActionContext context,
            UserProfileValidationResult emailUpdateValidationResult) {

        updateEmailNow(context.getEvent(), context.getRealm(), context.getUser(), emailUpdateValidationResult);
        context.success();
    }

    public static UserProfileValidationResult validateEmailUpdate(KeycloakSession session, UserModel user, String newEmail) {
        MultivaluedMap<String, String> formData = new MultivaluedHashMap<>();
        formData.putSingle(UserModel.EMAIL, newEmail);
        return UserProfileContextFactory.forEmailUpdate(user, formData, session).validate();
    }

    public static void updateEmailNow(EventBuilder event, RealmModel realm, UserModel user,
            UserProfileValidationResult emailUpdateValidationResult) {

        UserProfile updatedProfile = emailUpdateValidationResult.getProfile();
        ...
        UserUpdateHelper.updateUserEmail(realm, user, updatedProfile);
    }
