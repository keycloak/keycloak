...
public class LocaleHelper {
    ...
    public static Locale resolveLocale(KeycloakSession session, RealmModel realm, UserModel user) {
    ...
    private static Locale getUserLocale(KeycloakSession session, RealmModel realm, UserModel user) {
        ...
        if (uriInfo != null && uriInfo.getQueryParameters().containsKey(KC_LOCALE_PARAM)) {
            ...
            if (locale != null) {
                updateLocaleCookie(session, realm, locale);
                if (user != null) {
                    updateUsersLocale(user, locale);
                }
                return locale;
            }
        }
    ...
    private static void updateLocaleCookie(KeycloakSession session,
        ...
        addCookie(LOCALE_COOKIE, locale.toLanguageTag(), AuthenticationManager.getRealmCookiePath(realm, session.getContext().getUri()), null, null, -1, secure, true);
    }
    ...
    private static void updateUsersLocale(UserModel user, Locale locale) {
        if (!locale.toLanguageTag().equals(user.getFirstAttribute("locale"))) {
