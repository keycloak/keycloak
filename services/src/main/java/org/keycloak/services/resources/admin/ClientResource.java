...
public class ClientResource {
  ...
  public ClientResource(RealmModel realm, AdminPermissionEvaluator auth, ClientModel clientModel,
  ...
  public static ManagementPermissionReference toMgmtRef(ClientModel client,
  ...
  public ProtocolMappersResource getProtocolMappers() {
    AdminPermissionEvaluator.RequirePermissionCheck manageCheck = () -> auth.clients()
        .requireManage(client);
    AdminPermissionEvaluator.RequirePermissionCheck viewCheck = () -> auth.clients()
        .requireView(client);
    ProtocolMappersResource mappers = new ProtocolMappersResource(realm, client, auth, adminEvent,
        manageCheck, viewCheck);
  ...
  public Response update(final ClientRepresentation rep) {
    ...
    try {
      session.clientPolicy()
          .triggerOnEvent(new AdminClientUpdateContext(rep, client, auth.adminAuth()));

      updateClientFromRep(rep, client, session);

      ValidationUtil.validateClient(session, client, false, r -> {
        session.getTransactionManager().setRollbackOnly();
        throw new ErrorResponseException(Errors.INVALID_INPUT, r.getAllLocalizedErrorsAsString(
            AdminRoot.getMessages(session, realm, auth.adminAuth().getToken().getLocale())),
            Response.Status.BAD_REQUEST);
      });

      session.clientPolicy()
          .triggerOnEvent(new AdminClientUpdatedContext(rep, client, auth.adminAuth()));
  ...
  public ClientAttributeCertificateResource getCertficateResource(
      @PathParam("attr") String attributePrefix) {
  ...
  public Response getInstallationProvider(@PathParam("providerId") String providerId) {
    ...
    ClientInstallationProvider provider = session.getProvider(ClientInstallationProvider.class,
        providerId);
    if (provider == null) {
      throw new NotFoundException("Unknown Provider");
    }
    return provider.generateInstallation(session, realm, client,
  ...
  public void deleteClient() {
    ...
    try {
      session.clientPolicy()
          .triggerOnEvent(new AdminClientUnregisterContext(client, auth.adminAuth()));
    } catch (ClientPolicyException cpe) {
      throw new ErrorResponseException(cpe.getError(), cpe.getErrorDetail(),
          Response.Status.BAD_REQUEST);
    }
    ...
    if (new ClientManager(new RealmManager(session)).removeClient(realm, client)) {
      ...
      throw new ErrorResponseException(OAuthErrorException.INVALID_REQUEST,
          "Could not delete client", Response.Status.BAD_REQUEST);
    }
  }
  ...
  public ClientRepresentation regenerateRegistrationAccessToken() {
    ...
    String token = ClientRegistrationTokenUtils.updateRegistrationAccessToken(session, realm,
        client, RegistrationAuth.AUTHENTICATED);
  ...
  public ScopeMappedResource getScopeMappedResource() {
    ...
    AdminPermissionEvaluator.RequirePermissionCheck viewCheck = () -> auth.clients()
        .requireView(client);
    return new ScopeMappedResource(realm, auth, client, session, adminEvent, manageCheck,
        viewCheck);
  }

  @Path("roles")
  public RoleContainerResource getRoleContainerResource() {
    return new RoleContainerResource(session, session.getContext().getUri(), realm, auth, client,
        adminEvent);
  }
  ...
  private Stream<ClientScopeRepresentation> getDefaultClientScopes(boolean defaultScope) {
    ...
    return client.getClientScopes(defaultScope).values().stream()
  ...
  private void addDefaultClientScope(String clientScopeId, boolean defaultScope) {
    ...
    if (defaultScope && clientScope.isDynamicScope()) {
      throw new ErrorResponseException("invalid_request",
          "Can't assign a Dynamic Scope to a Client as a Default Scope",
          Response.Status.BAD_REQUEST);
    }
    client.addClientScope(clientScope, defaultScope);

    adminEvent.operation(OperationType.CREATE).resource(ResourceType.CLIENT_SCOPE_CLIENT_MAPPING)
        .resourcePath(session.getContext().getUri()).success();
  }
  ...
  public void removeDefaultClientScope(@PathParam("clientScopeId") String clientScopeId) {
    ...
    adminEvent.operation(OperationType.DELETE).resource(ResourceType.CLIENT_SCOPE_CLIENT_MAPPING)
        .resourcePath(session.getContext().getUri()).success();
  }
  ...
  public ClientScopeEvaluateResource clientScopeEvaluateResource() {
    return new ClientScopeEvaluateResource(session, session.getContext().getUri(), realm, auth,
  ...
  public GlobalRequestResult pushRevocation() {
    ...
    adminEvent.operation(OperationType.ACTION).resourcePath(session.getContext().getUri())
        .resource(ResourceType.CLIENT).success();
  ...
  @NoCache
  @Produces(MediaType.APPLICATION_JSON)
  public Stream<UserSessionRepresentation> getUserSessions(@QueryParam("first") Integer firstResult,
    ...
    return session.sessions()
        .getUserSessionsStream(client.getRealm(), client, firstResult, maxResults)
  ...
  @NoCache
  @Produces(MediaType.APPLICATION_JSON)
  public Stream<UserSessionRepresentation> getOfflineUserSessions(
    ...
    return session.sessions()
        .getOfflineUserSessionsStream(client.getRealm(), client, firstResult, maxResults)
   ...
   * <p>
   * Manually register cluster node to this client - usually it's not needed to call this directly
   * as adapter should handle by sending registration request to Keycloak
   *
   * @param formParams
   */
  @Path("nodes")
  ...
  public void registerNode(Map<String, String> formParams) {
    ...
    adminEvent.operation(OperationType.CREATE).resource(ResourceType.CLUSTER_NODE)
        .resourcePath(session.getContext().getUri(), node).success();
  }
  ...
  public void unregisterNode(final @PathParam("node") String node) {
    ...
    adminEvent.operation(OperationType.DELETE).resource(ResourceType.CLUSTER_NODE)
        .resourcePath(session.getContext().getUri()).success();
  }
  ...
  public GlobalRequestResult testNodesAvailable() {
    ...
    GlobalRequestResult result = new ResourceAdminManager(session).testNodesAvailability(realm,
        client);
    adminEvent.operation(OperationType.ACTION).resource(ResourceType.CLUSTER_NODE)
        .resourcePath(session.getContext().getUri()).representation(result).success();
    return result;
  }
  ...
  /**
   * Return object stating whether client Authorization permissions have been initialized or not and
   * a reference
   *
   * @return
   */
  ...
  @Consumes(MediaType.APPLICATION_JSON)
  @NoCache
  public ManagementPermissionReference setManagementPermissionsEnabled(
  ...
  private void updateClientFromRep(ClientRepresentation rep, ClientModel client,
      KeycloakSession session) throws ModelDuplicateException {
    ...
    if (rep.isFullScopeAllowed() != null
        && rep.isFullScopeAllowed() != client.isFullScopeAllowed()) {
      auth.clients().requireManage(client);
    }

    if ((rep.isBearerOnly() != null && rep.isBearerOnly()) || (rep.isPublicClient() != null
        && rep.isPublicClient())) {
   ...
   */
  private UserSessionRepresentation toUserSessionRepresentation(
      final UserSessionModel userSession) {
    UserSessionRepresentation rep = ModelToRepresentation.toRepresentation(userSession);

    // Update lastSessionRefresh with the timestamp from clientSession
    Map.Entry<String, AuthenticatedClientSessionModel> result = userSession.getAuthenticatedClientSessions()
        .entrySet().stream().filter(entry -> Objects.equals(client.getId(), entry.getKey()))
