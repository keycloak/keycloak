...
class ClientPermissions implements ClientPermissionEvaluator,  ClientPermissionManagement {
    ...
    private String getExchangeToPermissionName(ClientModel client) {
     ...
     private void initialize(ClientModel client) {
        ...
        Resource resource = authz.getStoreFactory().getResourceStore().findByName(resourceName, server);
        if (resource == null) {
    ...
    private void deletePermissions(ClientModel client) {
        ...
        Resource resource = authz.getStoreFactory().getResourceStore().findByName(getResourceName(client), server);;
    ...
    public boolean isPermissionsEnabled(ClientModel client) {
        ...
        return authz.getStoreFactory().getResourceStore().findByName(getResourceName(client), server) != null;
    }
    ...
    public boolean canExchangeTo(ClientModel authorizedClient, ClientModel to) {
        ...
        if (!authorizedClient.equals(to)) {
            ...
            Resource resource =  authz.getStoreFactory().getResourceStore().findByName(getResourceName(to), server);
            if (resource == null) {
