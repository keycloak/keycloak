...
class RealmPermissions implements RealmPermissionEvaluator {
    ...
    public boolean canListRealms() {
        return canViewRealm() || root.hasOneAdminRole(AdminRoles.ALL_QUERY_ROLES) || root.hasOneAdminRole(AdminRoles.ALL_MANAGE_ROLES);
    }
    ...
    public void requireViewRequiredActions() {
        if (!(canViewRealm() || root.hasOneAdminRole(AdminRoles.QUERY_USERS, AdminRoles.MANAGE_USERS))) {
            throw new ForbiddenException();
        }
    }
