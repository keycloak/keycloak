...
public class AccountIssuedVerifiableCredentialResource {
    ...
    public Response revokeIssuedCredential(@PathParam("credentialId") String credentialId) {
        ...
        if (!belongsToUser) {
            logger.warnf("Issued credential with ID '%s' not found for user '%s' in the realm '%s'.", credentialId, user.getUsername(), realm.getName());
        ...
        if (!removed) {
            logger.warnf("Failed to remove issued credential with ID '%s' for user '%s' in realm '%s'.", credentialId, user.getUsername(), realm.getName());
            throw new NotFoundException("Issued credential not found");
        }
