...
public class RejectMayActClaimExecutor implements ClientPolicyExecutorProvider<RejectMayActClaimExecutor.Configuration> {
    ...
    public static class Configuration extends ClientPolicyExecutorConfigurationRepresentation {
        ...
        protected Boolean avoidPermission;
        @JsonProperty(RejectMayActClaimExecutorFactory.AVOID_CONSENT_CHECK)
        protected Boolean avoidConsent;

        public Configuration() {
            // empty
        }

        public Configuration (boolean rejectAny, boolean avoidPermission, boolean avoidConsent) {
            this.rejectAny = rejectAny;
            this.avoidPermission = avoidPermission;
            this.avoidConsent = avoidConsent;
        }
        ...
        public Boolean isAvoidPermission() {
            return avoidPermission;
        }

        public void setAvoidPermission(Boolean avoidPermission) {
            this.avoidPermission = avoidPermission;
        }

        public Boolean isAvoidConsent() {
            return avoidConsent;
        }

        public void setAvoidConsent(Boolean avoidConsent) {
            this.avoidConsent = avoidConsent;
        }
    }
    ...
    private void validate(AbstractTokenResponseContext context) throws ClientPolicyException {
        ...
        if (admin.getServiceAccountClientLink() != null) {
            ...
            if (client == null || !client.isEnabled()) {
                logger.debugf("Invalid service account '%s' in the may_act sub", admin.getUsername(), user.getUsername(), realm.getName());
                throw new ClientPolicyException(OAuthErrorException.INVALID_REQUEST, "Invalid may_act sub in the token");
            }
        }

        // check if the admin in sub has delegation permission over the user
        if (!Boolean.TRUE.equals(configuration.isAvoidPermission())) {
        ...
        if (!Boolean.TRUE.equals(configuration.isAvoidConsent())) {
