    ...
    private static final ProviderConfigProperty HOSTNAME_ALLOWED_PATTERNS_PROPERTY =
            new ProviderConfigProperty(
                    ...
                    "A node hostname is accepted only if it matches at least one of "
                                + "these regex patterns. Matching is case-sensitive. "
                                + "DNS hostnames and IPv4 addresses are matched as-is. "
                                + "IPv6 addresses must be supplied in bracket notation (e.g. [2001:db8::1]); "
                                + "patterns are matched against the address without brackets (e.g. ^2001:db8::1$). "
                                + "Bare IPv6 without brackets and port suffixes are always rejected. "
