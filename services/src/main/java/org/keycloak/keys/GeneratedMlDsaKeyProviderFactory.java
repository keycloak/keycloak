...
public class GeneratedMlDsaKeyProviderFactory extends AbstractMlDsaKeyProviderFactory implements EnvironmentDependentProviderFactory {
    ...
    static String getAlgorithmFromPublicKey(String publicKeyBase64) {
        ...
        for (String algorithm : List.of(Algorithm.ML_DSA_44, Algorithm.ML_DSA_65, Algorithm.ML_DSA_87)) {
            try {
                KeyFactory keyFactory = CryptoIntegration.getProvider().getKeyFactory(algorithm);
                keyFactory.generatePublic(publicKeySpec);
                return algorithm;
            } catch (Exception ignored) {
                // Try the next supported ML-DSA parameter set.
            }
        }
