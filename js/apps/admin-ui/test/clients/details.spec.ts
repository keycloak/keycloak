...
test.describe.serial("OIDC Signing Key Selection", () => {
  ...
  test.beforeAll(async () => {
    ...
    if (!testKey?.kid) {
      throw new Error("Generated OIDC signing key was not found");
    }
  ...
  test("should show key status changes: passive, disabled, not found", async ({
    ...
    await assertAccessTokenSigningKeyDisplayText(
      page,
      new RegExp(`\\(Passive,.*${testKeyKid.slice(0, 12)}`),
    );
    ...
    await assertAccessTokenSigningKeyDisplayText(
      page,
      new RegExp(`\\(Disabled,.*${testKeyKid.slice(0, 12)}`),
    );
    ...
    await assertAccessTokenSigningKeyDisplayText(
      page,
      new RegExp(`Not found.*${testKeyKid.slice(0, 12)}`),
    );
  });
});
