...
async function selectSigningKeyByKid(
  ...
  if (keyId === "") {
    ...
    // The option label shows a truncated kid, but the full kid is in the option
    // description, so match the option by the full kid.
    await page.getByRole("option").filter({ hasText: keyId }).click();
  }
}
...
// The collapsed toggle shows the label only (metadata + a truncated kid, not the
// description), so assert against the truncated kid prefix (first 12 chars, matching
// KID_LABEL_LENGTH in SigningKeySelect).
async function assertSigningKeyValue(field: Locator, expectedKid: string) {
  if (expectedKid === "") {
    await expect(field).toHaveText(REALM_ACTIVE_KEY_TEXT);
  } else {
    await expect(field).toHaveText(new RegExp(`- ${expectedKid.slice(0, 12)}`));
  }
}
