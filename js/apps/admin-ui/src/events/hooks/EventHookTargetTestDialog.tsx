...
export const EventHookTargetTestDialog = ({
  ...
  const formatTestExampleLabel = (
    example: EventHookTestExampleRepresentation,
  ) => `${example.sourceType}: ${example.eventName}`;

  const testFailureMessage = (
    testResult: EventHookTestResultRepresentation,
  ) => {
  ...
  useEffect(() => {
    ...
    void adminClient.eventHooks
      .findTestExamples({ realm })
      .then((examples) => {
        if (!active) {
          return;
        }
    ...
    >
      <div className="pf-v5-u-mb-md">
        <div className="pf-v5-u-font-weight-bold">
          {target.name || t("eventHookTargets")}
        </div>
