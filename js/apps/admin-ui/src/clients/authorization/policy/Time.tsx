...
const NumberControl = ({ name, min, max }: NumberControlProps) => {
  const { control } = useFormContext();
  const setValue = (newValue: number) => Math.max(Math.min(newValue, max), min);
    ...
    <Controller
      ...
      render={({ field }) => (
        <NumberInput
          ...
          min={min}
          max={max}
          onPlus={() => field.onChange(Math.min(Number(field.value) + 1, max))}
          onMinus={() =>
            field.onChange(Math.max(Number(field.value) - 1, min))
          }
          onChange={(event) => {
