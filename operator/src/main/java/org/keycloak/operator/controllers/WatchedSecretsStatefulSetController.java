...
@ApplicationScoped
@ControllerConfiguration(namespaces = WATCH_CURRENT_NAMESPACE, labelSelector = Constants.DEFAULT_LABELS_AS_STRING)
public class WatchedSecretsStatefulSetController implements Reconciler<StatefulSet>, EventSourceInitializer<StatefulSet>, WatchedSecrets {
    ...
    private volatile IndexerResourceCache<StatefulSet> cache;
    private volatile IndexerResourceCache<Secret> secrets;

    @Override
    public void processWatched(List<String> desiredWatchedSecretsNames, Keycloak keycloakCR, StatefulSet deployment) {
    ...
    @Override
    public Map<String, EventSource> prepareEventSources(EventSourceContext<StatefulSet> context) {
        cache = context.getPrimaryCache();
        return Map.of();
    }

    @Override
    public UpdateControl<StatefulSet> reconcile(StatefulSet resource, Context<StatefulSet> context) throws Exception {
        for (Secret secret : fetchSecrets(getSecretNames(resource), resource.getMetadata().getNamespace())) {
            if (!secret.getMetadata().getLabels().containsKey(Constants.KEYCLOAK_COMPONENT_LABEL)) {
                ...
                client.resource(secret).edit(s -> new SecretBuilder(s)
                                .editMetadata()
                                .withResourceVersion(null)
                                .addToLabels(Constants.KEYCLOAK_COMPONENT_LABEL, WatchedSecrets.WATCHED_SECRETS_LABEL_VALUE)
                                .endMetadata()
                                .build());
            }
        }
    ...
    public void setSecrets(IndexerResourceCache<Secret> secrets) {
    ...
    public UpdateControl<Secret> reconcileSecret(Secret resource, Context<Secret> context) {
