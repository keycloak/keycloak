...
public abstract class AbstractKeycloak<T extends HasMetadata> extends OperatorManagedResource implements StatusUpdater<KeycloakStatusBuilder> {

    protected final Config config;
    protected final Keycloak keycloakCR;
    protected final T existing;
    protected final T base;
    protected final String adminSecretName;

    protected Set<String> serverConfigSecretsNames;

    public AbstractKeycloak(KubernetesClient client, Config config, Keycloak keycloakCR, T existing, String adminSecretName) {
        ...
        if (existing != null) {
            ...
            this.existing = fetchExisting();
        }

        this.base = createBase();
    }

    protected abstract T fetchExisting();

    protected abstract T createBase();

    public abstract void rollingRestart();
    ...
    protected <T, V> void mergeMaps(Map<T, V> map1, Map<T, V> map2, Consumer<Map<T, V>> consumer) {
    ...
    protected void mergePodTemplate(PodTemplateSpec baseTemplate) {
        if (keycloakCR.getSpec() == null ||
                keycloakCR.getSpec().getUnsupported() == null ||
                keycloakCR.getSpec().getUnsupported().getPodTemplate() == null) {
            return;
    ...
    protected void configureHostname(Container kcContainer) {
    ...
    protected void configureTLS(PodTemplateSpec podTemplateSpec) {
        var kcContainer = podTemplateSpec.getSpec().getContainers().get(0);
        ...
        if (this.keycloakCR.getSpec().isHttp()) {
            ...
            podTemplateSpec.getSpec().getVolumes().add(volume);
