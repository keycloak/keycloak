...
public class DefaultClientService implements ClientService {
    ...
    public Optional<BaseClientRepresentation> getClient(RealmModel realm, String clientId, ClientProjectionOptions projectionOptions) {
        if (clientResource != null) {
            var model = clientResource.viewClientModel();
            var mapper = Optional.ofNullable(session.getProvider(ClientModelMapper.class, model.getProtocol()))
                    .orElseThrow(() -> new ServiceException("Cannot find mapper for the specific kind of client", Response.Status.BAD_REQUEST));
            // TODO: is the access map on the representation needed
            return Optional.ofNullable(mapper.fromModel(model));
        } else {
            if (!permissions.clients().canList()) {
