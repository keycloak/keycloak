...
public class DefaultAdminApi implements AdminApi {
    ...
    public ClientsApiGroup clientsGroupDefault() {
        return clientsGroup("v2");
    }

    @Path("clients-api/{version:v\\d+}")
    @Override
    public ClientsApiGroup clientsGroup(@PathParam("version") String version) {
        return switch (version) {
            case "v2" -> new DefaultClientsApiGroup(session, realmAdminResource);
