...
public interface HttpClientProvider extends Provider {
     ...
     * (RFC 8705 mTLS).
     * <p>
     * The key material is merged into the server-wide TLS configuration rather than replacing it, so it
     * is also honored when the outbound client is configured with {@code disable-trust-manager} (the
     * client still presents its certificate while server trust verification is bypassed).
     * <p>
