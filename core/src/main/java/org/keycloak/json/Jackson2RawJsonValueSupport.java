...
public final class Jackson2RawJsonValueSupport implements RawJsonValueSupport {
    ...
    @Override
    public RawJsonValue path(RawJsonValue node, String key) {
        JsonNode jn = toNode(node.getValue());
        return RawJsonValue.of(jn.path(key));
    }
    ...
    @Override
    public String asText(Object value, String defaultValue) {
        return toNode(value).asText(defaultValue);
    }
