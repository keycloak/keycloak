...
@JsonPropertyOrder({"id", CONFIG_NAME, CONFIG_USES, CONFIG_ENABLED, CONFIG_ON_EVENT, CONFIG_RESET_ON, CONFIG_SCHEDULED, CONFIG_RECURRING, CONFIG_CONDITIONS, CONFIG_STEPS, CONFIG_ERROR})
...
public final class WorkflowRepresentation extends AbstractWorkflowComponentRepresentation {
    ...
    public WorkflowRepresentation(String id, String uses, MultivaluedHashMap<String, String> config, List<WorkflowConditionRepresentation> conditions, List<WorkflowStepRepresentation> steps) {
    ...
    public WorkflowStatus getStatus() {
        List<String> error = getConfigValues(CONFIG_ERROR);

        if (error == null || error.isEmpty()) {
            return null;
        }

        return new WorkflowStatus(error);
    }

    public void setStatus(WorkflowStatus status) {
        if (status != null) {
            setConfigValue(CONFIG_ERROR, status.getErrors());
        } else {
            getConfig().remove(CONFIG_ERROR);
        }
    }
