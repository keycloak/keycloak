...
const DEFAULT_APP_PATH = path.join(REPO_ROOT, "js", "apps", "admin-ui");
...
function usage() {
  console.log(`Usage:
  node misc/scripts/migrate-admin-ui-pf5-to-pf6.mjs [options]

Options:
  --app-path <path>          Admin UI app root (default: js/apps/admin-ui)
...
async function resolveAppDir(appPath) {
  ...
  if (!(await pathExists(packageJson))) {
    throw new Error(`Could not find admin-ui app at '${absolute}'. Expected package.json.`);
  }
...
function toMarkdownReport(report) {
  ...
  return `# Admin UI PF5 -> PF6 Migration Report
...
async function main() {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "kc-pf6-admin-migrate-"));
  try {
  const stagedAppDir = path.join(tempRoot, "admin-ui");
  await cp(appDir, stagedAppDir, { recursive: true });

  const steps = [];
  const stagedAppDirRelative = "admin-ui";
  ...
  if (!failedStep && options.apply && !hasUnresolvedResiduals) {
    ...
    backupPath = path.join(options.backupDir, `admin-ui-${runTimestamp}`);
  ...
  const reportPrefix = `admin-ui-pf5-to-pf6-${runTimestamp}`;
