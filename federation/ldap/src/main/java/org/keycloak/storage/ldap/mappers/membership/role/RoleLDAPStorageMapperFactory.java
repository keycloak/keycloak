...
public class RoleLDAPStorageMapperFactory extends AbstractLDAPStorageMapperFactory {
    ...
    private static List<ProviderConfigProperty> getProps(ComponentModel parent) {
        ...
        config.property().name(RoleMapperConfig.USER_ROLES_RETRIEVE_STRATEGY)
                ...
                .defaultValue("true")
                .add()
                .property().name(RoleMapperConfig.DROP_NON_EXISTING_ROLES_DURING_SYNC)
                .label("Drop non existing LDAP roles")
                .helpText("During LDAP to Keycloak role sync, remove roles created by this LDAP role mapper that no longer exist in LDAP. Existing local roles and roles from other mappers are preserved. Requires a complete LDAP search result.")
                .type(ProviderConfigProperty.BOOLEAN_TYPE)
                .defaultValue("false")
                .add()
