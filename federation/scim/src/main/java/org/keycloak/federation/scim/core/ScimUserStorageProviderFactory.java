 ...
 */
public class ScimUserStorageProviderFactory implements UserStorageProviderFactory<ScimUserStorageProvider>, ImportSynchronization {

    public static final String ID = "scim-federation";
    ...
    public SynchronizationResult sync(KeycloakSessionFactory sessionFactory, String realmId, UserStorageProviderModel model) {
        ...
        KeycloakModelUtils.runJobInTransaction(sessionFactory, session -> {
            ...
            if (BooleanUtils.TRUE.equals(model.get(ScimEndPointConfiguration.CONF_KEY_PROPAGATION_USER))) {
                dispatcher.dispatchUserModificationToOne(model, client -> client.sync(result));
            }
            if (BooleanUtils.TRUE.equals(model.get(ScimEndPointConfiguration.CONF_KEY_PROPAGATION_GROUP))) {
    ...
    public List<ProviderConfigProperty> getConfigProperties() {
        ...
        return ProviderConfigurationBuilder.create().property().name(ScimEndPointConfiguration.CONF_KEY_ENDPOINT)
                ...
                .add().property().name(ScimEndPointConfiguration.CONF_KEY_CONTENT_TYPE).type(ProviderConfigProperty.LIST_TYPE)
                ...
                .add().property().name(ScimEndPointConfiguration.CONF_KEY_AUTH_MODE).type(ProviderConfigProperty.LIST_TYPE)
                .label("Auth mode").helpText("Select the authorization mode").options("NONE", "BASIC_AUTH", "BEARER")
                .defaultValue("NONE").add().property().name(ScimEndPointConfiguration.CONF_KEY_AUTH_USER)
                .type(ProviderConfigProperty.STRING_TYPE).label("Auth username").helpText("Required for basic authentication.")
                .add().property().name(ScimEndPointConfiguration.CONF_KEY_AUTH_PASSWORD).type(ProviderConfigProperty.PASSWORD)
                .label("Auth password/token").helpText("Password or token required for basic or bearer authentication.").add()
                .property().name(ScimEndPointConfiguration.CONF_KEY_PROPAGATION_USER).type(ProviderConfigProperty.BOOLEAN_TYPE)
                .label("Enable user propagation").helpText("Should operation on users be propagated to this provider?")
                .defaultValue(BooleanUtils.TRUE).add().property().name(ScimEndPointConfiguration.CONF_KEY_PROPAGATION_GROUP)
                ...
                .property().name(ScimEndPointConfiguration.CONF_KEY_SYNC_IMPORT).type(ProviderConfigProperty.BOOLEAN_TYPE)
                .label("Enable import during sync").add().property()
                .name(ScimEndPointConfiguration.CONF_KEY_SYNC_IMPORT_ACTION).type(ProviderConfigProperty.LIST_TYPE)
                ...
                .name(ScimEndPointConfiguration.CONF_KEY_SYNC_REFRESH).type(ProviderConfigProperty.BOOLEAN_TYPE)
                .label("Enable refresh during sync").name(ScimEndPointConfiguration.CONF_KEY_LOG_ALL_SCIM_REQUESTS)
    ...
    public ScimUserStorageProvider create(KeycloakSession session, ComponentModel model) {
        return new ScimUserStorageProvider(session, new ScimEndPointConfiguration(model));
    }
}
