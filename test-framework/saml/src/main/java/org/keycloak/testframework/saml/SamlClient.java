...
public class SamlClient {
    ...
    public SAMLDocumentHolder doArtifactResolve(String artifact) {
        ...
        try {
            ArtifactResolveType artifactResolve = new ArtifactResolveType("_" + UUID.randomUUID(), XMLTimeUtil.getIssueInstant());
            artifactResolve.setDestination(URI.create(artifactResolutionEndpoint));
            ...
            new SAMLRequestWriter(StaxUtil.getXMLStreamWriter(xmlOut)).write(artifactResolve);
            Document doc = DocumentUtil.getDocument(new ByteArrayInputStream(xmlOut.toByteArray()));
            ...
            try (CloseableHttpResponse response = httpClient.execute(post, httpContext)) {
                ...
                if (status != 200) {
                    ...
                    throw new RuntimeException("ArtifactResolve failed with HTTP " + status + ": " + body);
                }
     ...
     * Navigates to the realm base URL first: Keycloak's SSO cookies are scoped to {@code /realms/{realm}/},
     * so WebDriver only sees (and can delete) them from a page under the realm path.
     */
    public void clearCookies() {
        managedWebDriver.driver().navigate().to(getRealmBaseUrl());
        managedWebDriver.cookies().deleteAll();
    }
    ...
    public String getFrontchannelLogoutUrl() {
        return testSamlApp.getFrontchannelLogoutUrl();
    }

    /**
     * Configure the mock SP to complete the front-channel SLO chain: it will auto-POST a LogoutResponse
     * (issued by the given entity) back to Keycloak, so Keycloak finishes the logout and clears the session.
     */
    public void setFrontchannelLogoutResponder(String issuer) {
        testSamlApp.setFrontchannelLogoutIssuer(issuer);
        testSamlApp.setKeycloakSamlEndpoint(getSamlEndpoint());
    }

    public void clearLastFrontchannelLogoutDocument() {
        testSamlApp.clearLastFrontchannelLogoutDocument();
    }

    /**
     * Poll until the browser-driven frontchannel LogoutRequest is received at the SP (or times out).
     */
    public Document getLastFrontchannelLogoutDocument() {
        long deadline = Instant.now().plusSeconds(10).toEpochMilli();
        while (Instant.now().toEpochMilli() < deadline) {
            Document doc = testSamlApp.getLastFrontchannelLogoutDocument();
            if (doc != null) {
                return doc;
            }
            try {
        ...
        return testSamlApp.getLastFrontchannelLogoutDocument();
    }
    ...
    private Document createLogoutRequestDocument(String nameId, String sessionIndex) {
        try {
            ...
            nameIdType.setValue(nameId);

            return new SAML2LogoutRequestBuilder()
                    .issuer(clientId)
                    .nameId(nameIdType)
                    .sessionIndex(sessionIndex)
                    .destination(getSamlEndpoint())
                    .buildDocument();
        } catch (Exception e) {
