...
public class SamlClientSupplier implements Supplier<SamlClient, InjectSamlClient> {
    ...
    public SamlClient getValue(InstanceContext<SamlClient, InjectSamlClient> instanceContext) {
        ...
        ClientRepresentation clientRep = clientConfig.configure(ClientBuilder.create())
                .redirectUris(acsUrl + "/*")
        ...
        if (!instanceContext.getAnnotation().ref().isEmpty()) {
            clientId = clientId + "-" + instanceContext.getAnnotation().ref();
