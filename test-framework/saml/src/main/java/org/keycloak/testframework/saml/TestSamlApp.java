...
public class TestSamlApp {
    ...
    public static final String FRONTCHANNEL_LOGOUT_PATH = "/saml/frontchannel-logout";
    ...
    private volatile String frontchannelLogoutIssuer;
    private volatile String keycloakSamlEndpoint;
    private final AtomicReference<Document> lastBackchannelLogoutDocument = new AtomicReference<>();
    private final AtomicReference<Document> lastFrontchannelLogoutDocument = new AtomicReference<>();
    ...
    public TestSamlApp(HttpServer httpServer) {
        ...
        frontchannelLogoutUrl = baseUrl + FRONTCHANNEL_LOGOUT_PATH;
        ...
        httpServer.createContext(BACKCHANNEL_LOGOUT_PATH, exchange -> {
            byte[] responseBytes;
            try {
                // Build the full response before committing the status, so a parse/build
                // failure can still be reported as 500 (headers aren't sent yet).
                ...
                responseBytes = Soap.createMessage().addToBody(responseDoc).getBytes();
        ...
        // Frontchannel logout endpoint - receives a POST-binding LogoutRequest via the browser, captures it,
        // and (when configured) auto-POSTs a LogoutResponse back to Keycloak to complete the SLO chain so the
        // logout finishes cleanly and Keycloak clears the browser's SSO session.
        httpServer.createContext(FRONTCHANNEL_LOGOUT_PATH, exchange -> {
            String body = new String(exchange.getRequestBody().readAllBytes(), StandardCharsets.UTF_8);
            String samlRequest = extractParam(body, GeneralConstants.SAML_REQUEST_KEY);

            String html;
            if (!samlRequest.isEmpty()) {
                SAMLDocumentHolder holder = SAMLRequestParser.parseRequestPostBinding(samlRequest);
                lastFrontchannelLogoutDocument.set(holder.getSamlDocument());
                html = buildLogoutResponseForm((LogoutRequestType) holder.getSamlObject());
            } else {
                html = "<html><body>Frontchannel logout received</body></html>";
            }
    ...
    public Document getLastFrontchannelLogoutDocument() {
        return lastFrontchannelLogoutDocument.get();
    }

    public void clearLastFrontchannelLogoutDocument() {
        lastFrontchannelLogoutDocument.set(null);
    }
    ...
    public void setFrontchannelLogoutIssuer(String issuer) {
        this.frontchannelLogoutIssuer = issuer;
    }

    public void setKeycloakSamlEndpoint(String keycloakSamlEndpoint) {
        this.keycloakSamlEndpoint = keycloakSamlEndpoint;
    }

    // Build an auto-submit form that POSTs a LogoutResponse back to Keycloak, completing the SLO chain.
    private String buildLogoutResponseForm(LogoutRequestType logoutRequest) {
        if (keycloakSamlEndpoint == null) {
            return "<html><body>Frontchannel logout received</body></html>";
        }
        try {
            String issuer = frontchannelLogoutIssuer != null ? frontchannelLogoutIssuer : "unknown";
            StatusResponseType logoutResponse = new SAML2LogoutResponseBuilder()
                    .issuer(issuer)
                    .logoutRequestID(logoutRequest.getID())
                    .destination(keycloakSamlEndpoint)
                    .buildModel();
            Document responseDoc = SAML2Response.convert(logoutResponse);
            String encoded = new BaseSAML2BindingBuilder().postBinding(responseDoc).encoded();
            return """
                    <html>
                        <body onload="document.forms[0].submit()">
                            <form method="POST" action="%s">
                    ...
                    """.formatted(keycloakSamlEndpoint, GeneralConstants.SAML_RESPONSE_KEY, encoded);
        } catch (Exception e) {
            throw new RuntimeException("Failed to build LogoutResponse", e);
        }
    }
