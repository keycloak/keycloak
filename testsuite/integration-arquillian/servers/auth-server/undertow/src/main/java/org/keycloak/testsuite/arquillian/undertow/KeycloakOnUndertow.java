/*
 * Copyright 2016 Red Hat, Inc. and/or its affiliates and other contributors as indicated by the @author tags. Licensed
...
public class KeycloakOnUndertow implements DeployableContainer<KeycloakOnUndertowConfiguration>
{
  ...
  public void start() throws LifecycleException
  {
    ...
    log.infof("Starting auth server on embedded Undertow on: http://%s:%d", configuration.getBindAddress(),
    ...
    undertow.start(Undertow.builder().addHttpListener(configuration.getBindHttpPort(), configuration.getBindAddress())
        ...
        .setWorkerThreads(configuration.getWorkerThreads()).setIoThreads(configuration.getWorkerThreads() / 8));
    ...
    ResteasyDeployment deployment =
        (ResteasyDeployment) di.getServletContextAttributes().get(ResteasyDeployment.class.getName());
    sessionFactory = ((KeycloakApplication) deployment.getApplication()).getSessionFactory();

    setupDevConfig();

    log.infof("Auth server started in %dms on http://%s:%d/auth", (System.currentTimeMillis() - start),
  ...
  private static class KeycloakUndertowJaxrsServer extends UndertowJaxrsServer
  {
    ...
    public KeycloakUndertowJaxrsServer start(Undertow.Builder builder)
    {
      try
      {
        // Need to wrap the original handler with ProxyPeerAddressHandler. Thanks to that, if undertow is behind proxy
        // and the proxy
        // forwards "https" request to undertow as "http" request, undertow will be able to establish protocol correctly
        // on the request
