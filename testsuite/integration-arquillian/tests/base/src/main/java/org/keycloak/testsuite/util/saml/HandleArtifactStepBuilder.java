...
public class HandleArtifactStepBuilder extends SamlDocumentStepBuilder<ArtifactResolveType, HandleArtifactStepBuilder> implements StepWithCheckers {
    ...
    public HttpUriRequest perform(CloseableHttpClient client, URI currentURI, CloseableHttpResponse currentResponse, HttpClientContext context) throws Exception {
        ...
        if (beforeStepChecker != null && beforeStepChecker instanceof SessionStateChecker) {
            ...
            sessionStateChecker.setUserSessionProvider(session -> session.getProvider(SingleUseStoreProvider.class).get(artifact).get(SamlProtocol.USER_SESSION_ID));
            sessionStateChecker.setClientSessionProvider(session -> session.getProvider(SingleUseStoreProvider.class).get(artifact).get(SamlProtocol.CLIENT_SESSION_ID));
        }
