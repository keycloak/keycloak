 ...
 *
 * Tests for the attribute store protocol mapper for the REST attribute store
 */
public class RESTAttributeMapperTest extends AbstractRESTAttributeStoreTest {

    /**
     * Test the attribute mapper with a REST api attribute store
     */
    @Test
    public void testBasic() {
        addAttributeStoreProtocolMapper(ATTR_LOOKUP_REALM, "oidc-attribute-store-mapper", new HashMap<>(){{
            put(AttributeStoreMapper.CONFIG_ATTRIBUTE_POINTER, "/userinfo_endpoint");
        ...
        assertClaim(tokens.idToken, "test-attr-1", oauth.getUserInfoUrl());
        assertClaim(tokens.accessToken, "test-attr-1", oauth.getUserInfoUrl());
    }
}