...
public class KcSamlSignedBrokerTest extends AbstractBrokerTest {
    ...
    public void withSignedEncryptedAssertions(Runnable testBody, boolean signedDocument, boolean signedAssertion, boolean encryptedAssertion) throws Exception {
        ...
        String providerSigCert = KeyUtils.getActiveSigningKeyOfType(providerKeysMetadata, KeyType.RSA).getCertificate();
        Assert.assertThat(providerSigCert, Matchers.notNullValue());

        String consumerEncCert = KeyUtils.getActiveEncKeyOfType(consumerKeysMetadata, KeyType.RSA).getCertificate();
        Assert.assertThat(consumerEncCert, Matchers.notNullValue());

        String consumerSigCert = KeyUtils.getActiveSigningKeyOfType(consumerKeysMetadata, KeyType.RSA).getCertificate();
    ...
    public class KcSamlSignedBrokerConfiguration extends KcSamlBrokerConfiguration {
        ...
        public List<ClientRepresentation> createProviderClients() {
            ...
            String consumerCert = KeyUtils.getActiveSigningKeyOfType(adminClient.realm(consumerRealmName()).keys().getKeyMetadata(), KeyType.RSA).getCertificate();
        ...
        public IdentityProviderRepresentation setUpIdentityProvider(IdentityProviderSyncMode syncMode) {
            ...
            String providerCert = KeyUtils.getActiveSigningKeyOfType(adminClient.realm(providerRealmName()).keys().getKeyMetadata(), KeyType.RSA).getCertificate();
    ...
    public void testSignatureDataWhenWantsRequestsSigned() throws Exception {
        ...
        String providerCert = KeyUtils.getActiveSigningKeyOfType(adminClient.realm(bc.providerRealmName()).keys().getKeyMetadata(), KeyType.RSA).getCertificate();
        Assert.assertThat(providerCert, Matchers.notNullValue());

        String consumerCert = KeyUtils.getActiveSigningKeyOfType(adminClient.realm(bc.consumerRealmName()).keys().getKeyMetadata(), KeyType.RSA).getCertificate();
        Assert.assertThat(consumerCert, Matchers.notNullValue());
