...
public class RPInitiatedFrontChannelLogoutTest extends AbstractChangeImportedUserPasswordsTest {
    ...
    public void testFrontChannelLogoutRedirectUriIsEscapedInJs() throws Exception {
        ...
        try (ClientAttributeUpdater updater = ClientAttributeUpdater
            ...
            Assert.assertFalse("Redirect URI must not be rendered in a single-quoted JS string",
                    pageSource.contains("window.location.replace('"));
            Assert.assertTrue("Redirect URI should be rendered as a double-quoted JS string via FreeMarker ?c outputformat escaping",
                    pageSource.contains("window.location.replace(\"" + specialCharsUri + "\")"));
        }
    }
