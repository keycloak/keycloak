...
public class KcRegUpdateTokenTest extends AbstractRegCliTest {
    ...
    public void testUpdateToken() throws IOException {
        ...
        try (TempFileResource configFile = new TempFileResource(handler.getConfigFile())) {
            testUpdateTokenUsingConfig(serverUrl, configFile.getFile(), "");
        }
    }

    static void testUpdateTokenUsingConfig(String serverUrl, File config, String trustStoreConfig) throws IOException {

        KcRegExec exe = execute("config credentials --config '" + config + "' --server " + serverUrl + " --realm master --user admin --password admin");
        ...
        exe = execute("update-token --config '" + config + "' reg-cli-secret-direct  --server " + serverUrl + " --realm test --user user1 --password userpass");
        ...
        exe = execute("update-token reg-cli-secret-direct --no-config --server " + serverUrl + " --realm test --user user1 --password userpass " + trustStoreConfig);
        ...
        exe = execute("get reg-cli-secret-direct --no-config --server " + serverUrl + " --realm test -t " + token + " " + trustStoreConfig);
