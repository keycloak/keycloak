...
public class RPInitiatedLogoutTest extends AbstractTestRealmKeycloakTest {
    ...
    public void logoutWithPostLogoutRedirectUriAndLogoutStateDeduplication() throws Exception {
        ...
        try {
            ...
            String currentUrl = driver.getCurrentUrl();
            assertTrue("Should contain existing parameter", currentUrl.contains("existing=true"));
            assertTrue("Should contain new state parameter", currentUrl.contains("state=logout-state-456"));
            assertFalse("Should not contain preloaded state parameter", currentUrl.contains("preloaded-state"));
            MatcherAssert.assertThat(false, is(isSessionActive(tokenResponse.getSessionState())));
        } finally {
            allowOidcParamsInRedirectUris(false);
        }
    ...
    public void testPostLogoutRedirectWithStateForbiddenByDefaultAndAllowedWhenConfigured() {
        ...
        try {
            ...
            events.expectLogout(tokenResponse.getSessionState())
    ...
    private void allowOidcParamsInRedirectUris(boolean allow) {
        getTestingClient().testing().setSystemPropertyOnServer(
                "oidc.allow-oidc-params-in-redirect-uris", String.valueOf(allow));
        getTestingClient().testing().reinitializeProviderFactoryWithSystemPropertiesScope(
