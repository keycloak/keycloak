...
public class ConcurrentTransactionsTest extends AbstractTestRealmKeycloakTest {
    ...
    public void persistClient(KeycloakSession session) {
        ...
        KeycloakModelUtils.runJobInTransaction(session.getKeycloakSessionFactory(), (KeycloakSession session1) -> {
            ...
            realm = session1.realms().getRealmByName("original");
            ClientModel clientFromCache = session1.realms().getClientById(clientDBId, realm);
            ClientModel clientFromDB = session1.getProvider(RealmProvider.class).getClientById(clientDBId, realm);
            ...
            session1.sessions().removeUserSessions(realm);

            UserModel user1 = session1.users().getUserByUsername("john1", realm);
            UserModel user2 = session1.users().getUserByUsername("john2", realm);

            UserManager um = new UserManager(session1);
            if (user1 != null) {
            ...
            assert(session1.realms().removeRealm(realm.getId()));
    ...
    public void removeUserAttribute(KeycloakSession session) throws Exception {
        KeycloakModelUtils.runJobInTransaction(session.getKeycloakSessionFactory(), (KeycloakSession sessionSet) -> {
            ...
            RealmModel realm = sessionSet.realms().getRealm("test");

            realm = sessionSet.realms().createRealm("original");
