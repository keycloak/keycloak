...
public class OID4VCSdJwtIssuingEndpointTest extends OID4VCTest {

    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();
    private static final TimeProvider TIME_PROVIDER = new StaticTimeProvider(1000);
    ...
    @Test
    public void testRequestSdJwtCredential() {
        String token = getBearerToken(oauth);
        ObjectMapper objectMapper = new ObjectMapper();
        testingClient
                .server(TEST_REALM_NAME)
                .run((session -> {
                    AppAuthManager.BearerTokenAuthenticator authenticator = new AppAuthManager.BearerTokenAuthenticator(session);
                    authenticator.setTokenString(token);
                    OID4VCIssuerEndpoint issuerEndpoint = prepareSdJwtIssuerEndpoint(session, authenticator);
                    CredentialRequest credentialRequest = new CredentialRequest()
                            .setFormat(Format.SD_JWT_VC)
    ...
    private static OID4VCIssuerEndpoint prepareSdJwtIssuerEndpoint(KeycloakSession session, AppAuthManager.BearerTokenAuthenticator authenticator) {
        ...
        return new OID4VCIssuerEndpoint(
                session,
                "did:web:issuer.org",
                Map.of(sdJwtSigningService.locator(), sdJwtSigningService),
                authenticator,
                new ObjectMapper(),
                TIME_PROVIDER,
                30,
