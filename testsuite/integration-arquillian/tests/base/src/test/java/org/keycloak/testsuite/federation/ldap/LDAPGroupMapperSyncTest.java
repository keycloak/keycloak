...
public class LDAPGroupMapperSyncTest extends AbstractLDAPTest {
    ...
    public void test03_syncWithGroupInheritance() throws Exception {
        testingClient.server().run(session -> {
            ...
            Assertions.assertNull(KeycloakModelUtils.findGroupByPath(session, realm, "/group12"));
    ...
    public void test04_syncWithDropNonExistingGroups() throws Exception {
        testingClient.server().run(session -> {
            ...
            // Assert groups are imported to keycloak including their inheritance from LDAP
            GroupModel kcGroup1 = KeycloakModelUtils.findGroupByPath(session, realm, "/group1");
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(session, realm, "/group1/group11"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(session, realm, "/group1/group12"));
            ...
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(session, realm, "/group1/group11"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(session, realm, "/group1/group12"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(session, realm, "/model1"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(session, realm, "/group1/model2"));
            ...
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(session, realm, "/group1/group11"));
            Assertions.assertNotNull(KeycloakModelUtils.findGroupByPath(session, realm, "/group1/group12"));
            Assertions.assertNull(KeycloakModelUtils.findGroupByPath(session, realm, "/model1"));
            Assertions.assertNull(KeycloakModelUtils.findGroupByPath(session, realm, "/group1/model2"));
        });
    }
    ...
    public void test08_flatSynchWithBatchSizeLessThanNumberOfGroups() {
        ...
        testingClient.server().run(session -> {
            ...
            Assertions.assertEquals(ctx.getLdapModel().getConfig().getFirst(LDAPConstants.PAGINATION), "true");
            Assertions.assertEquals(ctx.getLdapModel().getConfig().getFirst(LDAPConstants.BATCH_SIZE_FOR_SYNC), "1");
            Assertions.assertEquals(mapperModel.getConfig().getFirst(GroupMapperConfig.PRESERVE_GROUP_INHERITANCE), "false");
            ...
            Stream.of("/group1", "/group11", "/group12").forEach(path -> {
                ...
                Assertions.assertNotNull(kcGroup);
                Assertions.assertEquals(0, kcGroup.getSubGroupsStream().count());
            });
