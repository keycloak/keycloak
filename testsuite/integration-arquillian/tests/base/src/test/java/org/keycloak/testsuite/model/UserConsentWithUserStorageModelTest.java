...
public class UserConsentWithUserStorageModelTest extends AbstractTestRealmKeycloakTest {
    ...
    public void revokeTest(KeycloakSession session) {
        KeycloakModelUtils.runJobInTransaction(session.getKeycloakSessionFactory(), (KeycloakSession sessionT) -> {
            RealmManager realmManager = new RealmManager(sessionT);
            RealmModel realm = realmManager.getRealm("original");
            ClientModel fooClient = realm.getClientByClientId("foo-client");
            UserModel john = sessionT.users().getUserByUsername("john", realm);
            UserModel mary = sessionT.users().getUserByUsername("mary", realm);

            realmManager.getSession().users().revokeConsentForClient(realm, john.getId(), fooClient.getId());
            ClientModel hardcodedClient = sessionT.realms().getClientByClientId("hardcoded-client", realm);
            realmManager.getSession().users().revokeConsentForClient(realm, mary.getId(), hardcodedClient.getId());

            realm = realmManager.getRealm("original");
            john = sessionT.users().getUserByUsername("john", realm);
            Assert.assertNull(realmManager.getSession().users().getConsentByClient(realm, john.getId(), fooClient.getId()));
            mary = sessionT.users().getUserByUsername("mary", realm);
    ...
    public void deleteUserTest(KeycloakSession session) {
        KeycloakModelUtils.runJobInTransaction(session.getKeycloakSessionFactory(), (KeycloakSession sessionDT) -> {
            // Validate user deleted without any referential constraint errors
            RealmManager realmManager = new RealmManager(sessionDT);
            RealmModel realm = realmManager.getRealm("original");
            UserModel john = sessionDT.users().getUserByUsername("john", realm);
            sessionDT.users().removeUser(realm, john);
            UserModel mary = sessionDT.users().getUserByUsername("mary", realm);
            sessionDT.users().removeUser(realm, mary);
        });
    }
