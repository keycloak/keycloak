...
/* These two tests below still need to be migrated from the old testsuite
    @Test
...
/*
    ...
    public void resetUserPassword() {
        ...
        String accountUrl = RealmsResource.accountUrl(UriBuilder.fromUri(oauth.AUTH_SERVER_ROOT)).build(REALM_NAME).toString();

        driver.navigate().to(accountUrl);

        assertEquals("Log in to admin-client-test", driver.getTitle());

        loginPage.form().login("user1", "password");
