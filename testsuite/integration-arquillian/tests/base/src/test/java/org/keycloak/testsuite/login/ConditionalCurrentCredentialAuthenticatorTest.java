...
import org.keycloak.authentication.authenticators.conditional.ConditionalCurrentCredentialAuthenticatorFactory;
 ...
 * @author rmartinc
 */
public class ConditionalCurrentCredentialAuthenticatorTest extends AbstractTestRealmKeycloakTest {
    ...
    private void configureConditionalCurrentCredentialFlow(Boolean included, String... credentials) {
        ...
        authRes.addExecution("test Browser - Conditional 2FA", Map.of("provider", ConditionalCurrentCredentialAuthenticatorFactory.PROVIDER_ID));
        AuthenticationExecutionInfoRepresentation conditionExec = authRes.getExecutions("test Browser - Conditional 2FA").stream()
                .filter(e -> ConditionalCurrentCredentialAuthenticatorFactory.PROVIDER_ID.equals(e.getProviderId())).findAny().orElse(null);
        ...
        if (included != null || (credentials != null && credentials.length > 0)) {
            ...
            if (included != null) {
                configMap.put(ConditionalCurrentCredentialAuthenticatorFactory.CONF_INCLUDED, Boolean.toString(included));
            }
            if (credentials != null && credentials.length > 0) {
                configMap.put(ConditionalCurrentCredentialAuthenticatorFactory.CONF_CREDENTIALS,
