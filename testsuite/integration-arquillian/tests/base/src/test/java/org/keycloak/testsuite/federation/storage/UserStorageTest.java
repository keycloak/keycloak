...
public class UserStorageTest extends AbstractAuthTest {
    ...
    public void addProvidersBeforeTest() throws URISyntaxException, IOException {
        ...
        File configDir = new File(System.getProperty("auth.server.config.dir", ""));
        if (stResDir.exists() && stResDir.isDirectory() && configDir.exists() && configDir.isDirectory()) {
            for (File f : stResDir.listFiles()) {
                log.infof("Copying %s to %s", f.getName(), configDir.getAbsolutePath());
                FileUtils.copyFileToDirectory(f, configDir);
            }
        } else {
        ...
        propProviderRO.getConfig().putSingle("propertyFile", "${jboss.server.config.dir}/read-only-user-password.properties");
    ...
    protected ComponentRepresentation newPropProviderRW() {
        ...
        propProviderRW.getConfig().putSingle("propertyFile", "${jboss.server.config.dir}/user-password.properties");
