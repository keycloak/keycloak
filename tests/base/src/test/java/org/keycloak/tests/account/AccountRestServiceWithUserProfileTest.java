...
public class AccountRestServiceWithUserProfileTest extends AbstractRestServiceTest {
    ...
    public void testEditUsernameAllowed() throws IOException {
        ...
        Assertions.assertNull(uam.getAnnotations());
        Map<String, Object> vc = assertValidatorExists(uam, "length");
        Assertions.assertEquals(255, vc.get("max"));

        uam = assertUserProfileAttributeMetadata(user, "lastName", "Last name", true, false);
        Assertions.assertNotNull(uam.getAnnotations());
        Assertions.assertEquals(3, uam.getAnnotations().size());
        ...
        Assertions.assertNull(getUserProfileAttributeMetadata(user, "attr_no_permission"));
    }
    ...
    public void testGetUserProfileMetadata_NoAccessToNameFields() throws IOException {
        ...
        Assertions.assertNull(getUserProfileAttributeMetadata(user, "firstName"));
        Assertions.assertNull(getUserProfileAttributeMetadata(user, "lastName"));
        assertUserProfileAttributeMetadata(user, "attr_readonly", "attr_readonly", false, true);
    ...
    protected void assertAnnotationValue(UserProfileAttributeMetadata uam, String key, Object value) {
        Assertions.assertNotNull(uam.getAnnotations(), "Missing annotations for attribute " + uam.getName());
        Assertions.assertEquals(value, uam.getAnnotations().get(key), "Unexpected value of the " + key + " annotation for attribute " + uam.getName());
    }

    protected Map<String, Object> assertValidatorExists(UserProfileAttributeMetadata uam, String validatorId) {
        Assertions.assertNotNull(uam.getValidators(), "Missing validators for attribute " + uam.getName());
        Assertions.assertTrue(uam.getValidators().containsKey(validatorId), "Missing validator " + validatorId + " for attribute " + uam.getName());
    ...
    public void testManageUserLocaleAttribute() throws IOException {
        managedRealm.updateWithCleanup(r -> r.internationalizationEnabled(false));
        registerUserCleanup("test-user@localhost");
        ...
        managedRealm.updateWithCleanup(r -> r.internationalizationEnabled(true));

        user.singleAttribute(UserModel.LOCALE, "pt_BR");
        user = updateAndGet(user, token);
        Assertions.assertEquals("pt_BR", user.getAttributes().get(UserModel.LOCALE).get(0));
        ...
        Assertions.assertTrue(metadata.getAttributes().stream()
    ...
    protected UserRepresentation getUser(String token, boolean fetchMetadata) throws IOException {
        String accountUrl = getAccountUrl(null) + "?userProfileMetadata=" + fetchMetadata;
        return AccountRestServiceTest.getUser(accountUrl, simpleHttp, token);
    }
