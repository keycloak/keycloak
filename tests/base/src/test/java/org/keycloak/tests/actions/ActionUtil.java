...
public class ActionUtil {
    ...
    public static UserRepresentation findUserInRealmRep(RealmRepresentation testRealm, String username) {
        for (UserRepresentation rep : testRealm.getUsers()) {
            if (rep.getUsername().equals(username)) {
                return rep;
            }
        }
        return null;
    ...
    public static UserRepresentation findUserWithAdminClient(RealmResource realm, String username) {
        return realm.users().search(username, null, null, null, 0, 1).get(0);
    }
    ...
    public static void addRequiredActionForRealm(RealmRepresentation testRealm, String providerId) {
        ...
        int lastPriority = requiredActions.isEmpty() ? 0 : requiredActions.get(requiredActions.size() - 1).getPriority();
        ...
        action.setPriority(lastPriority + 1);
