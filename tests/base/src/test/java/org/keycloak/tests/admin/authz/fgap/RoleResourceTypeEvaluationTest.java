...
public class RoleResourceTypeEvaluationTest extends AbstractPermissionTest {
    ...
    public void testDeleteCompositeRoleRequiresMapCompositePermission() {
        ...
        createPermission(client, myclientId, AdminPermissionsSchema.CLIENTS_RESOURCE_TYPE, Set.of(MANAGE), policy);

        // grant MAP_ROLE_COMPOSITE only on allowedSubRole
        createPermission(client, allowedSubRole.getId(), rolesType, Set.of(MAP_ROLE_COMPOSITE), policy);
    ...
    public void testUiExtRoleMappingDeleteRespectsPerRolePermission() {
        ...
        UserPolicyRepresentation policy = createUserPolicy(realm, client, "Only My Admin User Policy", myadmin.getId());
        createPermission(client, targetUser.getId(), AdminPermissionsSchema.USERS_RESOURCE_TYPE, Set.of(MAP_ROLES), policy);
        createPermission(client, allowedRole.getId(), rolesType, Set.of(MAP_ROLE), policy);
