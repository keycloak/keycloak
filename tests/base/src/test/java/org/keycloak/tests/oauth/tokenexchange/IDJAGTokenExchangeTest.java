...
public class IDJAGTokenExchangeTest {
    ...
    public void testTokenExchangeSuceedsWithValidIDJAG() throws Exception {
        ...
        runOnServer.run(session -> {
            ...
            // Pairwise Sub Mapper (SHA-256)
             if (clientScope.getProtocolMapperByName(oidcProtocol, "pairwise-sub-mapper") == null) {
                ...
                pairwiseMapper.setProtocolMapper(org.keycloak.protocol.oidc.mappers.SHA256PairwiseSubMapper.PROVIDER_ID);

                java.util.Map<String, String> config = new java.util.HashMap<>();
                config.put("sectorIdentifierUri", targetAudience + "sector");
                config.put("salt", "test-salt");
        ...
        oauth.realm("ssodomain").client("agent", "password")/* .scope(scopeName)*/.openLoginForm();
        ...
        AccessToken idjag = oauth.parseToken(tokenResponse.getAccessToken(), AccessToken.class);
        ...
        Assertions.assertNotEquals("testuser", exchangedSub, "Subject must be pseudonymized (pairwise), not the original user ID");
