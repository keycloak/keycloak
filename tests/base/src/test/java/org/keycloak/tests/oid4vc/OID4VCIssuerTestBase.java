...
public abstract class OID4VCIssuerTestBase {
    ...
    public static ProtocolMapperRepresentation getIssuedAtTimeMapper(String subjectProperty, String truncateToTimeUnit, String valueSource) {
        ...
        if (subjectProperty != null) {
            configMap.put(OID4VCIssuedAtTimeClaimMapper.CLAIM_NAME, subjectProperty);
        }
        if (truncateToTimeUnit != null) {
            configMap.put(OID4VCIssuedAtTimeClaimMapper.TRUNCATE_TO_TIME_UNIT_KEY, truncateToTimeUnit);
        }
        if (valueSource != null) {
            configMap.put(OID4VCIssuedAtTimeClaimMapper.VALUE_SOURCE, valueSource);
        }

        return createBaseMapper(subjectProperty + "-oid4vc-issued-at-time-claim-mapper", "oid4vc-issued-at-time-claim-mapper", configMap);
    }

    public static ProtocolMapperRepresentation getJtiGeneratedIdMapper() {
        return createBaseMapper("generated-id-mapper", "oid4vc-generated-id-mapper",
                Map.of(OID4VCGeneratedIdMapper.CLAIM_NAME, "jti"));
    }
    ...
    public static ProtocolMapperRepresentation getSubjectIdMapper(String subjectProperty, String attributeName) {
        return createBaseMapper(attributeName + "-mapper", "oid4vc-subject-id-mapper",
                Map.of("claim.name", subjectProperty, "userAttribute", attributeName));
    }

    public static ProtocolMapperRepresentation getStaticClaimMapper(String scopeName) {
        return createBaseMapper(UUID.randomUUID().toString(), "oid4vc-static-claim-mapper",
                Map.of("claim.name", "scope-name", "staticValue", scopeName));
    }

    public static ProtocolMapperRepresentation getUserAttributeMapper(String subjectProperty, String attributeName) {
        return createBaseMapper(attributeName + "-mapper", "oid4vc-user-attribute-mapper",
                Map.of("claim.name", subjectProperty, "userAttribute", attributeName));
    }

    private static ProtocolMapperRepresentation createBaseMapper(String name, String mapperType, Map<String, String> config) {
    ...
    public static class OID4VCIClient implements ClientConfig {
        ...
        public ClientConfigBuilder configure(ClientConfigBuilder client) {
            client.clientId(OID4VCI_CLIENT_ID)
                    ...
                    .optionalClientScopes(jwtTypeCredentialScopeName, minimalJwtTypeCredentialScopeName, sdJwtTypeCredentialConfigurationIdName, "email")
                    .redirectUris("*")
                    .secret("test-secret");
