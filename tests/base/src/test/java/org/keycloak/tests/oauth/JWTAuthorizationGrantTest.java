...
public class JWTAuthorizationGrantTest  {
    ...
    @Test
    public void testClientSignatureAlg() {
        IdentityProviderResource idpRes = realm.admin().identityProviders().get(IDP_ALIAS);
        IdentityProviderRepresentation idpRep = idpRes.toRepresentation();
        try {
            idpRep.getConfig().put("clientAssertionSigningAlg", Algorithm.ES256);
            ...
            idpRep.getConfig().put("clientAssertionSigningAlg", Algorithm.ES512);
            idpRes.update(idpRep);
            ...
            idpRep.getConfig().remove("clientAssertionSigningAlg");
            idpRes.update(idpRep);
        }
    }
