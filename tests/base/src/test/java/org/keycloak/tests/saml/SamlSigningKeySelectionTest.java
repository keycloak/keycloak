...
public class SamlSigningKeySelectionTest {
    ...
    @InjectClient(config = FrontchannelLogoutSamlClientConfig.class, lifecycle = LifeCycle.METHOD)
    ManagedClient frontchannelClient;
    ...
    /**
     * Assertion signing (Sign Assertions on, Sign Documents off): the signature sits inside the Assertion
     * element, so verify the configured key is used there rather than at the Response root.
     */
    @Test
    public void testAssertionSigningKey() throws Exception {
        configureClient(SamlConfigAttributes.SAML_SERVER_SIGNATURE, "false",
                SamlConfigAttributes.SAML_ASSERTION_SIGNATURE, "true");

        SAMLDocumentHolder response = saml.doLogin("test-user@localhost", "password");
        assertNotNull(response, "SAML response should not be null");
        assertAssertionSignedWithKey(response.getSamlDocument(), realmActiveKeyPublicKeyPem, null);
        ...
        response = saml.doLogin("test-user@localhost", "password");
        assertNotNull(response, "SAML response should not be null");
        assertAssertionSignedWithKey(response.getSamlDocument(), newKeyPublicKeyPem, realmActiveKeyPublicKeyPem);
    }

    /**
     * Logout response signing when the browser still holds the SSO session cookie: Keycloak processes
     * the LogoutRequest in the context of that authenticated session.
     */
    @Test
    ...
    /**
     * Logout response signing for an SP-initiated logout with no browser SSO cookie: the cookie is
     * cleared before logout, so Keycloak resolves the session from the LogoutRequest's NameID and
     * SessionIndex instead of the cookie. Verifies the LogoutResponse is still signed with the
     * selected key on this distinct logout path.
     */
    @Test
    public void testLogoutWithoutCookieSignedWithClientSpecificKey() throws Exception {
        ...
        // Drop the browser SSO cookie so the logout is resolved purely from the LogoutRequest.
        saml.clearCookies();
        SAMLDocumentHolder logoutResponse = saml.doLogout(nameId.getValue(), sessionIndex);
        assertNotNull(logoutResponse, "Logout response should not be null");
        assertSignedWithKey(logoutResponse.getSamlDocument(), realmActiveKeyPublicKeyPem, null);
    ...
    public void testErrorResponseSigningKey() throws Exception {
        // Passive auth only returns an error response when there is no SSO session. The WebDriver is
        // shared across methods, so drop any cookie a prior test left behind to reach the error path.
        saml.clearCookies();
    ...
    @Test
    public void testFrontchannelLogoutRequestSigningKey() throws Exception {
        // Without KID
        Document defaultDoc = performFrontchannelLogoutAndCaptureRequest(null);
        assertSignedWithKey(defaultDoc, realmActiveKeyPublicKeyPem, null);

        // With KID
        Document configuredDoc = performFrontchannelLogoutAndCaptureRequest(newKeyKid);
        assertSignedWithKey(configuredDoc, newKeyPublicKeyPem, realmActiveKeyPublicKeyPem);
    }

    /**
     * SAML frontchannel logout requires 2 clients: the logout initiator (client 1) and the
     * frontchannel target (client 2). Keycloak sends a signed LogoutRequest to client 2's POST
     * logout URL via the browser; the SP (TestSamlApp) captures it and returns a LogoutResponse
     * to complete the SLO chain so Keycloak finishes the logout cleanly.
     */
    private Document performFrontchannelLogoutAndCaptureRequest(String signingKeyKid) throws Exception {
        saml.clearLastFrontchannelLogoutDocument();
        String originalClientId = saml.getClientId();

        // Configure frontchannel client: POST logout URL, signing, frontchannel=true
        ClientRepresentation fcRep = frontchannelClient.admin().toRepresentation();
        fcRep.setFrontchannelLogout(true);
        fcRep.getAttributes().put(SamlProtocol.SAML_SINGLE_LOGOUT_SERVICE_URL_POST_ATTRIBUTE, saml.getFrontchannelLogoutUrl());
        fcRep.getAttributes().put(SamlConfigAttributes.SAML_SERVER_SIGNATURE, "true");
        if (signingKeyKid != null) {
            fcRep.getAttributes().put(SamlConfigAttributes.SAML_SERVER_SIGNATURE_KID, signingKeyKid);
        }
        frontchannelClient.admin().update(fcRep);
        // Let the mock SP answer the front-channel LogoutRequest with a LogoutResponse as this client.
        saml.setFrontchannelLogoutResponder(frontchannelClient.getClientId());

        // Login with client 1 (initiator)
        configureClient(SamlConfigAttributes.SAML_SERVER_SIGNATURE, "true");
        SAMLDocumentHolder loginResponse = saml.doLogin("test-user@localhost", "password");
        NameIDType nameId = saml.extractNameId(loginResponse);
        String sessionIndex = saml.extractSessionIndex(loginResponse);

        // Login with frontchannel client (SSO)
        saml.client(frontchannelClient.getClientId());
        saml.doLogin("test-user@localhost", "password");

        // Logout from client 1 -- triggers frontchannel logout to frontchannel client
        saml.client(originalClientId);
        saml.doLogout(nameId.getValue(), sessionIndex);

        Document doc = saml.getLastFrontchannelLogoutDocument();
        assertNotNull(doc, "Frontchannel logout request should be received");
        return doc;
    }
    ...
    public void testFallbackWhenConfiguredKeyIsPassive() throws Exception {
        ...
        configureClient(SamlConfigAttributes.SAML_SERVER_SIGNATURE, "true",
                SamlConfigAttributes.SAML_SERVER_SIGNATURE_KID, kid);
    ...
    private void assertAssertionSignedWithKey(Document doc, String expectedPublicKeyPem, String negativePublicKeyPem) {
        Element assertion = (Element) doc
                .getElementsByTagNameNS(ASSERTION_NSURI.get(), JBossSAMLConstants.ASSERTION.get()).item(0);
        assertNotNull(assertion, "Response should contain a signed Assertion element");

        PublicKey expectedKey = PemUtils.decodePublicKey(expectedPublicKeyPem);
        if (!AssertionUtil.isSignatureValid(assertion, expectedKey)) {
            fail("Assertion signature verification should succeed with the expected key");
        }

        if (negativePublicKeyPem != null) {
            PublicKey negativeKey = PemUtils.decodePublicKey(negativePublicKeyPem);
            if (AssertionUtil.isSignatureValid(assertion, negativeKey)) {
                fail("Assertion signature verification with wrong key should fail");
            }
        }
    }
    ...
    public static class FrontchannelLogoutSamlClientConfig implements ClientConfig {
        @Override
        public ClientBuilder configure(ClientBuilder client) {
            return client.clientId("http://localhost:8280/test-saml-app-frontchannel/")
                    .protocol("saml")
                    .redirectUris("http://127.0.0.1:8500/saml/acs/*")
                    .attribute(SamlConfigAttributes.SAML_AUTHNSTATEMENT, "true")
                    .attribute(SamlConfigAttributes.SAML_CLIENT_SIGNATURE_ATTRIBUTE, "false")
                    .attribute(SamlProtocol.SAML_ASSERTION_CONSUMER_URL_POST_ATTRIBUTE, "http://127.0.0.1:8500/saml/acs");
            // Note: frontchannelLogout flag and POST SLO URL are set dynamically in the test
        }
    }
}
