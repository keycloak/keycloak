...
public abstract class AbstractClientRegistrationTest {
    ...
    public static class ClientRegistrationRealmConfig implements RealmConfig {
        ...
        public RealmConfigBuilder configure(RealmConfigBuilder realm) {
            ClientRepresentation clientRep = ClientConfigBuilder.create().clientId("myclient-test")
                    .publicClient(true)
                    .directAccessGrantsEnabled(true).build();

            realm.client(clientRep);

            UserRepresentation manageClientUser = UserConfigBuilder.create()
                    .username("manage-clients")
                    .name("manage", "clients")
                    .password("password")
                    .email("manage-clients@test.com")
                    .emailVerified(true)
                    .clientRoles(Constants.REALM_MANAGEMENT_CLIENT_ID, AdminRoles.MANAGE_CLIENTS).build();

            UserRepresentation createClientUser = UserConfigBuilder.create()
                    .username("create-clients")
                    .name("create", "clients")
                    .password("password")
                    .email("create-clients@test.com")
                    .emailVerified(true)
                    .clientRoles(Constants.REALM_MANAGEMENT_CLIENT_ID, AdminRoles.CREATE_CLIENT).build();

            UserRepresentation viewClientUser = UserConfigBuilder.create()
                    .username("view-clients")
                    .name("view", "clients")
                    .password("password")
                    .email("view-clients@test.com")
                    .emailVerified(true)
                    .clientRoles(Constants.REALM_MANAGEMENT_CLIENT_ID, AdminRoles.VIEW_CLIENTS).build();

            UserRepresentation noAccessUser = UserConfigBuilder.create()
                    .username("no-access")
                    .name("no", "access")
                    .password("password")
            ...
            UserRepresentation appUser = UserConfigBuilder.create()
                    .username("test-user")
                    .name("test", "user")
                    .password("password")
            ...
            realm.addUser(manageClientUser);
            realm.addUser(createClientUser);
            realm.addUser(viewClientUser);
            realm.addUser(noAccessUser);
            realm.addUser(appUser);
