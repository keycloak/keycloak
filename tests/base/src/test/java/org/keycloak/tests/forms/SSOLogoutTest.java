...
public class SSOLogoutTest {
    ...
    @Test
    void logoutMixedProtocolSessionSamlInitiated() throws Exception {
        // OIDC Login
        oidcClient.openLoginForm();
        loginPage.assertCurrent();
        loginPage.fillLogin(managedUser.getUsername(), managedUser.getPassword());
        ...
        String code = oidcClient.parseLoginResponse().getCode();
        oidcClient.doAccessTokenRequest(code);
        ...
        AuthnRequestType loginRequest = SamlClient.createLoginRequestDocument(
                samlClient.getClientId(), acsUrl, URI.create(samlEndpoint));
        loginRequest.setProtocolBinding(SamlClient.Binding.POST.getBindingUri());
        ...
        public ClientBuilder configure(ClientBuilder client) {
            ...
            String samlEndpoint = RealmsResource.protocolUrl(UriBuilder.fromUri(keycloakUrls.getBase()))
                   .build(realm.getName(), SamlProtocol.LOGIN_PROTOCOL)
                    .toString();
            String samlClientId = RealmsResource.realmBaseUrl(UriBuilder.fromUri(keycloakUrls.getBase()))
                    .build(realm.getName())
                    .toString();
            ...
            return client.clientId(samlClientId)
                    ...
                    .attribute(SamlProtocol.SAML_SINGLE_LOGOUT_SERVICE_URL_POST_ATTRIBUTE, samlEndpoint)
