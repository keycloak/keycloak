 ...
 */
@KeycloakIntegrationTest(config = AdminClientTest.AdminClientTestServerConfig.class)
public class AdminClientTest {

    @InjectRealm(config = TestRealmConfig.class)
    ManagedRealm testRealm;

    @InjectAdminClientFactory(ref = "test")
    AdminClientFactory adminClientFactory;

    @InjectAdminClientFactory(ref = "mTlsTest", mTls = true)
    AdminClientFactory mTlsAdminClientFactory;
    ...
    public void noClientSecretWithClientCredentialsAuthSuccess() {
        ...
        try (Keycloak adminClient = mTlsAdminClientFactory.create().realm(testRealm.getName()).grantType(OAuth2Constants.CLIENT_CREDENTIALS).clientId(X509_CLIENT_ID).scope(scopeName).build()) {
            final AccessTokenResponse accessToken = adminClient.tokenManager().getAccessToken();
            Assertions.assertTrue(accessToken.getScope().contains(scopeName));
            Assertions.assertNotNull(adminClient.realm(testRealm.getName()).clientScopes().get(scopeId).toRepresentation());
        }
        // without scope and no client secret
        try (Keycloak adminClient = mTlsAdminClientFactory.create().realm(testRealm.getName()).grantType(OAuth2Constants.CLIENT_CREDENTIALS).clientId(X509_CLIENT_ID).scope(null).build()) {
            final AccessTokenResponse accessToken = adminClient.tokenManager().getAccessToken();
            Assertions.assertFalse(accessToken.getScope().contains(scopeName));
            Assertions.assertNotNull(adminClient.realm(testRealm.getName()).clientScopes().get(scopeId).toRepresentation());
        }
    ...
    static class AdminClientTestServerConfig implements KeycloakServerConfig {
