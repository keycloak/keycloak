...
public class WebAuthnOtherSettingsTest extends AbstractWebAuthnVirtualTest {
    ...
    public void defaultValues() {
        ...
        assertThat(data.hasCredentialId, is(true));
        assertThat(data.aaguid, is(ALL_ZERO_AAGUID));
        assertThat(data.hasAttestationStatement, is(false));
        assertThat(data.hasCredentialPublicKey, is(true));
        assertThat(data.counter, is(credentialCount));
        assertThat(data.attestationStatementFormat, is(AttestationConveyancePreference.NONE.getValue()));
        assertThat(data.coseAlgorithm, is((long) COSEAlgorithmIdentifier.ES256.getValue()));
        assertThat(data.coseKeyType, is(COSEKeyType.EC2.name()));
        assertThat(data.coseHasPublicKey, is(true));
    }
    ...
    public void timeout() {
        ...
        assertThat(isPasswordless() ? realmRep.getWebAuthnPolicyPasswordlessCreateTimeout() : realmRep.getWebAuthnPolicyCreateTimeout(), is(timeout));
    ...
    public void acceptableAaguidsShouldBeEmptyOrNullByDefault() {
        assertThat(getAcceptableAaguids(), anyOf(nullValue(), Matchers.empty()));
    }
    ...
    private List<String> getAcceptableAaguids() {
        RealmRepresentation realmRep = managedRealm.admin().toRepresentation();
        return isPasswordless() ? realmRep.getWebAuthnPolicyPasswordlessAcceptableAaguids() : realmRep.getWebAuthnPolicyAcceptableAaguids();
    }

    private void setWebAuthnPolicyCreateTimeout(int timeout) {
        managedRealm.updateWithCleanup(r -> isPasswordless()
                ? r.webAuthnPolicyPasswordlessCreateTimeout(timeout)
                : r.webAuthnPolicyCreateTimeout(timeout));
    }

    private void setWebAuthnPolicyRpId(String rpId) {
        managedRealm.updateWithCleanup(r -> isPasswordless()
                ? r.webAuthnPolicyPasswordlessRpId(rpId)
                : r.webAuthnPolicyRpId(rpId));
    }

    private void setAcceptableAaguidsWithDirectAttestation(List<String> aaguids) {
        final String direct = AttestationConveyancePreference.DIRECT.getValue();
        managedRealm.updateWithCleanup(r -> {
            if (isPasswordless()) {
                r.webAuthnPolicyPasswordlessAcceptableAaguids(aaguids).webAuthnPolicyPasswordlessAttestationConveyancePreference(direct);
            } else {
                r.webAuthnPolicyAcceptableAaguids(aaguids).webAuthnPolicyAttestationConveyancePreference(direct);
            }
            return r;
        });
    }

    private void disableTruststoreSpi() {
        runOnServer.run(session -> {
            ...
            if (factory instanceof FileTruststoreProviderFactory) {
                ((FileTruststoreProviderFactory) factory).setProvider(null);
            }
        });
    }

    private void reenableTruststoreSpi() {
        runOnServer.run(session -> {
            ProviderFactory<TruststoreProvider> factory = session.getKeycloakSessionFactory().getProviderFactory(TruststoreProvider.class);
            if (factory instanceof FileTruststoreProviderFactory) {
                FileTruststoreProviderFactory fileFactory = (FileTruststoreProviderFactory) factory;
                fileFactory.setProvider(fileFactory.create(session));
            }
        });
    }
