...
public class IssuerUnsupportedEncryptionAlgorithmTest extends AbstractNonHaipVciConformanceTest {
    ...
    protected Stream<ConformanceModuleVariant> moduleVariants() {
        return discoverModuleVariants(
                NON_HAIP_PLAN,
                // This module only runs when a credential response encryption is requested; the generic non-HAIP
                // plan does not pin encryption (defaults to plain), so request the encrypted variant explicitly.
                encrypted(walletInitiated()),
