...
The tests are grouped into three JUnit suites, one per profile/role, each with its own Keycloak server
configuration. CI runs every suite as a separate matrix job (`.github/workflows/conformance.yml`), so each
suite gets its own JVM. Running a single suite locally (see [Running the tests](#running-the-tests)) is equivalent.
Note that the combined `./mvnw package` invocation does **not** give per-suite process isolation: Surefire uses a
single reusable fork, so all three suites run in the same JVM.
...
Run all suites together in one invocation (all three suites share a single Surefire reusable fork, i.e. the same
JVM — this does not reproduce the per-process isolation of the CI matrix jobs):
