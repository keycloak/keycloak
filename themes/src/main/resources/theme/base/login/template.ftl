              ...
              <div class="alert-${message.type} ${properties.kcAlertClass!} pf-m-<#if message.type = 'error'>danger<#else>${message.type}</#if>">
                  <div class="pf-c-alert__icon">
                      ...
                      <#if nestedIdpUsername?? && nestedIdpHeader?? && nestedIdpSentinel??>
                          ...
                          <span class="${properties.kcAlertTitleClass!}">${kcSanitize(msg(nestedIdpHeader, _m0, _m1))?replace(_m1, ((nestedIdpUsername!)?esc)?markup_string)?replace(_m0, ((nestedIdpAlias!)?esc)?markup_string)?no_esc}</span>
                      <#else>
