...
public class KeycloakDeployment {
    ...
    public void setAuthServerBaseUrl(AdapterConfig config) {
        this.authServerBaseUrl = stripDefaultPorts(config.getAuthServerUrl());
    ...
    public void setAuthServerBackChannelBaseUrl(AdapterConfig config) {
        this.authServerBackChannelBaseUrl = stripDefaultPorts(config.getAuthServerBackChannelUrl());
    ...
    private String stripDefaultPorts(String url) {
        return url != null ? KeycloakUriBuilder.fromUri(url).build().toString() : null;
    }
    ...
    protected void resolveUrls(KeycloakUriBuilder authUrlBuilder) {
        ...
        resolveFrontChanneUrls(authUrlBuilder);
        resolveBackChannelUrls(authUrlBuilder);
    }

    protected void resolveFrontChanneUrls(KeycloakUriBuilder authUrlBuilder) {
        authUrl = KeycloakUriBuilder.fromUri(authUrlBuilder.clone().path(ServiceUrlConstants.AUTH_PATH).build(getRealm()).toString());
