...
import static org.keycloak.constants.AdapterConstants.CLIENT_SESSION_HOST;
import static org.keycloak.constants.AdapterConstants.CLIENT_SESSION_STATE;
...
public class ServerRequest {
    ...
    public static AccessTokenResponse invokeAccessCodeToToken(KeycloakDeployment deployment, String code, String redirectUri, String sessionId) throws IOException, HttpFailure {
        List<NameValuePair> formparams = buildForm(code, redirectUri, sessionId);
        return sendAccessCodeRequest(formparams, deployment);
    }

    private static List<NameValuePair> buildForm(String code, String redirectUri, String sessionId) {
        List<NameValuePair> formparams = new ArrayList<>();
        ...
        if (sessionId != null) {
            formparams.add(new BasicNameValuePair(CLIENT_SESSION_STATE, sessionId));
            formparams.add(new BasicNameValuePair(CLIENT_SESSION_HOST, HostUtils.getHostName()));
        }
        return formparams;
    }

    private static AccessTokenResponse sendAccessCodeRequest(List<NameValuePair> formparams, KeycloakDeployment deployment) throws IOException, HttpFailure {
