---
name: kc-update-release-notes
description: "Update Keycloak release notes and upgrade guide for a given version"
argument-hint: "<version> [review|add <topic>]"
allowed-tools: Bash(git log:scan git history), Bash(git tag:list tags), Bash(git branch:check current branch), Bash(gh issue *:search and view issues), Bash(gh pr *:view PRs), Bash(gh search *:search issues), Bash(gh api *:paginate API), Bash(ls docs/*:list doc files), Bash(wc:count results), Bash(grep:search file contents), Read, Edit, Agent
---
...
First, discover all patch releases for the previous minor version from upstream tags:

```bash
# List all patch release tags for the previous minor (e.g., 26.7.1, 26.7.2, ...)
gh api --paginate "repos/keycloak/keycloak/tags" --jq '.[].name' | grep -E "^{PREVIOUS_MINOR}\.[1-9]" | sort -V
```

Then check the corresponding files and GitHub issue labels for each discovered patch version:

```bash
# List patch release note files for the current minor
...
1. **Missing issue links**: Every `==` heading in the release notes and every `===` heading in the upgrade guide must have a `// https://github.com/keycloak/keycloak/issues/NNNNN` comment on the line immediately after it
...
1. Determine the cutoff date using the same method as Step 2 in Discover mode (find the previous minor release tag date).
2. Search git log for related commits: `git log --since="CUTOFF" --oneline --grep="<topic>"`
3. Find the related GitHub issue: check commit bodies for `Closes #NNNNN` or `fixes #NNNNN`
4. Read any new/modified guide files for context
5. Ask the user whether it belongs in release notes, upgrade guide, or both
6. Write the entry following the conventions below
...
- Headings should advertise the benefit, not just describe the change. Avoid generic words like "improvements", "enhancements", or "updates" — focus on the solved problem. Also avoid hollow marketing language ("at scale", "seamlessly", "effortlessly"). Good: "Track impersonation across token lifecycle and downstream services", "Manage and revoke verifiable credentials with OID4VCI". Bad: "Impersonation audit improvements", "Verifiable Credential Issuance improvements". When the entry covers multiple concrete additions, name them: "SCIM with User Profile permissions and multivalued attributes". If unsure what the specific benefit is, ask the user.
...
- **CVE references in breaking changes**: If a breaking change was introduced to fix a CVE, append the CVE number to the heading in parentheses, for example `=== Group policy matching for bare group names (CVE-2026-19608)`. Check the GitHub issue body and commit message for CVE references, and search the repository's published security advisories with `gh api "repos/keycloak/keycloak/security-advisories"`. Issues labeled `kind/weakness` are candidates but may not have an assigned CVE.
...
- **No Latin acronyms**: Do not use "e.g.", "i.e.", "etc.", "viz.", or similar. Use plain English instead: "for example", "such as", "that is", "and so on".
...
- **Audience-appropriate detail**: Write for administrators and operators, not for developers of {project_name} itself. Describe observable improvements (faster searches, lower memory usage, fewer queries) without naming internal classes, caches, or allocation patterns. Internal implementation details belong in the commit message, not in the release notes or upgrade guide. Database behavior changes (new columns, rebuilt indexes, changed transaction isolation, increased connection usage) are observable and belong in the upgrade guide because administrators manage the database. Exception: the Extension Development section of release notes and SPI-related entries in the upgrade guide target extension developers and may reference internal APIs.
...
3. Verify all `==` headings (release notes) and `===` headings (upgrade guide) have issue link comments. To find missing ones programmatically:
   ```bash
   # Release notes — check == headings
   awk '/^== [^=]/{h=NR; t=$0; next} h && NR==h+1 && !/^\/\/ https:\/\/github\.com/{print h": "t}' FILE
   # Upgrade guide — check === headings
   awk '/^=== [^=]/{h=NR; t=$0; next} h && NR==h+1 && !/^\/\/ https:\/\/github\.com/{print h": "t}' FILE
   ```
