...
the {productname} image to be deployed properly.
...
Run following commands to create an administrator account for the `master` realm of the RH-SSO server, when deploying the {productname} image directly from the image stream (without the xref:../introduction/introduction.adoc#sso-templates[template]), after the RH-SSO application pod has been started:
...
----
. Open a remote shell session to the {productname} container.
+
----
$ oc rsh sso-12-pt93n
sh-4.2$
