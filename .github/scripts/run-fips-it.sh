...
TESTS=""
PROJECT=""
# Check which testsuite to run
if [ "$2" = "new-tests" ]; then
  TESTS="FIPSTestSuite"
  PROJECT="tests"
  else
    TESTS=`testsuite/integration-arquillian/tests/base/testsuites/suite.sh fips`
    PROJECT="testsuite/integration-arquillian/tests/base"
fi
...
./mvnw test -Dsurefire.rerunFailingTestsCount=$SUREFIRE_RERUN_FAILING_COUNT -nsu -B -Pauth-server-quarkus,auth-server-fips140-2,app-server-wildfly -Dcom.redhat.fips=false $STRICT_OPTIONS -Dtest=$TESTS -pl $PROJECT 2>&1 | misc/log/trimmer.sh
